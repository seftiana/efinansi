...
// Images Directory
imagesDir = "main/lib/wysiwyg/icons/";

// CSS Directory
cssDir = "main/lib/wysiwyg/styles/";

// Popups Directory
popupsDir = "main/lib/wysiwyg/popups/";
...
//Name              buttonID                 buttonTitle           buttonImage                            buttonImageRollover
   ...
   "selectline":     ['SelectLine',           'SelectLine',         imagesDir + 'insert_line.gif',        imagesDir + 'insert_line_on.gif']
};
  ...
  toolbar += '<td>&nbsp;</td></tr></table>';  
   ...
   for (var j = 0; j <= buttonName2.length;) {
      if (buttonName2[j]) {
         ...
         else if (buttonName2[j] == "selectline"){
            toolbar2 += '<td style="width: 90px;"><span id="LineSizes'  + n + '"></span></td>';
         }
         else {
...
/* ---------------------------------------------------------------------- *\
  Function    : outputLineSizes()
  Description : creates the Line Sizes drop down and inserts it into 
	              the toolbar
  Usage       : outputLineSizes(n)
  Arguments   : n   - The editor identifier that the Font Sizes will update
	                    when making font changes (the textarea's ID)
\* ---------------------------------------------------------------------- */
function outputLineSizes(n) {

  var LineSizesObj        = ToolbarList['selectline'];
  var LineSize           = LineSizesObj[2];
  var LineSizeOn         = LineSizesObj[3];

	LineSizes.sort();
	var LineSizesDropDown = new Array;
   LineSizesDropDown[n] = '<table border="0" cellpadding="0" cellspacing="0"><tr><td onMouseOver="document.getElementById(\'selectLine' + n + '\').src=\'' + LineSizeOn + '\';" onMouseOut="document.getElementById(\'selectLine' + n + '\').src=\'' + LineSize + '\';"><img src="' + LineSize + '" id="selectLine' + n + '" width="85" height="20" onClick="showLineSizes(\'' + n + '\');" unselectable="on"><br>';
   LineSizesDropDown[n] += '<span id="Lines' + n + '" class="dropdown" style="width: 170px;">';

	for (var i = 0; i <= LineSizes.length;) {
	  if (LineSizes[i]) {
      LineSizesDropDown[n] += '<button type="button" onClick="insertHTML(\'<hr noshade size=\\\''+ LineSizes[i] + '\\\' />\',\'' + n + '\')\;hideLineSizes(\'' + n + '\');" onMouseOver="this.className=\'mouseOver\'" onMouseOut="this.className=\'mouseOut\'" class="mouseOut" style="width: 145px;"><table cellpadding="0" cellspacing="0" border="0"><tr><td width ="85"><hr noshade size="' + LineSizes[i] + '"/></td></tr></table></button><br>';	
      //FontSizesDropDown[n] += '<button type="button" onClick="insertNodeAtSelection(\'<hr size=\'' + LineSizes[i] '/>,' + n + '\')\;hideFontSizes(\'' + n + '\');" onMouseOver="this.className=\'mouseOver\'" onMouseOut="this.className=\'mouseOut\'" class="mouseOut" style="width: 145px;"><table cellpadding="0" cellspacing="0" border="0"><tr><td align="left" style="font-family: arial, verdana, helvetica;"><hr size="' + LineSizes[i] + '"/></td></tr></table></button><br>';	
    }	  
	  i++;
  }
	LineSizesDropDown[n] += '</span></td></tr></table>';
	document.getElementById('LineSizes' + n).insertAdjacentHTML("afterBegin", LineSizesDropDown[n]);
}
...
/* ---------------------------------------------------------------------- *\
  Function    : hideLineSizes()
  Description : Hides the list of font sizes in the font sizes drop down
  Usage       : hideFontSizes(n)
  Arguments   : n   - The editor identifier (the textarea's ID)
\* ---------------------------------------------------------------------- */
function hideLineSizes(n) {
  document.getElementById('Lines' + n).style.display = 'none'; 
};
...
/* ---------------------------------------------------------------------- *\
  Function    : showLineSizes()
  Description : Shows the list of line sizes in the line sizes drop down
  Usage       : showLines(n)
  Arguments   : n   - The editor identifier (the textarea's ID)
\* ---------------------------------------------------------------------- */
function showLineSizes(n) { 
  if (document.getElementById('Lines' + n).style.display == 'block') {
    document.getElementById('Lines' + n).style.display = 'none';
	}
  else {
    document.getElementById('Lines' + n).style.display = 'block'; 
    document.getElementById('Lines' + n).style.position = 'absolute';		
  }
};

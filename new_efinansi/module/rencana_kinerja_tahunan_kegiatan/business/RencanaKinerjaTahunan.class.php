...
class RencanaKinerjaTahunan extends Database
{
   ...
   public function doSaveKegiatan($param = array())
   {
      ...
      $result     &= $this->Execute($this->mSqlQueries['do_insert_kegiatan_detail'], array(
         ...
         $param['iku_id'],
         $param['iku_id'],
	 $param['jenis_rpc'], #add ccp 28-11-2019
         $this->mUserId
      ));
   ...
   public function getType($pathInfo = null)
   {
      ...
      return $type;
   	...
   	#add ccp 28-11-2019
	function GetComboJenisRPC() {
		$result = array(
			0 => array('id' => '', 'name' => '-Pilih Jenis-'),
			1 => array('id' => 'R', 'name' => 'Rutin'),
			2 => array('id' => 'P', 'name' => 'Program'),
			3 => array('id' => 'C', 'name' => 'Cadangan'),
			4 => array('id' => 'K', 'name' => 'Kerjasama')
		);
		return $result;
	}
}

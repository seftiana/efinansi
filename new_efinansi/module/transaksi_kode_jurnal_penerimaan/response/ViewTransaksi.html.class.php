<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_kode_jurnal_penerimaan/business/AppTransaksi.class.php';
...
class ViewTransaksi extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/transaksi_kode_jurnal_penerimaan/template');
	...
	function ProcessRequest() {
	  ...
	  $arr_tipe_transaksi = $Obj->GetTipeTransaksi("Penerimaan Anggaran");
	...
	function ParseTemplate($data = NULL) {
      ...
      $url_view = Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'transaksi', 'view', 'html');
      $this->mrTemplate->AddVar('content', 'URL_CHECK', $url_view);
		$this->mrTemplate->AddVar('content', 'URL_DETIL', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'detilTransaksi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_POPUP_SKENARIO', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'popupSkenario', 'view', 'html'));
	   $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'popupKegiatanDetil', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK_PENERIMAAN', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'PopupMakPenerimaan', 'view', 'html')); 
      $this->mrTemplate->AddVar('content', 'URL_POPUP_KEGIATAN_UNIT_KERJA', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'popupKegiatanUnitKerja', 'view', 'html')); 
	   $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'popupKegiatanDetil', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_SISA_ANGGARAN', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'popupSisaAnggaran', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_KODE_JURNAL', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'popupKodeJurnal', 'view', 'html'));
      $datadb = $data['datadb'];
...
//print_r($this->Data);
      //jika edit, lihat jenis transaksi, jika jenis=umum, set mak == disabled
		...
		if($data['role_name'] == "Administrator") {
			...
			$this->mrTemplate->AddVar('role', 'URL_POPUP_UNITKERJA', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'popupUnitkerja', 'view', 'html'));
      ...
      if(isset($_GET['dataId']) AND $_GET['dataId']->Integer()->Raw() > 0) {
         ...
         $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'updateTransaksi', 'do', 'html') . '&dataId=' . Dispatcher::Instance()->Encrypt($_GET['dataId']));
         if($this->Data['jenis_transaksi'] != "1") {
            ...
            $this->mrTemplate->AddVar('content', 'MAK_IS_DISABLED', 'disabled="disabled"');
         	...
         	$this->mrTemplate->AddVar('content', "MODE_20080109", 'Tambah');
		   	$this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'addTransaksi', 'do', 'html'));
      }

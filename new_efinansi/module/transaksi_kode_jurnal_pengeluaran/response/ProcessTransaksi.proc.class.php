...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
	'module/transaksi_kode_jurnal_pengeluaran/business/AppTransaksi.class.php';
   ...
   var $cssAlert = "notebox-alert";
	...
	function __construct() {
		...
		$this->pageView = Dispatcher::Instance()->GetUrl(
											'transaksi_kode_jurnal_pengeluaran', 
											'Transaksi', 
											'view', 
											'html');
	...
	function Add() { 
		...
		if($cek === true) {
         //if(empty($this->_POST['no_invoice_list'])) {
           // Messenger::Instance()->Send('transaksi_kode_jurnal', 'Transaksi', 'view', 'html', array($this->_POST,'Nomor Ivoice harap diisi', $this->cssAlert),Messenger::NextRequest);
        ...
        // }else{   
            ...
            $arrData['transDueDate'] = $this->_POST['due_date_year'] . "-" . $this->_POST['due_date_mon'] . "-" . $this->_POST['due_date_day'];
            ...
            if($id_transaksi != "") {
               ...
               $detil_pembukuan = $this->Obj->DoAddPembukuanDetilKodeJurnal($id_pembukuan, $this->_POST['COA']);
        ...
        // }
         ...
         //echo "666";
         Messenger::Instance()->Send('transaksi_kode_jurnal_pengeluaran', 'Transaksi', 'view', 'html', array($this->_POST,'Transaksi Dengan Nomor <b>'.($this->_POST['no_kkb']).'</b> Sudah Dibuat', $this->cssFail),Messenger::NextRequest);
      } elseif($cek === "COA") {
         //echo "666";
         Messenger::Instance()->Send('transaksi_kode_jurnal_pengeluaran', 'Transaksi', 'view', 'html', array($this->_POST,'Kode Jurnal belum dipilih!', $this->cssFail),Messenger::NextRequest);
      } else {
         //gagal masukin data
         Messenger::Instance()->Send('transaksi_kode_jurnal_pengeluaran', 'Transaksi', 'view', 'html', array($this->_POST,'Gagal Menambah Data', $this->cssFail),Messenger::NextRequest);
      }
         //exit();
      ...
      $cek = true;
		if($cek === true) {
         ...
         #}else{
            ...
            $arrData['transTtId'] = $this->_POST['tipe_transaksi'];
            /**
             * cek apakah unitkerja id dan tipe transaksi berubah
             * jika berubah naka generate no bukti transaksi lagi sesui dengan
             * unitkerja id dan tipe transaksi yang baru
             */
         ...
         //gagal masukin data
         //echo "gagal";
         Messenger::Instance()->Send('transaksi_kode_jurnal_pengeluaran', 'Transaksi', 'view', 'html', array($this->_POST,'Gagal Merubah Data Transaksi', $this->cssFail),Messenger::NextRequest);
		   return $this->pageView . '&dataId=' . $this->encId;
      }
      //echo "adflj";
		return $this->pageView;

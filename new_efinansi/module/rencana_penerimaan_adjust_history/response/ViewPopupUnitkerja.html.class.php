...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
			'module/rencana_penerimaan_adjust_history/business/AppPopupUnitkerja.class.php';
...
class ViewPopupUnitkerja extends HtmlResponse {
	...
	var $Pesan;
	protected $mPopupUnitKerjaObj;
	protected $mUserUnitKerjaObj;
	...
	public function ProcessRequest() {
		$this->mPopupUnitKerjaObj = new AppPopupUnitkerja();
		
		$this->mUserUnitKerjaObj = new UserUnitKerja();
		$userId = trim(Security::Instance()->mAuthentication->GetCurrentUser()->GetUserId());
		
		$role               = $this->mUserUnitKerjaObj->GetRoleUser($userId);
		$unitkerjaUserId    = $this->mUserUnitKerjaObj->GetUnitKerjaUser($userId);
		//print_r($unitkerjaUserId);
	...
	//view
		...
		$totalData = $this->mPopupUnitKerjaObj->GetCountListDataUnitkerja(
		    $kode, 
		    $unitkerja, 
		    $tipeunit, 
		    $role, 
		...
		$dataUnitkerja = $this->mPopupUnitKerjaObj->GetListDataUnitkerja(
						 	  $startRec, 
						 	  $itemViewed, 
						 	  $kode, 
						 	  $unitkerja, 
      ...
      $dest = "popup-subcontent";
		...
		$arr_tipeunit = $this->mPopupUnitKerjaObj->GetDataTipeunit();
		...
		$return['userUnitKerjaId']=$unitkerjaUserId ;
	...
	public function ParseTemplate($data = NULL) {
		...
		if (empty($data['dataUnitkerja'])) {
			...
			for ($i=0; $i<sizeof($dataUnitkerja); $i++) {
				$dataUnitkerja[$i]['enc_unitkerja_id'] = 
							Dispatcher::Instance()->Encrypt($dataUnitkerja[$i]['unitkerja_id']);
				$dataUnitkerja[$i]['enc_unitkerja_nama'] = 
							Dispatcher::Instance()->Encrypt($dataUnitkerja[$i]['unitkerja_nama']);
			}
			...
			for ($i=0; $i<sizeof($dataUnitkerja); $i++) {
				...
				if($this->mPopupUnitKerjaObj->GetTotalSubUnitKerja($dataUnitkerja[$i]['id'])> 0 OR $dataUnitkerja[$i]['parentId'] == 0) {

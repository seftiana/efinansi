...
class ViewMovementAnggaranApproval extends HtmlResponse {
    ...
    public function TemplateModule() {
        ...
        $this->setTemplateFile('movement_anggaran_approval.html');
    }
    ...
    public function ParseTemplate($data = null) {
        ...
        $url_search = Dispatcher::Instance()->GetUrl('movement_anggaran_approval', 'MovementAnggaranApproval', 'view', 'html');
        $url_detail = Dispatcher::Instance()->GetUrl('movement_anggaran_approval', 'DetailMovementAnggaranApproval', 'view', 'html');
        ...
        if (empty($dataList)) {
            ...
            for ($i = 0; $i < count($dataList); $i++) {
                ...
                $dataList[$i]['nilai'] = number_format($dataList[$i]['nilai'], 0, ',', '.');
                ...
                $dataList[$i]['url_approval'] = Dispatcher::Instance()->GetUrl(
                                'movement_anggaran_approval', 'UpdateMovementAnggaranApproval', 'view', 'html'
                        ) . '&id=' . $dataList[$i]['id'];
                $this->mrTemplate->AddVar('status_approval', 'STATUS', strtoupper($dataList[$i]['status_approval']));
                $this->mrTemplate->AddVar('status_approval_button', 'STATUS', strtoupper($dataList[$i]['status_approval']));
                $this->mrTemplate->AddVar('status_approval_button', 'ID', $dataList[$i]['id']);
                $this->mrTemplate->AddVar('status_approval_button', 'URL_APPROVAL', $dataList[$i]['url_approval']);

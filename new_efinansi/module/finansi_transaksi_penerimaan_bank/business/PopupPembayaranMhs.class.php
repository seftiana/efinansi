 ...
 * @package     : finansi_transaksi_penerimaan_bank
 * scope        : PUBLIC
 * @Author      : noor hadi
 * @Created     : 2015-04-24
 * @Modified    : 2015-04-24
...
class PopupPembayaranMhs extends Database {
    ...
    public function __construct($connectionNumber = 0) {
        $this->mSqlFile = 'module/finansi_transaksi_penerimaan_bank/business/popup_pembayaran_mhs.sql.php';
    ...
    public function getDataPembayaranPengakuan($param = array(), $method = 'post') {
        ...
        if (!empty($result['data_list'])) { 
            ...
            /*
            if (!empty($result['data_list']['deposit_masuk'])) {
                $depositMasuk = null;
                foreach ($result['data_list']['deposit_masuk'] as $value) {
                    $depositMasuk[$value['prodi']]['nama_prodi'] = $value['nama_prodi'];
                    $depositMasuk[$value['prodi']]['pemasukan_deposit'] = $value['pemasukan_deposit'];
                    $depositMasuk[$value['prodi']]['penanggung_jawab'] = $value['penanggung_jawab'];
                    $depositMasuk[$value['prodi']]['keterangan'] = $value['keterangan'];
                }
                
                $result['data_list']['deposit_masuk'] = $depositMasuk;
            }
            */
            ...
            if (!empty($result['data_list']['pengakuan'])) {
                ...
                foreach ($result['data_list']['pengakuan'] as $value) {
                    ...
                    //jumlah pengakuan per dari perodi
                    $pengakuanProdiJumlah[(int) $value['prodi']]['nominal'] += $value['nominal'];
                    $pengakuanProdiPotJumlah[(int) $value['prodi']]['nominal_pot'] += $value['potongan'];
                    $pengakuanProdiDepJumlah[(int) $value['prodi']]['nominal_dep'] += $value['penggunaan_deposit'];
                    //jumlah pengakuan per jenis biaya
                    $pengakuanJumlah[(int) $value['prodi']][(int) $value['id_jenis_biaya']]['nominal'] += $value['nominal'];
                    $pengakuanPotJumlah[(int) $value['prodi']][(int) $value['id_jenis_biaya']]['nominal_pot'] += $value['potongan'];
                    $pengakuanDepJumlah[(int) $value['prodi']][(int) $value['id_jenis_biaya']]['nominal_dep'] += $value['penggunaan_deposit'];
                    
                    $pengakuan[(int) $value['prodi']]['data_jb'][] = array(
                        'prodi' => (int) $value['prodi'],
                        'nama_prodi' => $value['nama_prodi'],
                        'jenis_biaya_id' => $value['id_jenis_biaya'],
                        'jenis_biaya' => $value['jenis_biaya'],
                        ...
                        'penggunaan_deposit' => $value['penggunaan_deposit'],
                        'penanggung_jawab' => $value['penanggung_jawab'],
                        'keterangan' => $value['keterangan'],
                        'id_detil' => $value['id_detil']
                    );
                ...
                foreach ($pengakuan as $value) {
                    if (is_array($value['data_jb']) && !empty($value['data_jb'])) {
                        foreach ($value['data_jb'] as $jb) {
                            $pengakuanUrut[$index]['prodi'] = (int) $jb['prodi'];
                            $pengakuanUrut[$index]['nama_prodi'] = (empty($jb['nama_prodi']) ? '-' : $jb['nama_prodi']);
                            $pengakuanUrut[$index]['jenis_biaya_id'] = $jb['jenis_biaya_id'];
                            $pengakuanUrut[$index]['jenis_biaya'] = $jb['jenis_biaya'];
                            $pengakuanUrut[$index]['nominal'] = (int) $jb['nominal'];
                            $pengakuanUrut[$index]['potongan'] = (int) $jb['potongan'];
                            $pengakuanUrut[$index]['penggunaan_deposit'] = (int) $jb['penggunaan_deposit'];
                            
                            $pengakuanUrut[$index]['total_nominal'] = $pengakuanJumlah[(int) $jb['prodi']][(int) $jb['jenis_biaya_id']]['nominal'];
                            $pengakuanUrut[$index]['total_potongan'] = $pengakuanPotJumlah[(int) $jb['prodi']][(int) $jb['jenis_biaya_id']]['nominal_pot'];
                            $pengakuanUrut[$index]['total_penggunaan_deposit'] = $pengakuanDepJumlah[(int) $jb['prodi']][(int) $jb['jenis_biaya_id']]['nominal_dep'];
                            
                            $pengakuanUrut[$index]['total_prodi_nominal'] = $pengakuanProdiJumlah[(int) $jb['prodi']]['nominal'];
                            $pengakuanUrut[$index]['total_prodi_potongan'] = $pengakuanProdiPotJumlah[(int) $jb['prodi']]['nominal_pot'];
                            $pengakuanUrut[$index]['total_prodi_penggunaan_deposit'] = $pengakuanProdiDepJumlah[(int) $jb['prodi']]['nominal_dep'];
                            
                            $pengakuanUrut[$index]['penanggung_jawab'] = $jb['penanggung_jawab'];
                            $pengakuanUrut[$index]['keterangan'] =$jb['keterangan'];
                            $pengakuanUrut[$index]['id_detail'] = $jb['id_detil'];

...
class AppReferensi extends Database
{
   ...
   {
      $this->mSqlFile   = 'module/finansi_realisasi_pencairan/business/app_referensi.sql.php';
   ...
   public function getTahunAnggaran(){
       $taData = $this->Open($this->mSqlQueries['get_tahun_anggaran'],array());       
       return $taData;
   }
   
   public function getTahunAnggaranAktifId(){
       $taData = $this->Open($this->mSqlQueries['get_tahun_anggaran_aktif_id'],array());
       if(!empty($taData)){
           return $taData[0]['id'];
       } else {
           return null;
   ...
   function GetKomponenAnggaran( $param = array())
   {
      //$this->SetDebugOn();
        $dataResult    = $this->Open($this->mSqlQueries['get_komponen_anggaran'], 
            array(
                $param['ta_id'],
                $param['unit_id']
            )
      ...
      if(empty($dataResult)){
         ...
         for ($i=0; $i < count($data_komponen);) {
            if((int)$pId === (int)$data_komponen[$i]['p_id']){
               $data_grid[$pId][$index]    = $data_komponen[$i];
               $i++;
               $index++;
            }else{
               $pId      = (int)$data_komponen[$i]['p_id'];
               unset($index);
               $index      = 0;
            }
         }

...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/jurnal_kode/business/AppPopupCoa.class.php';
...
class PopupCoa extends HtmlResponse {
   ...
   function ProcessRequest() {	  	
     ...
     for ($i=0; $i<count($dataGrid); $i++) {
      ...
      $url = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, 
               ...
               Dispatcher::Instance()->mType).'&data[kode]='.$this->data['kode'].'&data[nama]='.$this->data['nama'];
			   
      Messenger::Instance()->SendToComponent('paging', 'Paging', 'view', 'html', 'paging_top', 
         array($itemViewed,$totalData, $url, $currPage, "popup-subcontent"), 
   ...
   function ParseTemplate($data = NULL) {      
      ...
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('jurnal_kode', 'coa', 'popup', 'html'));	  
	  if (isset ($data['msg'])) {	     
      ...
      if (empty($data['dataGrid'])) {	     
         ...
         for ($i=0; $i<sizeof($dataGrid); $i++) {              			   
            ...
            if(!$dataGrid[$i]['isParent']) {
               $dataGrid[$i]['set_parent'] ='<div id=\''.$dataGrid[$i]['kode'].'\' name=\''.$dataGrid[$i]['kode'].'\'><a href="javascript:void(0)" onclick="setParent(\''.$dataGrid[$i]['kode'].'\', \''.$dataGrid[$i]['id'].'\',\''.$dataGrid[$i]['kode'].'\',\''.$dataGrid[$i]['nama'].'\', 1)" onmouseover="status=\'Set preferences...\';return true" ><img src="images/button-check.gif" alt="Pilih"/>DEBET</a>';          
               $dataGrid[$i]['set_parent'] .='<a href="javascript:void(0)" onclick="setParent(\''.$dataGrid[$i]['kode'].'\', \''.$dataGrid[$i]['id'].'\',\''.$dataGrid[$i]['kode'].'\',\''.$dataGrid[$i]['nama'].'\', 0)" onmouseover="status=\'Set preferences...\';return true" ><img src="images/button-check.gif" alt="Pilih"/>KREDIT</a></div>';

...
'module/laporan_rekap_angsuran/business/LaporanRekapAnggaranBelanjaBulanan.class.php';

class ViewLaporanRekapAnggaranBelanjaBulanan extends HtmlResponse
{
   function TemplateModule(){
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
      'module/laporan_rekap_angsuran/template/');
...
//echo "cui<pre>";var_dump ($dataList);
//echo "</pre>";
      $urlSearch     = Dispatcher::Instance()->GetUrl(
         'laporan_rekap_angsuran',
         ...
         for ($i=0; $i < count($dataList);) {
            if(
              ...
              //$index--;
			   ...
			   $ttl_pinjaman =$dataList[$i]['pinjaman']+$ttl_pinjaman;
			   $sisa_pinjaman =$dataList[$i]['sisapinjaman']+$sisa_pinjaman;
               ...
               $dataGrid[$index]['pinjaman']    = $dataList[$i]['pinjaman'] = number_format($dataList[$i]['pinjaman'], 0, ',','.');
               $dataGrid[$index]['sisapinjaman']= $dataList[$i]['sisapinjaman'] = number_format($dataList[$i]['sisapinjaman'], 0, ',','.');
               $dataGrid[$index]['nominal']       = $dataList[$i]['bayar'];
...
//echo"<pre>" ;print_r($dataGrid);echo"</pre>";
         foreach ($dataGrid as $grid) {
          ...
          $totalNominal = '';
          foreach($getHeaderBulan as $key => $bulan) {                  
            $totalNominal  .= '<td style="text-align:right;">'.number_format($nominalPerBulan['total'][$bulan['kode_bulan']], 0, ',','.').'</td>';                           
          }  
          $this->mrTemplate->AddVar('data_grid', 'TOTAL_NOMINAL_PER_BULAN', $totalNominal);
          $this->mrTemplate->AddVar('data_grid', 'TOTAL_NOMINAL', number_format($nominalPerBulan['jml_total'],0,',','.'));      
          $this->mrTemplate->AddVar('data_grid', 'TOTAL_PINJAMAN',number_format($ttl_pinjaman,0,',','.'));       
          $this->mrTemplate->AddVar('data_grid', 'TOTAL_SISA_PINJAMAN',number_format($sisa_pinjaman,0,',','.'));       
        }
   }
}
?>
...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
		'module/transaksi_spj_pertransaksi/business/AppTransaksiDua.class.php';
...
class ViewTransaksi extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
					'module/transaksi_spj_pertransaksi/template');
		$this->SetTemplateFile('view_transaksi.html');
	}
	
	function ProcessRequest() {
      //instance2
		$Obj = new AppTransaksiDua();
      ...
      if(isset($_GET['dataId'])) {
         ...
         //tanggal2
         $due_date_selected =   $due_date_year . "-" . 
		 						$due_date_mon . "-" . 
					 			$due_date_day;
      ...
      if(!empty($this->_POST['check']) AND $this->_POST['check'] == 'yes') {
         ...
         $count_bukti_trans = $Obj->CountBuktiTrans( $this->Data['tipe_transaksi'], 
		                                             date("m"), 
													 $this->Data['unitkerja']);
      ...
      if(!empty($this->_GET['check']) AND $this->_GET['check'] == 'yes') {
		...
		*/
      //due_date
      Messenger::Instance()->SendToComponent( 'tanggal', 
											  'Tanggal', 
											  'view', 
											  'html', 
											  'due_date', 
											  array( $due_date_selected, 
								  			         $periode_awal, 
											  		 $periode_akhir), 
      ...
      Messenger::Instance()->SendToComponent( 'tanggal', 
											  ...
											  'html', 
											  'tanggal_transaksi', 
											  array( $tanggal_transaksi_selected, 
											         $periode_awal, 
													 $periode_akhir), 
						                      Messenger::CurrentRequest);
      ...
      $arr_jenis_transaksi = $Obj->GetComboJenisTransaksi();
		Messenger::Instance()->SendToComponent( 'combobox', 
												...
												'html', 
												'jenis_transaksi', 
												array( 'jenis_transaksi', 
												       $arr_jenis_transaksi, 
													   5, "false", 
													   ' style="width:200px;" id="jenis_transaksi" 
													   onchange="showHideMAK(this)"'), 
									           Messenger::CurrentRequest);
      ...
      $arr_tipe_transaksi = $Obj->GetComboTipeTransaksi();
		Messenger::Instance()->SendToComponent( 'combobox', 
												...
												'html', 
												'tipe_transaksi', 
												array( 'tipe_transaksi', 
												       $arr_tipe_transaksi, 
													   6, "false", 
													   ' style="width:200px;" onChange="jsCheck(this.value)" 
													   id="tipe_transaksi"'), 
									   			Messenger::CurrentRequest);
	...
	{
		$url_view = Dispatcher::Instance()->GetUrl('transaksi_spj_pertransaksi', 'transaksi', 'view', 'html');
		...
		$this->mrTemplate->AddVar('content', 'URL_POPUP_SKENARIO', 
      	...
      	$this->mrTemplate->AddVar('content', 'URL_POPUP_KODE_JURNAL', 
		  								Dispatcher::Instance()->GetUrl(
										  								'transaksi_spj_pertransaksi',
										  								'popupKodeJurnal', 
										  								'view', 
      	...
      	$this->mrTemplate->AddVar('content', 'URL_POPUP_COA', 
										Dispatcher::Instance()->GetUrl(
																		'transaksi_spj_pertransaksi', 
																		'popupCoa', 
																		'view', 
	   	...
	   	$this->mrTemplate->AddVar('content', 'URL_POPUP_COA_KREDIT', 
		   								Dispatcher::Instance()->GetUrl(
										   								'transaksi_spj_pertransaksi',
																		'popupCoaKredit', 
																		'view', 
	...
	//	$this->mrTemplate->AddVar('content', 'NO_KKB', $datadb['no_kkb']);
		...
		$this->mrTemplate->AddVar('content', 'PENANGGUNG_JAWAB', $datadb['penanggung_jawab']);
      	...
      	$this->mrTemplate->AddVar('content', 'COUNT_NO_BUKTI_UNIT', $data['count_bukti_trans']['unit_kerja_kode'].
		  									'/'.date("m").'.'.date("Y"));
      	*/
		//print_r($data);
		//print_r($this->Data);

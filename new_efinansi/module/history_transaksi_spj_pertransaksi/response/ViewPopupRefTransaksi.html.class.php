...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/history_transaksi_spj_pertransaksi/business/AppPopupRefTransaksi.class.php';
...
class ViewPopupRefTransaksi extends HtmlResponse {
   ...
   function ProcessRequest() {	       
	...
	$this->Ref = new AppPopupRefTransaksi;	
   $this->data['referensi'] = '';  	  
   ...
   function ParseTemplate($data = NULL) {      
      ...
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('history_transaksi_spj_pertransaksi', 'PopupRefTransaksi', 'view', 'html'));
      ...
      if (empty($data['dataGrid'])) {	     
         ...
         for ($i=0; $i<sizeof($dataGrid); $i++) {              			   
			   ...
			   $dataGrid[$i]['catatan_js1'] = str_replace(array("\r\n","\r","\n"),'\n',htmlentities($dataGrid[$i]['catatan']));
            $dataGrid[$i]['catatan_js'] = str_replace("'","\'",$dataGrid[$i]['catatan_js1']);

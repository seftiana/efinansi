...
class PopupCoa extends HtmlResponse {
   ...
   function ProcessRequest() {
	...
	//view
		$totalData = $kodeObj->GetCountKodePenerimaan($kodeCoa, $namaCoa);
		$itemViewed = 20;
		$currPage = 1;
		$startRec = 0 ;
		if(isset($_GET['page'])) {
		...
		$dataList = $kodeObj->GetDataKodePenerimaan($startRec, $itemViewed, $kodeCoa, $namaCoa);
//		print_r($dataList);
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, 'coa', 'popup', 'html'));

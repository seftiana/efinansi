...
class ViewTransaksiDetil extends HtmlResponse {
    ...
    function ParseTemplate($data = NULL) {
        ...
        $this->mrTemplate->AddVar('content', 'URL_DETIL', Dispatcher::Instance()->GetUrl(
                        'transaksi_realisasi_penerimaan', 'detilTransaksi', 'view', 'html'));
        $this->mrTemplate->AddVar('content', 'URL_TAMBAH', Dispatcher::Instance()->GetUrl(
        ...
        /**
          $this->mrTemplate->AddVar('content', 'COUNT_NO_BUKTI_COUNT',
          $data['count_bukti_trans']['count_trans']);
        ...
        if ($data['role_name'] == "Administrator") {
            ...
            $this->mrTemplate->AddVar('role', 'URL_POPUP_UNITKERJA', Dispatcher::Instance()->GetUrl(
                            'transaksi_realisasi_penerimaan', 'popupUnitkerja', 'view', 'html'));
        ...
        if (isset($_GET['dataId']) AND $_GET['dataId']->Integer()->Raw() > 0) {
            ...
            if ($this->Data['jenis_transaksi'] != "1") {
                ...
                $this->mrTemplate->AddVar('content', "NOMINAL_DISETUJUI", number_format($data['datadbMAK']['nominal_disetujui'], 2, ',', '.')
                );
                $this->mrTemplate->AddVar('content', "SISA", number_format($data['datadbMAK']['sisa'], 2, ',', '.'));
            } else {

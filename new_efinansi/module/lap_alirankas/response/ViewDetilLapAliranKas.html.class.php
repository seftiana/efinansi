<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/lap_alirankas/business/AppDetilLapAliranKas.class.php';

class ViewDetilLapAliranKas extends HtmlResponse {

   var $Pesan;

   function TemplateModule() {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/lap_alirankas/template');
      $this->SetTemplateFile('view_detil_lap_aliran_kas.html');
   }
   ...
   function ProcessRequest() {
      ...
      $tanggal_awal=Dispatcher::Instance()->Decrypt($_GET['tgl_awal']);
      $tanggal=Dispatcher::Instance()->Decrypt($_GET['tgl']);
      $isKasSetaraKas=Dispatcher::Instance()->Decrypt($_GET['ksk']);
      $tanggalKas=Dispatcher::Instance()->Decrypt($_GET['tgl_kas']);
   //view
      ...
      if ($isKasSetaraKas == 'Ya'){
         $data_list = $Obj->GetDataDetilKlpLaporanKasSetaraKas($tanggalKas, $id_kel_lap, $startRec, $itemViewed); 
      }else{
         $data_list = $Obj->GetDataDetilKlpLaporan($tanggal_awal,$tanggal, $id_kel_lap, $startRec, $itemViewed);
      }
      //print_r($data_list);

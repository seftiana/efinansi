...
class ViewPopupUnitkerja extends HtmlResponse {
	...
	function ProcessRequest() {
		...
		//foreach($role AS $field=>$val):
		//	echo $field." = ".$val."<br />";
		//endforeach;
		//foreach($unitkerjaUserId AS $u=>$d):
		//	echo $u." = ".$d."<br />";
		//endforeach;
	...
	//view
		...
		$totalData 			= $this->unitKerjaObj->GetCountListDataUnitkerja(
		    $kode, 
		    $unitkerja, 
		    $tipeunit, 
		    $role, 
		...
		$dataUnitkerja 		= $this->unitKerjaObj->GetListDataUnitkerja(
						 	  $startRec, 
						 	  $itemViewed, 
						 	  $kode, 
						 	  $unitkerja, 
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', 
								  Dispatcher::Instance()->
								  GetUrl('realisasi_pencairan_2', 'popupUnitkerja', 'view', 'html') . 
		...
		if (empty($data['dataUnitkerja'])) {
			...
			for ($i=0;$i<sizeof($dataUnitkerja);$i++) {
				...
				if($this->unitKerjaObj->GetTotalSubUnitKerja(
						$dataUnitkerja[$i]['id']) > 0  OR $dataUnitkerja[$i]['parentId'] == 0) {
					$dataUnitkerja[$i]['class_name'] 	= 'table-common-even1';
				}
				//$datas		= str_replace("'","\'",$dataUnitkerja[$i]);

...
class ViewRealisasiPencairan extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $sheet->setCellValue('C11', 'Yang akan mengambil uang : Non Cash');
	  #
	  $sheet->setCellValue('J5', 'KODE DOKUMEN : F-KUA-001.01');
      $sheet->getStyle('J5')->applyFromArray(array(
         'font' => array(
            'size' => 11,
            'bold' => true
         ), 'alignment' => array(
      ...
      $sheet->setCellValue('H13', 'Sisa Anggaran (Rp)'); #update ccp 15-8-2019
      ...
      for ($i=0; $i < $maxRow; $i++) {
         ...
         if(!empty($dataList[$i])){
            $rowHeight  = max(array(
		      ...
		      $sisa = $dataList[$i]['anggaran_approve']-$dataList[$i]['total_fpa']; #add ccp 15-8-2019
            ...
            //$sheet->setCellValueExplicit('H'.$row, $dataList[$i]['anggaran_approve'], PHPExcel_Cell_DataType::TYPE_NUMERIC);
	         $sheet->setCellValueExplicit('H'.$row, $sisa, PHPExcel_Cell_DataType::TYPE_NUMERIC); #add ccp 15-8-2019
            $sheet->setCellValueExplicit('I'.$row, '');//, PHPExcel_Cell_DataType::TYPE_NUMERIC);
            $sheet->setCellValue('J'.$row, $dataList[$i]['keterangan']);

            if( $dataList[$i]['is_pengadaan'] === 'Y') {
               $isPengadaan += 1;
            } 
            $nomor+=1;
         }
	 ...
	 $sheet->setCellValue('I'.($rows+7), $namaKabiroKeuangan);
         // $sheet->setCellValue('J'.($rows+7), $namaRektor);

         $sheet->mergeCells('B'.($rows+8).':E'.($rows+8));
         $sheet->setCellValue('B'.($rows+8), 'Staf/Kabag/Kaprodi/Sekretaris');
         #$sheet->setCellValue('E'.($rows+8), 'Kabiro/Direktur/Dekan*');
         $sheet->setCellValue('F'.($rows+8), 'Dekan/Biro/Direktur');

         $sheet->setCellValue('G'.($rows+8), 'Kabag Keuangan');
         #$sheet->setCellValue('H'.($rows+8), 'Kabiro Keuangan');
         $sheet->setCellValue('I'.($rows+8), 'Kabiro Keuangan dan Akuntansi');
         ...
         // pengadaan / mengandung pengadaan ( 3 penanda tangan operasional )
         $sheet->getRowDimension(($rows+1))->setRowHeight(18);
         $sheet->mergeCells('B'.($rows+1).':G'.($rows+1));
         $sheet->setCellValue('B'.($rows+1), 'Persetujuan Transaksi Operasional');

         $sheet->mergeCells('H'.($rows+1).':J'.($rows+1));
         $sheet->setCellValue('H'.($rows+1), 'Persetujuan Atas Pencairan Anggaran');
         ...
         $sheet->setCellValue('B'.($rows+3), 'Yang Mengajukan,');
         #$sheet->setCellValue('E'.($rows+3), 'Mengetahui,');
         $sheet->setCellValue('F'.($rows+3), 'Mengetahui,');
         $sheet->setCellValue('G'.($rows+3), 'Menyetujui,');

         $sheet->mergeCells('B'.($rows+7).':E'.($rows+7));
         $sheet->setCellValue('B'.($rows+7), str_repeat('.', 25));
         #$sheet->setCellValue('E'.($rows+7), str_repeat('.', 25));
         $sheet->setCellValue('F'.($rows+7), str_repeat('.', 25));
         $sheet->setCellValue('G'.($rows+7), str_repeat('.', 25));
         $sheet->getStyle('B'.($rows+7).':G'.($rows+7))->applyFromArray(array(
            'font' => array(
               'underline' => true
            )
         ));
         ...
         #$sheet->setCellValue('E'.($rows+8), 'Kabiro/Direktur/Dekan*');
         $sheet->setCellValue('F'.($rows+8), 'Dekan/Biro/Direktur');
         $sheet->setCellValue('G'.($rows+8), 'Rektor/Wakil Rektor');

         ///
         $sheet->setCellValue('H'.($rows+7), $namaKasubagKeuangan);
         $sheet->setCellValue('H'.($rows+7), $namaKabiroKeuangan);
         #$sheet->setCellValue('J'.($rows+7), $namaWarekSdm);
         // $sheet->setCellValue('J'.($rows+7), $namaRektor);

         $sheet->setCellValue('H'.($rows+8), 'Kabag Keuangan');
         #$sheet->setCellValue('H'.($rows+8), 'Kabiro Keuangan');
         $sheet->setCellValue('J'.($rows+8), 'Kabiro Keuangan dan Akuntansi');
         // $sheet->setCellValue('J'.($rows+8), 'Rektor');

         $sheet->getStyle('B'.($rows+7).':J'.($rows+8))->getFont()->setBold(true);
         $sheet->getStyle('G'.($rows+2).':G'.($rows+10))->applyFromArray(array(
            'borders' => array(
               'right' => array(
                  'style' => PHPExcel_Style_Border::BORDER_THIN,
                  'color' => array('argb' => 'ff000000')
      ...
      #add ccp request keuangan 16-12-2019
	  $sheet->setCellValue('G'.($rows+13), 'Diisi oleh bagian Keuangan');
	  $sheet->getStyle('G'.($rows+14).':H'.($rows+14))->applyFromArray(array(
         'borders' => array(
            'top' => array(
               'style' => PHPExcel_Style_Border::BORDER_THIN,
               'color' => array('argb' => 'ff000000')
	  ...
	  $sheet->getStyle('G'.($rows+13).':H'.($rows+14))->applyFromArray(array(
         'borders' => array(
            'bottom' => array(
               'style' => PHPExcel_Style_Border::BORDER_THIN,
               'color' => array('argb' => 'ff000000')
	  ...
	  $sheet->getStyle('G'.($rows+13).':H'.($rows+16))->applyFromArray(array(
		 'font' => array(
            'bold' => true
         ),
         'alignment' => array(
	  ...
	  $sheet->getStyle('G'.($rows+14).':H'.($rows+17))->applyFromArray(array(
         'borders' => array(
            'left' => array(
               'style' => PHPExcel_Style_Border::BORDER_THIN,
               'color' => array('argb' => 'ff000000')
	  ...
	  $sheet->getStyle('G'.($rows+17).':H'.($rows+17))->applyFromArray(array(
         'borders' => array(
            'bottom' => array(
               'style' => PHPExcel_Style_Border::BORDER_THIN,
               'color' => array('argb' => 'ff000000')
      ...
      $sheet->setCellValue('H'.($rows+14), 'FPA');
      $sheet->setCellValue('G'.($rows+16), 'No. :');
		#end

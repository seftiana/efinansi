...
/**
 * Class ProcessSubAccountKeenam
 * @package sub_account_keenam
 * @copyright 2011 gamatechno
 */

require_once GTFWConfiguration::GetValue( 'application', 'docroot') .
'module/sub_account_keenam/business/SubAccountKeenam.class.php';
...
class ProcessSubAccountKeenam {
   ...
   {
      $this->mSubAccoutKeenam = new SubAccountKeenam();
      $queryString             = $this->mSubAccoutKeenam->_getQueryString();
   ...
   {
      $requestData      = array();
      if($this->mSubAccoutKeenam->method == 'post'){
         $kode       = preg_replace('/\s[\s]+/', '', $this->mSubAccoutKeenam->_POST['kode']);
         $kode       = preg_replace('/[\s\W]+/', '', $kode);
         $kode       = preg_replace('/^[\_]+/', '', $kode);
         $kode       = preg_replace('/[\_]+$/', '', $kode);
         $requestData['id']      = $this->mSubAccoutKeenam->_POST['dataId'];
         $requestData['kode']    = $kode;
         $requestData['nama']    = $this->mSubAccoutKeenam->_POST['nama'];
      }
   ...
   private function checkData()
   {
      ...
      $unique           = $this->mSubAccoutKeenam->doCheckUniqueData(
         $requestData['kode'],
         $requestData['id']
      );
      if($unique === false){
   ...
   public function Update()
   {
      ...
      if($checkData['result'] === false){
         ...
         $process             = $this->mSubAccoutKeenam->DoUpdate(
            $this->mData['kode'],
            $this->mData['nama'],
            $this->mData['id']
         );
   ...
   public function Delete()
   {
      ...
      if($this->mSubAccoutKeenam->method != 'post'){
         ...
         $arrId         = $this->mSubAccoutKeenam->_POST['idDelete'];
         $success       = 0;
         $failed        = 0;
         foreach ($arrId as $id) {
            $delete     = $this->mSubAccoutKeenam->DoDelete($id);
            if($delete === true){
               $success+=1;
            }else{
               $failed+=1;

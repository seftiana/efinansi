...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') .
	'module/history_transaksi_pencairan/business/transaksi_realisasi_penerimaan/AppTransaksi.class.php';

require_once GTFWConfiguration::GetValue( 'application', 'docroot') .
	'module/user_unit_kerja/business/UserUnitKerja.class.php';

class ViewHTFormRealisasiPenerimaan extends HtmlResponse {
	...
	function TemplateModule() {
		...
		$this->SetTemplateFile('transaksi_realisasi_penerimaan/view_transaksi.html');
		//$userRealName = trim(Security::Instance()->mAuthentication->GetCurrentUser()->GetRealName());
	}
	...
	function ProcessRequest() {
      ...
      if(isset($_GET['dataId'])) {
		...
		//////print_r($userRealName);
         ...
         $this->Data['tipe_transaksi'] = $datadb['tipe'];
			//print_r($datadb['tipe']);
			//$datadb['penanggung_jawab'] = $userRealName;
      ...
      $arr_tipe_transaksi = $Obj->GetComboTipeTransaksi();
		Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'tipe_transaksi', array('tipe_transaksi', $arr_tipe_transaksi, 1, "false", ' style="width:200px;"  id="tipe_transaksi"'), Messenger::CurrentRequest);
         //print_r($arr_tipe_transaksi);
	...
	function ParseTemplate($data = NULL) {
        $url_view = Dispatcher::Instance()->GetUrl(
	  								'history_transaksi_pencairan', 'HTFormRealisasiPenerimaan', 'view', 'html');
		$userRealName = trim(Security::Instance()->mAuthentication->GetCurrentUser()->GetRealName());
		$this->mrTemplate->AddVar('content', 'URL_CHECK', $url_view);
		$this->mrTemplate->AddVar('content', 'URL_BATAL',
		...
		$this->mrTemplate->AddVar('content', 'URL_POPUP_SKENARIO',
					Dispatcher::Instance()->GetUrl(
								'history_transaksi_pencairan',
								'popupSkenario',
								'view',
	   ...
	   $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK',
	   				Dispatcher::Instance()->GetUrl(
					   			'history_transaksi_pencairan',
					   			'popupKegiatanDetil',
					   			'view',
      ...
      $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK_PENERIMAAN',
	  				Dispatcher::Instance()->GetUrl(
					  			'history_transaksi_pencairan',
					            'PopupMakPenerimaan',
								'view',
      ...
      $this->mrTemplate->AddVar('content', 'URL_POPUP_KEGIATAN_UNIT_KERJA',
	  				Dispatcher::Instance()->GetUrl(
					  			'history_transaksi_pencairan',
					    		'popupKegiatanDetil',
								'view',
	   ...
	   #$this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl('transaksi_realisasi_penerimaan', 'popupKegiatanDetil', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_SISA_ANGGARAN',
	  				Dispatcher::Instance()->GetUrl(
					  			'history_transaksi_pencairan',
					  			'popupSisaAnggaran',
					  			'view',
       ...
       */
		 ...
		 * end
		 */
		$this->mrTemplate->AddVar('content', 'NOMINAL', $datadb['nominal']);//////print_r($datadb['nominal']);
      ...
      if(!empty($datadb['penanggung_jawab'])){
		...
		$this->mrTemplate->AddVar('content', 'PENANGGUNG_JAWAB', $userRealName);
		}
		...
		$this->mrTemplate->AddVar('content', 'JSON_JENIS_TRANS_LIST', $data['jns_transaksi']);
      	...
      	$this->mrTemplate->AddVar('content', 'COUNT_NO_BUKTI_UNIT',
		  	$data['count_bukti_trans']['unit_kerja_kode'].'/'.date("m").'.'.date("Y"));
      ...
      if(isset($_GET['dataId']) AND $_GET['dataId']->Integer()->Raw() > 0) {
         ...
         if($this->Data['jenis_transaksi'] != "1") {
            //////print_r($data['datadbMAK']);
		      $this->mrTemplate->AddVar('content', "MAK_LABEL", $data['datadbMAK']['nama']);
		      $this->mrTemplate->AddVar('content', "MAK", $data['datadbMAK']['id']);
		      //$this->mrTemplate->AddVar('content', "MAK_LAMA", $data['datadbMAK']['kode']);
		      //$this->mrTemplate->AddVar('content', "MAK_LAMA_ID", $data['datadbMAK']['id']);
				$this->mrTemplate->AddVar('content', "NOMINAL_DISETUJUI", $data['datadbMAK']['nominal_disetujui']);
				$this->mrTemplate->AddVar('content', "SISA", $data['datadbMAK']['sisa']);
         } else {
      	...
      	if(isset($datadb['is_jurnal']) && $datadb['is_jurnal'] == 'Y'){
			$status = 'auto';
      	} else {
      		$status ='manual';
   	  	}

      $this->mrTemplate->AddVar('content', 'SKENARIO', $status);
      $this->mrTemplate->AddVar('content', 'SKENARIO_LABEL', $status);

...
SELECT
       ...
       substr(coaKodeAkun,1,1) AS coa_kelompok_id, 
       ...
       IF(akumulasi_rl.flag = 'LR',
            IF((akumulasi_rl.flag = 'LR' AND  akumulasi_rl.nominal_rl < 0),(akumulasi_rl.nominal_rl *-1),0),
             IFNULL( jurnal.nominalDebet,0))+IFNULL(fromBB.debet,0)  AS nominal_debet,
       IF(akumulasi_rl.flag = 'LR',
	         IF((akumulasi_rl.flag = 'LR' AND  akumulasi_rl.nominal_rl > 0),akumulasi_rl.nominal_rl,0),       
	         IFNULL( jurnal.nominalKredit,0))+IFNULL(fromBB.kredit,0) AS nominal_kredit
    FROM 
        coa
        ...
        LEFT JOIN (
           ...
           WHERE  
               (
                  bbTanggal < (select tp.tppTanggalAwal from tahun_pembukuan_periode tp where tp.tppIsBukaBuku ='Y')
                  OR
                  bbTanggal  BETWEEN '%s' AND '%s'
               )
            ...
            LEFT JOIN (
                ...
                FROM transaksi tr
                 ...
                 WHERE
                   ...
                   and pr.prIsPosting = 'T'                       
                   AND (tr.`transTanggalEntri` < (select tp.tppTanggalAwal from tahun_pembukuan_periode tp where tp.tppIsBukaBuku ='Y')
                   OR tr.`transTanggalEntri` BETWEEN '%s' AND '%s')
                   ) AS nominal_rl
                FROM coa c
...
FROM
    ...
    LEFT JOIN (
        SELECT
            ...
            IF(akumulasi_rl.flag = 'LR',
               IF((akumulasi_rl.flag = 'LR' AND  akumulasi_rl.nominal_rl < 0),(akumulasi_rl.nominal_rl *-1),0),
               sum(IF(UPPER(jurnal.pdStatus) = 'D', jurnal.pdNilai, 0)) ) + IFNULL(fromBB.debet,0)  AS nominalDebet,
       		IF(akumulasi_rl.flag = 'LR',
               IF((akumulasi_rl.flag = 'LR' AND  akumulasi_rl.nominal_rl > 0),akumulasi_rl.nominal_rl,0),       
               sum(IF(UPPER(jurnal.pdStatus) = 'K', jurnal.pdNilai, 0))) + IFNULL(fromBB.kredit,0)  AS nominalKredit       
        FROM
            coa         
            JOIN coa_kelompok_laporan_ref
                ON coakellapCoaId = coaId

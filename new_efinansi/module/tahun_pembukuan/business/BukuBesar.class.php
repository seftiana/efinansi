...
class BukuBesar extends Database {
   ...
   function DoSaveJurnalTutupBukuRL($params) {
	    // get user loggin
		...
		$noref = date('Ymd',strtotime($params['tgl_trans'])).'.'. $getCoaDebetId[0]['coaKodeAkun'].'.'.$getCoaKreditId[0]['coaKodeAkun'];
		$catatanTrans = 'Tutup Buku - '.$getCoaDebetId['0']['coaNamaAkun'];
		// create transaction   			
		$result = true;		
		$this->StartTrans();
		...
		$result &= $this->InsertBukuBesarHistoryTutupBuku($param_lr_berjalan_his);
		// kredit
		$result &= $this->Execute($this->mSqlQueries['do_insert_pembukuan_detail'], array(
			$pembukuanId,
		...
		$result &= $this->InsertBukuBesarHistoryTutupBuku($param_lr_awal_tahun_his);

		
		$result &= $this->UpdateBukuBesarTutupBuku($params['param_lr_awal_tahun']);
		$result &= $this->UpdateBukuBesarTutupBuku($params['param_lr_berjalan']);   
		// end 

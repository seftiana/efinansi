...
class ViewInputRkaklProgram extends HtmlResponse {

	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot') . 'module/rkakl_program/template');
		$this->SetTemplateFile('input_rkakl_program.html');
	}
	...
	function ProcessRequest() {
		...
		$data = $Obj->GetRkaklProgramById($idDec);
	...
	function ParseTemplate($data = NULL) {
      ...
      $this->mrTemplate->AddVar('content', 'KODE', empty($dataK[0]['kode'])?$data['Data']['kode']:$dataK[0]['kode']);
		...
		$this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('rkakl_program', 'inputRkaklProgram', 'do', 'html') . "&dataId=" . Dispatcher::Instance()->Encrypt($data['decDataId']));

...
class ViewUnitkerja extends HtmlResponse {
	...
	public function TemplateModule() {
		...
		$this->SetTemplateFile('view_unitkerja.html');
	}
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_ADD_UNIT',
					Dispatcher::Instance()->GetUrl('unitkerja_tree', 'inputUnitkerja', 'view', 'html') .
							'&jenis=unit');
		$this->mrTemplate->AddVar('content', 'URL_UNITKERJA_TREE',
                Dispatcher::Instance()->GetUrl('unitkerja_tree', 'UnitKerjaTree', 'view', 'html'));
                                    /**
                                    '&kode=' . Dispatcher::Instance()->Encrypt($search['kode']) .
                                    '&nama=' . Dispatcher::Instance()->Encrypt($search['unitkerja']) .
                                    '&tipeunit=' . Dispatcher::Instance()->Encrypt($search['tipeunit']));
                                    */

        $this->mrTemplate->AddVar('content', 'URL_CARI',

...
FROM 
    ...
    LEFT JOIN transaksi tr ON tr.`transId` = real_pen.`realterimaTransId`
    LEFT JOIN unit_kerja_ref uk ON uk.`unitkerjaId` = tr.`transUnitkerjaId`
    LEFT JOIN rencana_penerimaan rpen ON rpen.`renterimaId`  = real_pen.`realrenterimaId`
    LEFT JOIN kode_penerimaan_ref kref ON kref.`kodeterimaId` = rpen.`renterimaKodeterimaId`
    LEFT JOIN finansi_coa_map cmap ON cmap.`kodeterimaId` = kref.`kodeterimaId`
    LEFT JOIN coa c ON c.`coaId` = cmap.`coaId`
    LEFT JOIN kode_penerimaan_ref kref_parent ON kref_parent.`kodeterimaId` = kref.`kodeterimaParentId`
WHERE
    tr.`transTanggalEntri`  BETWEEN '%s' AND '%s'
...
//get penerimaan dari transaksi penerimaan bank
$sql['get_nominal_per_item_penerimaan_bank'] = "
SELECT
    '1' AS identitas,
    IFNULL(kref.`kodeterimaId`,0) AS kelompokId,
    IFNULL(kref.`kodeterimaKode` ,0) AS kelompokKode,
    IFNULL(c.`coaId`,0 ) AS subKelompokId,
    IFNULL(c.`coaKodeAkun`,0) AS subKelompokKode,
    SUM(pdet.`pdNilai`) AS nominal
FROM
   ...
   LEFT JOIN rencana_penerimaan rterima ON
    rterima.`renterimaId` = tb.`transaksiBankPenerimaanId` AND rterima.`renterimaUnitkerjaId` = tb.`transaksiBankUnitId`
   LEFT JOIN kode_penerimaan_ref kref ON kref.`kodeterimaId` = kref.`kodeterimaParentId`
   LEFT JOIN pembukuan_referensi pref ON pref.`prTransId` = tr.`transId`
   LEFT JOIN pembukuan_detail pdet ON pdet.`pdPrId` = pref.`prId`
   LEFT JOIN coa c ON c.`coaId` = pdet.`pdCoaId`
WHERE
    tb.transaksiBankTipe = 'penerimaan'
    AND tb.transaksiBankTipeTransaksi NOT IN ('piutang','pengakuan_depmasuk')
    AND pdet.`pdStatus` = 'D'
...
$sql['get_nominal_per_unit_penerimaan_bank']="
SELECT
    tb.`transaksiBankUnitId` AS unitKerjaId,
    '1' AS identitas,
    IFNULL(kref.`kodeterimaId`,0) AS kelompokId,
    IFNULL(kref.`kodeterimaKode` ,0) AS kelompokKode,
    IFNULL(c.`coaId`,0 ) AS subKelompokId,
...
$sql['get_nominal_per_kelompok_unit_penerimaan_bank'] ="
SELECT
    tb.`transaksiBankUnitId` AS unitKerjaId,
    '1' AS identitas,
    IFNULL(kref.`kodeterimaId`,0) AS kelompokId,    
    IFNULL(c.`coaId`,0 ) AS coaId,
    SUM(pdet.`pdNilai`) AS nominal
FROM
   finansi_pa_transaksi_bank AS tb
   JOIN unit_kerja_ref uk ON uk.`unitkerjaId` = tb.`transaksiBankUnitId`
...
$sql['get_total_per_kelompok_penerimaan_bank'] ="
SELECT
    IFNULL(kref.`kodeterimaId`,0) AS kelompokId,
    SUM(pdet.`pdNilai`) AS nominal
FROM
...
$sql['get_nominal_per_penerimaan_bank'] ="
SELECT
    tb.`transaksiBankUnitId` AS unitKerjaId,
    '1' AS identitas,
    IFNULL(c.`coaId`,0 ) AS coaId,
...
$sql['get_total_per_penerimaan_bank'] ="
SELECT
    SUM(pdet.`pdNilai`) AS nominal
FROM
   finansi_pa_transaksi_bank AS tb

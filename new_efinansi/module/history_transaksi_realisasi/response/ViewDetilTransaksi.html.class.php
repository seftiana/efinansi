...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
	'module/history_transaksi_realisasi/business/AppDetilTransaksi.class.php';
...
class ViewDetilTransaksi extends HtmlResponse 
{
	...
	{
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/history_transaksi_realisasi/template');
	...
	{
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('history_transaksi_realisasi', 'detilTransaksi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_KEMBALI', Dispatcher::Instance()->GetUrl('history_transaksi_realisasi', 'Transaksi', 'view', 'html'));
		if($this->Pesan) {
		...
		if (empty($data['dataDetilTransaksi'])) {
         ...
         $label = 'Transaksi';
         $urlDelete = 'history_transaksi_realisasi|deleteTransaksi|do|html';
         $urlReturn = 'history_transaksi_realisasi|detilTransaksi|view|html';
         $URLDELETE = Dispatcher::Instance()->GetUrl('history_transaksi_realisasi', 'deleteTransaksi', 'do', 'html');
...
//mulai bikin tombol delete
         ...
         $j=0; #print_r($dataDetilTransaksi);
            ...
            $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('history_transaksi_realisasi', 'FormCetakBKK', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkk';
            ...
            $dataDetilTransaksi[$i]['nominal_label'] = number_format($dataDetilTransaksi[$i]['nominal'], 2, ',', '.');
				...
				$dataDetilTransaksi[$i]['url_cetak'] = Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'FormCetakTransaksi', 'view', 'html') . '&dataId=' . $idEnc;
            ...
            '&label='.$label.'&dataName='.$dataDetilTransaksi[$i]['kkb'];
               $url_edit = Dispatcher::Instance()->GetUrl('history_transaksi_realisasi', 'Transaksi', 'view', 'html') . '&dataId=' . $idEnc;

...
class ViewPopupUnitkerja extends HtmlResponse {
   ...
   function TemplateBase() {
	...
	function ProcessRequest() {
		...
		$satker=$_GET['satker']->mrVariable;
		$satker_label=$_GET['satker_label']->mrVariable;
		
      if (isset($_GET['uk']->mrVariable)){
         $uk=$_GET['uk']->mrVariable;
      }else{
         $uk=$_GET['satker']->mrVariable;
      };
	...
	//view
		...
		$dataUnitkerja = $unitkerjaObj->GetDataUnitkerja($startRec, $itemViewed, $satker, $kode, $unitkerja, $tipeunit,$uk);
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('usulan_kegiatan', 'popupUnitkerja', 'view', 'html') . "&satker=" . Dispatcher::Instance()->Encrypt($search['satker']) . "&satker_label=" . Dispatcher::Instance()->Encrypt($search['satker_label']));
		if($this->Pesan) {
		...
		if (empty($data['dataUnitkerja'])) {
			...
			for ($i=0; $i<sizeof($dataUnitkerja); $i++) {
				...
				$dataUnitkerja[$i]['link'] = str_replace("'","\'",$dataUnitkerja[$i]['unitkerja_nama']);

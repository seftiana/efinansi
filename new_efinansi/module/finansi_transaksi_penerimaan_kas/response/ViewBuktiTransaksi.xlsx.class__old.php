...
class ViewBuktiTransaksi extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $sheet->setCellValue('H2', date('d-m-Y', strtotime($data_transaksi['tanggal'])));
      ...
      $sheet->setCellValue('A3', 'BUKTI PENERIMAAN KAS');
      ...
      for ($i=0; $i < $maxRow; $i++) {
         ...
         if(!empty($transaksi_detail[$i])){
            $sheet->setCellValue('A'.$row, $transaksi_detail[$i]['keterangan']);
         ...
         $getHeightRow = ceil(strlen($transaksi_detail[$i]['keterangan'])/45) * 14;     

...
class ViewBuktiTransaksi extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $row  = 11;
      $rowAwal  = $row;
      $maxRow  = count($transaksi_detail);
      for ($i=0; $i < $maxRow; $i++) {
         $sheet->mergeCells('A'.$row.':E'.$row);
         $sheet->mergeCells('F'.$row.':I'.$row);
		 
         if(!empty($transaksi_detail[$i])){
            $sheet->setCellValue('A'.$row, strtoupper($transaksi_detail[$i]['nama']));
               ...
               $getCountContent = ceil(strlen($transaksi_detail[$i]['nama'])/30); 
               if($getCountContent > 10){
                  $getHeightRow = ceil(strlen($transaksi_detail[$i]['nama'])/30) * 20;     
                  $sheet->getRowDimension($row)->setRowHeight($getHeightRow);
               }elseif($getCountContent > 20){
                  $getHeightRow = ceil(strlen($transaksi_detail[$i]['nama'])/30) * 25;     
                  $sheet->getRowDimension($row)->setRowHeight($getHeightRow);
               }else{
                  $getHeightRow = ceil(strlen($transaksi_detail[$i]['nama'])/30) * 16;     
                  $sheet->getRowDimension($row)->setRowHeight($getHeightRow);
               }
         }
         $row++;

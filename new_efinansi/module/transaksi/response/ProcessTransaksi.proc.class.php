...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
	'module/transaksi/business/AppTransaksi.class.php';
...
class ProcessTransaksi {
   ...
   var $cssAlert = "notebox-alert";
	...
	function __construct() {
		...
		$this->pageView = Dispatcher::Instance()->GetUrl('transaksi', 'Transaksi', 'view', 'html');
		$this->pageDetil = Dispatcher::Instance()->GetUrl('history_transaksi', 'HTTransaksi', 'view', 'html');
		$this->pageInputDetil = Dispatcher::Instance()->GetUrl('transaksi', 'transaksiDetil', 'view', 'html');
	...
	function Add() { 
        ...
        // }else{   
            ...
            if($id_transaksi != "") {
               ...
               if($this->_POST['mak'] != "") {
                  ...
                  if($this->_POST['tipe_transaksi'] == 5 AND $this->_POST['unitkerja'] != 1)
                     $pencairan = $this->Obj->DoAddTransaksiDetilPengembalianAnggaran(
					 								$id_transaksi, $this->_POST['mak']);
         ...
         //echo "666";
         Messenger::Instance()->Send('transaksi', 'Transaksi', 'view', 'html', array($this->_POST,'Transaksi Dengan Nomor <b>'.($this->_POST['no_kkb']).'</b> Sudah Dibuat', $this->cssFail),Messenger::NextRequest);
      } else {
         //gagal masukin data
         Messenger::Instance()->Send('transaksi', 'Transaksi', 'view', 'html', array($this->_POST,'Gagal Menambah Data', $this->cssFail),Messenger::NextRequest);
      }
         //exit();
	...
	function Update() {
      ...
      $cek = true;
		if($cek === true) {
         ...
         //}else{
            ...
            if(($this->_POST['unitkerja'] == $this->_POST['unitkerja_lama']) &&
			    ($this->_POST['tipe_transaksi'] == $this->_POST['tipe_transaksi_lama'])
				){
        ...
        // }
         ...
         //gagal masukin data
         //echo "gagal";
         Messenger::Instance()->Send('transaksi', 'Transaksi', 'view', 'html', array($this->_POST,'Gagal Merubah Data Transaksi', $this->cssFail),Messenger::NextRequest);
		   return $this->pageView . '&dataId=' . $this->encId;
      }
      //echo "adflj";
		return $this->pageView;
	...
	function Delete() {
      ...
      $deleteArrData = $this->Obj->DoDeleteDataById($arrId);  
		if($deleteArrData === true) {
			Messenger::Instance()->Send('history_transaksi', 'HTTransaksi', 'view', 'html', array($this->_POST,'Penghapusan Data Berhasil Dilakukan', $this->cssDone),Messenger::NextRequest);
		} else {
			...
			Messenger::Instance()->Send('history_transaksi', 'HTTransaksi','view', 'html', array($this->_POST, $gagal . ' Data Tidak Dapat Dihapus.', $this->cssFail),Messenger::NextRequest);
		}

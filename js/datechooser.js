...
/**
@Edited by: Gabri Nurtinaz Shally (gabri.ns@gmail.com)
@Date: November 28, 2008
@Info: Now depend on janime.js (mouseOutDateChooser)
**/
...
if (!objPHPDate)
{
	var objPHPDate =
	{
		...
		aDay: ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jum\'at', 'Sabtu'],
		aShortDay: ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'],
		aLetterDay: ['M', 'S', 'S', 'R', 'K', 'J', 'S'],
		aMonth: ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'],
		aShortMonth: ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'],
...
function DateChooser()
{
	...
	var fnUpdate = null;
	var sDefaultIcon = 'images/datechooser.png';
	var objUpdateFields = {};
	...
	var AddClickEvents = function()
	{
		...
		{
			DcEvents.add(aNavLinks[nNavLink], 'click', function(e)
			{
				e = e || DcEvents.fix(event);
		...
		{
			DcEvents.add(aDateLinks[nDateLink], 'click', function(e)
			{
				e = e || DcEvents.fix(event);
				var ndClicked = e.target || e.srcElement;
				if (ndClicked.nodeName == '#text') ndClicked = ndClicked.parentNode;
	...
	this.setLink = function()
	{
		...
		DcEvents.add(ndAnchor, 'click', this.display);
	...
	this.setIcon = function()
	{
		...
		DcEvents.add(ndIcon, 'click', this.display);
	...
	var clickWindow = function(e)
	{
		...
		{
			if (!document.getElementById('calendar' + nCount)) continue;
			if (!document.getElementById('calendar' + nCount).style) continue;
			if (!document.getElementById('calendar' + nCount).style.visibility) continue;
	...
	{
		anime = new janime();
		if (nTimeout > 0) objTimeout = setTimeout('for (i = 0; i < 20; i++) setTimeout("anime.element = \'' + sDateChooserID + '\'; anime.alpha("+(100 - (i*5))+");", i * 25); setTimeout(\'document.getElementById("' + sDateChooserID + '").style.visibility = "hidden"; anime.element = "' + sDateChooserID + '"; anime.alpha(100); if (document.getElementById("iframehack")) document.getElementById("iframehack").style.display = "none";\', i * 25);', nTimeout);
      
		return true;
	};

	DcEvents.add(ndDateChooser, 'mouseover', mouseoverDateChooser);
	DcEvents.add(ndDateChooser, 'mouseout', mouseoutDateChooser);
	DcEvents.add(document, 'mousedown', clickWindow);
...
if (!DcEvents)
{
	...
	DcEvents.fix.preventDefault = function()
	{
	...
	DcEvents.fix.stopPropagation = function()
	{

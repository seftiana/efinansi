...
'module/rencana_pengeluaran/business/RencanaPengeluaran.class.php';

class ViewRencanaPengeluaran extends HtmlResponse
{
   function TemplateModule(){
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application', 'docroot') .
         'module/rencana_pengeluaran/template');
      $this->SetTemplateFile('view_rencana_pengeluaran.html');
   }
         ...
         for ($i=0; $i < count($dataList);) {
            if((int)$dataList[$i]['program_id'] === (int)$programId && (int)$dataList[$i]['kegiatan_id'] === (int)$kegiatanId && (int)$dataList[$i]['id'] === (int)$subkegiatanId){
               ...
               if(strtoupper($dataList[$i]['jenis_kegiatan_nama']) == 'RUTIN' OR is_null($dataList[$i]['jenis_kegiatan_nama'])){
                  ...
                  $urlAksi          = Dispatcher::Instance()->GetUrl(
                     'rencana_pengeluaran',
                     'inputRencanaPengeluaranNonRutin',
                     'view',
                     'html'
                  );
               }
...
$url_cetak_rtf = Dispatcher::Instance()->GetUrl('rencana_pengeluaran', 'RtfRencanaPengeluaran', 'view', 'html'). '&grp=' . $dataGrid[$i]['kegiatandetail_id'];
//}else{
//$url_cetak_rtf = Dispatcher::Instance()->GetUrl('rencana_pengeluaran', 'RtfRencanaPengeluaranApproved', 'view', 'html'). '&grp=' . $dataGrid[$i]['kegiatandetail_id'];
//}
...
$url_cetak_rkakl = Dispatcher::Instance()->GetUrl('rencana_pengeluaran', 'cetakRencanaPengeluaranRkakl', 'view', 'html'). '&grp=' . $dataGrid[$i]['kegiatandetail_id'];

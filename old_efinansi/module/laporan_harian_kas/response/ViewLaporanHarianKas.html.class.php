...
class ViewLaporanHarianKas extends HtmlResponse
{
   ...
   function ProcessRequest(){
      ...
      $return     = compact('query_string', 'request_data', 'data_list', 'start', 'saldoAwal');
      return $return;
   }
   ...
   function ParseTemplate($data = null){
      ...
      if(empty($data_list)){
         ...
         foreach($saldoAwal as $saldo){
            if($saldo < 0){
               $saldo_awal  = '('.number_format(abs($saldo['saldo_awal']), 2, ',','.').')';
               $saldo_total  = '('.number_format(abs($saldo['saldo_awal']+$saldo_akhir), 2, ',','.').')';
            }else{
               $saldo_awal  = number_format($saldo['saldo_awal'], 2, ',','.');
               $saldo_total = number_format($saldo['saldo_awal']+$saldo_akhir, 2, ',','.');
            }
         }
         ...
         if($mutasi_jumlah < 0){
            $mutasi_jumlah    = '('.number_format(abs($mutasi_jumlah), 2, ',','.').')';
         }else{
            $mutasi_jumlah    = number_format($mutasi_jumlah, 2, ',','.');
         }
         ...
         $this->mrTemplate->AddVar('data_grid', 'SALDO_AKHIR', $saldo_total);
      }
   }
}
?>
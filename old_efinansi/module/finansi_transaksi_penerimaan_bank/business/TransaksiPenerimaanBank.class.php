...
class TransaksiPenerimaanBank extends Database {
    ...
    public function __construct($connectionNumber = 0) {
        $this->mSqlFile = 'module/finansi_transaksi_penerimaan_bank/business/transaksi_penerimaan_bank.sql.php';
   ...
   public function getTahunPembukuanPeriode($param = array())
   {
    ...
    public function doSaveJurnal($param = array()) {
        ...
        $queryLog[] = sprintf($this->mSqlQueries['do_insert_transaksi_bank'], $nomorReferensi, $nomorReferensi, $param['tanggal'], $param['nama_penyetor'], $param['nama_penerima'], $param['nominal_debet'], $userId, $param['unit_kerja_id'], $param['rpen_id'], $param['rpen_nominal'], (empty($param['skenario_id']) ? NULL : $param['skenario_id']), $tipeTransaksiBank, $param['pemb_nominal'], $param['auto_number']);
    ...
    public function doUpdateData($param = array()) {
        ...
        //hapus table finansi_transaksi_pemerinaan_bank_detail untuk diisi lagi
        $result &= $this->Execute(
                $this->mSqlQueries['do_delete_transaksi_bank_detail_transaksi'], array(
            $transaksiId
                )
        );

        $queryLog[] = sprintf($this->mSqlQueries['do_update_transaksi_bank'], $nomorReferensi, $nomorReferensi, $param['tanggal'], $param['nama_penyetor'], $param['nama_penerima'], $param['nominal_debet'], $userId, $param['unit_kerja_id'], $param['rpen_id'], $param['rpen_nominal'], (empty($param['skenario_id']) ? NULL : $param['skenario_id']), $tipeTransaksiBank, $param['pemb_nominal'], $param['auto_number'], $transaksiId);

...
class PopupKodePenerimaan extends HtmlResponse {
	...
	function TemplateModule() {
		...
		$this->SetTemplateFile('popup_kode_penerimaan.html');
	}
   ...
   function ProcessRequest() {
		...
		if($_POST || isset($_GET['cari'])) {
			if(isset($_POST['kode'])) {
				...
				$kodeCode = Dispatcher::Instance()->Decrypt($_GET['kode']);
				$kodeNama = Dispatcher::Instance()->Decrypt($_GET['nama']);
			} else {
				$kodeCode = '';
				$kodeNama = '';
			}
	...
	//view
		$totalData = $kodeObj->GetCountKodePenerimaan($kodeCode, $kodeNama);
		$itemViewed = 20;
		$currPage = 1;
		$startRec = 0 ;
		if(isset($_GET['page'])) {
		...
		$dataList = $kodeObj->GetDataKodePenerimaan($startRec, $itemViewed, $kodeCode, $kodeNama);
//		print_r($dataList);
		$url = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, Dispatcher::Instance()->mSubModule, Dispatcher::Instance()->mAction, Dispatcher::Instance()->mType 
				. '&kode=' . Dispatcher::Instance()->Encrypt($kodeCode)
				. '&nama=' . Dispatcher::Instance()->Encrypt($kodeNama)
		...
		$return['search']['kode'] = $kodeCode;
		$return['search']['nama'] = $kodeNama;
		return $return;
	}
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('pm_jenis_biaya', 'KodePenerimaan', 'popup', 'html'));
		;
		if($this->Pesan) {

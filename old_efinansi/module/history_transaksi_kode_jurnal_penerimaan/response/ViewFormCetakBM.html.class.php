...
class ViewFormCetakBM extends HtmlResponse {
	var $Data;
	var $Pesan;
	...
	function TemplateModule() {
		...
		$this->SetTemplateFile('view_form_cetak_bm.html');
	}
	...
	function ProcessRequest() {
      ...
      $arr_pejabat_rektor = $Obj->GetJabatanNama('rektor');
		Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'pejabat_rektor', array('pejabat_rektor', $arr_pejabat_rektor, '', '-', ' id="pejabat_rektor" style="width:200px;" '), Messenger::CurrentRequest);

      $arr_pejabat_bendahara = $Obj->GetJabatanNama('bendahara');
		Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'pejabat_finansial', array('pejabat_finansial', $arr_pejabat_bendahara, '', '-', ' id="pejabat_finansial" style="width:200px;" '), Messenger::CurrentRequest);
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('history_transaksi_kode_jurnal_penerimaan', 'CetakBuktiTransaksi', 'view', 'html') . "&dataId=" . Dispatcher::Instance()->Encrypt($data['decDataId']) . '&tipe=bm');
		$this->mrTemplate->AddVar('content', 'URL_CETAK', Dispatcher::Instance()->GetUrl('history_transaksi_kode_jurnal_penerimaan', 'CetakBuktiTransaksi', 'view', 'html') . "&dataId=" . Dispatcher::Instance()->Encrypt($data['decDataId']) . '&tipe=bm');
		...
		$this->mrTemplate->AddVar('content', 'JUDUL', GTFWConfiguration::GetValue('organization', 'company_name'));

...
class RkaklKegiatan extends Database {
   ...
   protected $mSqlFile= 'module/pagu_anggaran_unit_per_mak/business/rkaklKegiatan.sql.php';
   ...
   function GetCountRkaklKegiatan($kode='', $nama=''){
      ...
      $sql		= sprintf($this->mSqlQueries['get_count_rkakl_kegiatan'],'%s',$and,'%s');
      $count	= $this->Open($sql,array('%'.$kode.'%','%'.$nama.'%'));
      //$count = $this->Open($this->mSqlQueries['get_count_rkakl_kegiatan'],
	  //		   array( '%'.$kode.'%', '%'.$nama.'%'));
   ...
   function GetRkaklKegiatan($kode='', $nama='', $start, $limit) {
      ...
      $sql		= sprintf($this->mSqlQueries['get_rkakl_kegiatan'],'%s',$and,'%s','%d','%d');

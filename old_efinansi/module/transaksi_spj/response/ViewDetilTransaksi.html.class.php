<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_spj/business/AppDetilTransaksi.class.php';
...
class ViewDetilTransaksi extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/transaksi_spj/template');
	...
	function ParseTemplate($data = NULL) {
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_spj', 'detilTransaksi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_KEMBALI', Dispatcher::Instance()->GetUrl('transaksi_spj', 'Transaksi', 'view', 'html'));
		if($this->Pesan) {
		...
		if (empty($data['dataDetilTransaksi'])) {
         ...
         $label = 'Transaksi';
         $urlDelete = 'transaksi_spj|deleteTransaksi|do|html';
         $urlReturn = 'transaksi_spj|detilTransaksi|view|html';
         $URLDELETE = Dispatcher::Instance()->GetUrl('transaksi_spj', 'deleteTransaksi', 'do', 'html');

//mulai bikin tombol delete
			#$label = "Transaksi";
			#$urlDelete = Dispatcher::Instance()->GetUrl('transaksi_spj', 'deleteTransaksi', 'do', 'html');
			#$urlReturn = Dispatcher::Instance()->GetUrl('transaksi_spj', 'detilTransaksi', 'view', 'html');
         ...
         $j=0; #print_r($dataDetilTransaksi);
            ...
            if($dataDetilTransaksi[$i]['tipe_id'] == 1) {
               $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('transaksi_spj', 'FormCetakBKM', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkm';
               $dataDetilTransaksi[$i]['label_cetak_bukti'] = 'Cetak BKM';
               #$url_cetak_bukti = Dispatcher::Instance()->GetUrl('transaksi_spj', 'CetakBuktiTransaksi', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkm';
               ...
               $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('transaksi_spj', 'FormCetakBKK', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkk';
               $dataDetilTransaksi[$i]['label_cetak_bukti'] = 'Cetak BKK';
               #$url_cetak_bukti = Dispatcher::Instance()->GetUrl('transaksi_spj', 'CetakBuktiTransaksi', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkk';
               ...
               $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('transaksi_spj', 'FormCetakBM', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bm';
            ...
            $dataDetilTransaksi[$i]['nominal_label'] = number_format($dataDetilTransaksi[$i]['nominal'], 2, ',', '.');
				...
				$dataDetilTransaksi[$i]['url_cetak'] = Dispatcher::Instance()->GetUrl('transaksi_spj', 'FormCetakTransaksi', 'view', 'html') . '&dataId=' . $idEnc;
            ...
            '&id='.Dispatcher::Instance()->Encrypt($dataDetilTransaksi[$i]['id']).
            '&label='.$label.'&dataName='.$dataDetilTransaksi[$i]['kkb']; 
               $url_edit = Dispatcher::Instance()->GetUrl('transaksi_spj', 'Transaksi', 'view', 'html') . '&dataId=' . $idEnc;

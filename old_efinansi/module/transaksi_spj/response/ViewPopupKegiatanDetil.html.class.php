<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_spj/business/AppPopupKegiatanDetil.class.php';
...
class ViewPopupKegiatanDetil extends HtmlResponse {
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_spj', 'popupKegiatanDetil', 'view', 'html') . '&unitkerja=' . $this->encUnitkerjaId);
		if($this->Pesan) {
		...
		if (empty($data['data_kegiatan_detil'])) {
			...
			for ($i=0; $i<sizeof($data_kegiatan_detil); $i++) {
				...
				$nominal_yang_bisa_dispj = $data_kegiatan_detil[$i]['nominal_yg_sudah_dicairkan'] - 
						$data_kegiatan_detil[$i]['nominal_yg_sudah_dispjkan'];
            $data_kegiatan_detil[$i]['nominal_yg_bisa_dispjkan'] =
							 number_format($nominal_yang_bisa_dispj,2,',','.');
					
            
            $data_kegiatan_detil[$i]['nominal_aprove_hidden'] = $data_kegiatan_detil[$i]['nominal_aprove'];   
            $data_kegiatan_detil[$i]['nominal_yg_bisa_dicairkan'] = number_format($data_kegiatan_detil[$i]['nominal_aprove'] - $data_kegiatan_detil[$i]['nominal_yg_sudah_dicairkan'], 2, ',', '.');
            ...
            $data_kegiatan_detil[$i]['nominal_yg_sudah_dispjkan'] = number_format($data_kegiatan_detil[$i]['nominal_yg_sudah_dispjkan'], 2, ',', '.');

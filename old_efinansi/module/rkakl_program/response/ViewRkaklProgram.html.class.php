...
class ViewRkaklProgram extends HtmlResponse {

   var $Pesan;

   function TemplateModule() {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot')
      .'module/rkakl_program/template');
      $this->SetTemplateFile('view_rkakl_program.html');
   }
   
   function ProcessRequest() {
      $Obj        = new RkaklProgram();
      $_POST      = $_POST->AsArray();
   ...
   //view
      $totalData  = $Obj->GetCountRkaklProgram($kode, $nama);
      $itemViewed = 20;
      $currPage   = 1;
      $startRec   = 0 ;
      if(isset($_GET['page'])) {
      ...
      $data       = $Obj->GetRkaklProgram($kode, $nama, $startRec, $itemViewed);
   ...
   function ParseTemplate($data = NULL) {
      ...
      if (empty($data['data'])) {
         ...
         for ($i=0; $i<sizeof($dataK); $i++) {
            ...
            $dataK[$i]['rencana_kegiatan_url']  = Dispatcher::Instance()->GetUrl(
               'rencana_kegiatan_rkakl',
               'RencanaKegiatan',
               'view',
               'html'
               .'&prog_id='.$idEnc.'&prog_label='.$dataK[$i]['nama']
            );
            
            // check jika program mempunyai kegiatan
            if($dataK[$i]['has_kegiatan']){
               $dataK[$i]['checkbox']     = 'disabled=true;';
            }else{
               $dataK[$i]['checkbox']     = '';
            }

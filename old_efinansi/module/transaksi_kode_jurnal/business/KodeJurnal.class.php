...
class KodeJurnal extends Database {
   ...
   protected $mSqlFile= 'module/transaksi_kode_jurnal/business/kode_jurnal.sql.php';
  ...
  function GetQueryKeren($sql,$params) {
   ...
   function GetData($offset, $limit, $nama='') {
      return $this->Open($this->mSqlQueries['get_data'], array('%'.$nama.'%', '%'.$nama.'%', $offset, $limit));
   }

   function GetCountData($nama) {
      $result = $this->Open($this->mSqlQueries['get_count_data'], array('%'.$nama.'%', '%'.$nama.'%'));
      if (!$result) {
         return 0;
      } else {
   ...
   function getDetailKodeJurnal ($kode_jurnal_from_function_get_data = array())
   {
      $return        = array();
      if(!empty($kode_jurnal_from_function_get_data) AND is_array($kode_jurnal_from_function_get_data)){
         foreach ($kode_jurnal_from_function_get_data as $jurnal) {
            $return[$jurnal['id']]['data']      = $jurnal;
            $return[$jurnal['id']]['jurnal']    = (array)$this->Open($this->mSqlQueries['get_detail_kode_jurnal'], array(
               $jurnal['id']
            ));
         }
      }

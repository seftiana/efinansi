...
class TahunPembukuan extends Database {
   ...
   function GetListCoaAsTahunPembukuan($idUnitKerja='1') {
      return $this->Open($this->mSqlQueries['get_list_coa_as_tahun_pembukuan'], array($idUnitKerja));
   }
   ...
   function GetPasiva() {
       return ($this->GetKewajiban() + $this->GetModal());
   }
   ...
   function GetBukuBesarAsTahunPembukuan($params) {

         $sql = $this->mSqlQueries['get_buku_besar_as_tahun_pembukuan'];
         if($this->subAccJml > 0){
            for($i=0;$i<=($this->subAccJml-1);$i++){
               $arrView[$i] = 'bbSubacc'.$this->subAccName[$i].'Kode';
            }
            $addSqlView = ',CONCAT('.implode(",'-',",$arrView).') AS subacc';

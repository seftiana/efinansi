...
$sql['check_balance']="
SELECT
   IF((SUM(tpDebet)=SUM(tpKredit)), 1, 0) AS status_balance
FROM
   tahun_pembukuan
LEFT JOIN coa ON (coaId=tpCoaId)
...
$sql['get_jumlah_transaksi_not_jurnal'] = "
   SELECT count(transId) AS jumlah
   FROM transaksi
   WHERE transIsJurnal = 'T'
";

$sql['get_jumlah_jurnal_not_posting'] = "
   SELECT count(prId) As jumlah
...
$sql['get_buku_besar_as_tahun_pembukuan'] = "
   SELECT
      ...
      coaIsLocked,
      coaIsDebetPositif,
      sum(bbSaldoAwal) as saldo_awal,
      sum(bbDebet) as debet,
      sum(bbKredit) as kredit,
      sum(bbSaldo) as saldo,
      sum(bbSaldo) as perubahan,
      sum(bbSaldoAkhir) as saldo_akhir,
      bbSaldoAkhir
      [SUBACC_VIEW]
   FROM buku_besar
   INNER JOIN coa ON(coaId=bbCoaId)
   WHERE coaId = '%s'
   GROUP BY bbCoaId [SUBACC_GROUP]
   ORDER BY bbId ASC
";

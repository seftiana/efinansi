<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/adjustment_pengeluaran/business/AppPopupSubProgram.class.php';

class PopupSubProgram extends HtmlResponse {

	var $Pesan;

	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/adjustment_pengeluaran/template');
		$this->SetTemplateFile('popup_sub_program.html');
	}
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('adjustment_pengeluaran', 'subProgram', 'popup', 'html') . '&programId=' . $this->encProgramId);
		if($this->Pesan) {
		...
		if (empty($data['dataSubProgram'])) {
			...
			for ($i=0; $i<sizeof($dataSubProgram); $i++) {
				...
				$dataSubProgram[$i]['link'] = str_replace("'","\'",$dataSubProgram[$i]['nama']);

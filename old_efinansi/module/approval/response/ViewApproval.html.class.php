...
'module/approval/business/AppApproval.class.php';
require_once GTFWConfiguration::GetValue( 'application', 'docroot') .
'module/user_unit_kerja/business/UserUnitKerja.class.php';

class ViewApproval extends HtmlResponse
{
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
         'module/approval/template');
      $this->SetTemplateFile('view_approval.html');
   }
   ...
   function ProcessRequest()
   {
      ...
      $dataList         = $mObj->GetDataApproval($offset, $limit, (array)$requestData);
      $total_data       = $mObj->GetCountDataApproval((array)$requestData);
      ...
      if (empty($dataList)) {
         ...
         for ($i=0; $i < count($dataList);) {
            if((int)$dataList[$i]['program_id'] === (int)$programId  && (int)$dataList[$i]['kegiatan_id'] === (int)$kegiatanId){
               ...
               $dataGrid[$index]['component_class']      = (is_null($dataList[$i]['pengadaan']) OR (int)$dataList[$i]['pengadaan'] === 0) ? 'no_component' : '';
               if(is_null($dataList[$i]['pengadaan']) OR (int)$dataList[$i]['pengadaan'] === 0){
                  $dataGrid[$index]['status']   = 'NO_COMPONENT';
               }else{
         ...
         foreach ($dataGrid as $list) {
            ...
            $this->mrTemplate->AddVar('LEVEL','TYPE', strtoupper($list['level']));
            $this->mrTemplate->AddVar('LEVEL', 'deskripsi', $list['deskripsi']);
            ...
            switch (strtoupper($list['status'])) {
               case 'YA':
                  $this->mrTemplate->AddVar('links', 'URL_APPROVAL', '');
                  break;
              case 'PARSIAL':
                  $url        = $urlApproval.'&dataId='.Dispatcher::Instance()->Encrypt($list['id']).'&jenis_kegiatan='.Dispatcher::Instance()->Encrypt($list['jenis_kegiatan_nama']).'&search=1&'.$queryString;
                  $this->mrTemplate->AddVar('links', 'URL_APPROVAL', $url);
                  break;   
               case 'BELUM':
                  $url        = $urlApproval.'&dataId='.Dispatcher::Instance()->Encrypt($list['id']).'&jenis_kegiatan='.Dispatcher::Instance()->Encrypt($list['jenis_kegiatan_nama']).'&search=1&'.$queryString;
                  $this->mrTemplate->AddVar('links', 'URL_APPROVAL', $url);

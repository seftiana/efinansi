<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_kode_jurnal_penerimaan/business/AppDetilTransaksi.class.php';
...
class ViewDetilTransaksi extends HtmlResponse {
	...
	function ParseTemplate($data = NULL) {
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'detilTransaksi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_KEMBALI', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'Transaksi', 'view', 'html'));
		if($this->Pesan) {
		...
		if (empty($data['dataDetilTransaksi'])) {
         ...
         $label = 'Transaksi';
         $urlDelete = 'transaksi_kode_jurnal_penerimaan|deleteTransaksi|do|html';
         $urlReturn = 'transaksi_kode_jurnal_penerimaan|detilTransaksi|view|html';
         $URLDELETE = Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'deleteTransaksi', 'do', 'html');
...
//mulai bikin tombol delete
         ...
         $j=0; #print_r($dataDetilTransaksi);
            ...
            if($dataDetilTransaksi[$i]['tipe_id'] == 1) {
               $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'FormCetakBKM', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkm';
               ...
               $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'FormCetakBKK', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkk';
               ...
               $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'FormCetakBM', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bm';
            ...
            $dataDetilTransaksi[$i]['nominal_label'] = number_format($dataDetilTransaksi[$i]['nominal'], 2, ',', '.');
				...
				$dataDetilTransaksi[$i]['url_cetak'] = Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'FormCetakTransaksi', 'view', 'html') . '&dataId=' . $idEnc;
            ...
            '&id='.Dispatcher::Instance()->Encrypt($dataDetilTransaksi[$i]['id']).
            '&label='.$label.'&dataName='.$dataDetilTransaksi[$i]['kkb']; 
               $url_edit = Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'Transaksi', 'view', 'html') . '&dataId=' . $idEnc;

...
class ProcessTransaksi 
{
	...
	function Check() {
		if (isset($_POST['btnsimpan'])) { 
         if (!isset($this->_POST['data'])) return "COA";
         if($this->decId != '') {
	...
	function Add() {
		...
		if($cek === true) {
         if(empty($this->_POST['no_invoice_list'])) {
            ...
            $arrData['arrCoa'] =$this->_POST['data'];
            $arrData['refTransaksiId'] = $this->_POST['transaksi'];
            //echo "<pre>";
            //print_r($this->_POST);
            //print_r($_FILES);
            //echo "</pre>";
            //$id_transaksi = $this->Obj->DoAddTransaksi($arrData);
            $result_transaksi = $this->Obj->DoAddTransaksi($arrData);
            //echo $id_transaksi;
            if($result_transaksi) {
               //insert ke table file, tidak wajib
               $id_transaksi = $this->Obj->GetLastTransId();
               //if($id_transaksi != ''){
               ...
               //}
               /**
                * kode di non aktifkan 
                * cos : proses penjurnalan dijadikan satu dengan proses transaksi
                * 
               $arrJurnal['id_transaksi']= $id_transaksi;
               $arrJurnal['user_transaksi'] = $arrData['transUserId'];
               $arrCoa = $this->_POST['data'];
               $jurnal =  $this->Obj->TransaksiJurnal(
			    							$arrCoa, 
			                              $id_transaksi, 
			   							$arrData['transUserId'],
                  ...
                  Messenger::Instance()->Send(
									   ...
									   array(
												...
												$this->cssDone),
										Messenger::NextRequest);
					return $this->pageInputDetil. '&dataId=' .$this->Obj->GetLastTransId();
               }else {
                  Messenger::Instance()->Send(
	  								     'transaksi_spj_pertransaksi', 
										 'Transaksi', 

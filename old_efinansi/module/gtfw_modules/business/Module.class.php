...
class Module extends Database
{
   ...
   {
      $this->mSqlFile   = 'module/gtfw_modules/business/module.sql.php';
   ...
   public function DoCleanConflictedModule($appId = NULL, $module = NULL)
   {
      ...
      foreach ($dataModule as $mod) {
         ...
         $result        &= $this->Execute($this->mSqlQueries['clean_dummy_module'], array());
         $result        &= $this->Execute($this->mSqlQueries['insert_dummy_module'], array());
         $result        &= $this->Execute($this->mSqlQueries['clean_gtfw_group_module'], array());
         $result        &= $this->Execute($this->mSqlQueries['insert_gtfw_group_module'], array());
      }
   ...
   {
      $patern        = '/([A-Za-z0-9-\(\)]+).([A-Za-z0-9-\(\)]+).([A-Za-z0-9-\(\)]+).([A-Za-z0-9-\(\)]+)/';
      ...
      if(is_dir($modulePath) && file_exists($modulePath)){
         if($hd = opendir($modulePath)){
            while ($sub = readdir($hd)) {
               $check   = (bool)preg_match($patern, $sub);
               ...
               if(preg_match($patern, $sub, $matches)){
                  $name    = $matches[1];
                  $type    = $matches[2];
                  $class   = $matches[3];
                  $ext     = $matches[4];

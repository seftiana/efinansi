...
/**
 * Class ProcessSubAccountKeempat
 * @package sub_account_keempat
...
class ProcessSubAccountKeempat {
   ...
   {
      $this->mSubAccoutKeempat = new SubAccountKeempat();
      $queryString             = $this->mSubAccoutKeempat->_getQueryString();
   ...
   {
      $requestData      = array();
      if($this->mSubAccoutKeempat->method == 'post'){
         $kode       = preg_replace('/\s[\s]+/', '', $this->mSubAccoutKeempat->_POST['kode']);
         $kode       = preg_replace('/[\s\W]+/', '', $kode);
         $kode       = preg_replace('/^[\_]+/', '', $kode);
         $kode       = preg_replace('/[\_]+$/', '', $kode);
         $requestData['id']      = $this->mSubAccoutKeempat->_POST['dataId'];
         $requestData['kode']    = $kode;
         $requestData['nama']    = $this->mSubAccoutKeempat->_POST['nama'];
      }
   ...
   private function checkData()
   {
      ...
      $unique           = $this->mSubAccoutKeempat->doCheckUniqueData(
         $requestData['kode'],
         $requestData['id']
      );
      if($unique === false){
   ...
   public function Update()
   {
      ...
      if($checkData['result'] === false){
         ...
         $process             = $this->mSubAccoutKeempat->DoUpdate(
            $this->mData['kode'],
            $this->mData['nama'],
            $this->mData['id']
         );
   ...
   public function Delete()
   {
      ...
      if($this->mSubAccoutKeempat->method != 'post'){
         ...
         $arrId         = $this->mSubAccoutKeempat->_POST['idDelete'];
         $success       = 0;
         $failed        = 0;
         foreach ($arrId as $id) {
            $delete     = $this->mSubAccoutKeempat->DoDelete($id);
            if($delete === true){
               $success+=1;
            }else{
               $failed+=1;

...
require_once GTFWConfiguration::GetValue('application', 'docroot') . 'module/transaksi_realisasi_penerimaan/business/AppPopupKegiatanDetil.class.php';
...
class ViewPopupKegiatanDetil extends HtmlResponse {
    ...
    function ParseTemplate($data = NULL) {
        ...
        $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_realisasi_penerimaan', 'popupKegiatanDetil', 'view', 'html') . '&unitkerja=' . $this->encUnitkerjaId);
        if ($this->Pesan) {
        ...
        if (empty($data['data_kegiatan_detil'])) {
            ...
            for ($i = 0; $i < sizeof($data_kegiatan_detil); $i++) {
                ...
                if ($no % 2 != 0)
                    $data_kegiatan_detil[$i]['class_name'] = 'table-common-even';
                else
                    $data_kegiatan_detil[$i]['class_name'] = '';
                ...
                if ($data_kegiatan_detil[$i]['nominal_aprove_hidden'] < 0)
                    $data_kegiatan_detil[$i]['nominal_aprove_hidden'] = 0;

                $data_kegiatan_detil[$i]['nominal_aprove'] = number_format($data_kegiatan_detil[$i]['nominal_aprove'], 2, ',', '.');
                $data_kegiatan_detil[$i]['nominal_yg_bisa_dicairkan'] = number_format($data_kegiatan_detil[$i]['sisa_pencairan'], 2, ',', '.');
                ...
                if ( $data_kegiatan_detil[$i]['sisa'] > 0) {
                ...
                $this->mrTemplate->AddVars('pilih', $data_kegiatan_detil[$i], 'KEGIATAN_DETIL_');

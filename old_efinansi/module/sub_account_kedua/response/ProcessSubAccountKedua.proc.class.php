...
class ProcessSubAccountKedua {
   ...
   {
      $this->mSubAccoutKedua  = new SubAccountKedua();
      $queryString             = $this->mSubAccoutKedua->_getQueryString();
   ...
   {
      $requestData      = array();
      if($this->mSubAccoutKedua->method == 'post'){
         $kode       = preg_replace('/\s[\s]+/', '', $this->mSubAccoutKedua->_POST['kode']);
         $kode       = preg_replace('/[\s\W]+/', '', $kode);
         $kode       = preg_replace('/^[\_]+/', '', $kode);
         $kode       = preg_replace('/[\_]+$/', '', $kode);
         $requestData['id']      = $this->mSubAccoutKedua->_POST['dataId'];
         $requestData['kode']    = $kode;
         $requestData['nama']    = $this->mSubAccoutKedua->_POST['nama'];
      }
   ...
   private function checkData()
   {
      ...
      $unique           = $this->mSubAccoutKedua->doCheckUniqueData(
         $requestData['kode'],
         $requestData['id']
      );
      if($unique === false){
   ...
   public function Update()
   {
      ...
      if($checkData['result'] === false){
         ...
         $process             = $this->mSubAccoutKedua->DoUpdate(
            $this->mData['kode'],
            $this->mData['nama'],
            $this->mData['id']
         );
   ...
   public function Delete()
   {
      ...
      if($this->mSubAccoutKedua->method != 'post'){
         ...
         $arrId         = $this->mSubAccoutKedua->_POST['idDelete'];
         $success       = 0;
         $failed        = 0;
         foreach ($arrId as $id) {
            $delete     = $this->mSubAccoutKedua->DoDelete($id);
            if($delete === true){
               $success+=1;
            }else{
               $failed+=1;

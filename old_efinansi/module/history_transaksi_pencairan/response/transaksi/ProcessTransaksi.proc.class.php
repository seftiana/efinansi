	...
	function Update() {
      ...
      $cek = true;
            ...
            if($upd_transaksi == true) {
               ...
               */
               if(!$this->_POST['mak_lama'] && $this->_POST['mak']) {
                  ...
                  $this->Obj->DoDeleteTransaksiDetilAnggaran($this->_POST['mak_lama']);
			   			...
			   			$this->_POST['mak_lama'] != $this->_POST['mak']) {
                  //kasus 4, update
               ...
               */
               ...
               Messenger::Instance()->Send(
			   								'history_transaksi_pencairan',
   											'HTFormTransaksiDetil', 'view', 'html',
											array(
			   								$this->_POST, '
        ...
        // }
         ...
         //echo "666";
         //echo "exist";
         Messenger::Instance()->Send('history_transaksi_pencairan', 'HTFormTransaksi', 'view', 'html', array($this->_POST,'Transaksi Dengan Nomor <b>'.($this->_POST['no_kkb']).'</b> Sudah Dibuat', $this->cssFail),Messenger::NextRequest);
		   return $this->pageView . '&dataId=' . $this->encId;
      } else {
         //gagal masukin data
         //echo "gagal";
         Messenger::Instance()->Send('history_transaksi_pencairan', 'HTFormTransaksi', 'view', 'html', array($this->_POST,'Gagal Merubah Data Transaksi', $this->cssFail),Messenger::NextRequest);
		   return $this->pageView . '&dataId=' . $this->encId;
      }
      //echo "adflj";
	...
	function Delete() {
      ...
      $deleteArrData = $this->Obj->DoDeleteDataById($arrId);
		if($deleteArrData === true) {
			Messenger::Instance()->Send('history_transaksi_pencairan', 'HTTransaksi', 'view', 'html', array($this->_POST,'Penghapusan Data Berhasil Dilakukan', $this->cssDone),Messenger::NextRequest);
		} else {
			...
			Messenger::Instance()->Send('history_transaksi_pencairan', 'HTTransaksi','view', 'html', array($this->_POST, $gagal . ' Data Tidak Dapat Dihapus.', $this->cssFail),Messenger::NextRequest);
		}

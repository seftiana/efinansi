...
class ViewHTFormRealisasiPencairan extends HtmlResponse {
	...
	function TemplateModule() {
		...
		$this->SetTemplateFile('transaksi_realisasi/view_transaksi.html');
	}
	...
	function ParseTemplate($data = NULL) {
      	$url_view = Dispatcher::Instance()->GetUrl('history_transaksi_pencairan', 'HTFormRealisasiPencairan',
	  											 'view', 'html');
      	$this->mrTemplate->AddVar('content', 'URL_CHECK', $url_view);
		$this->mrTemplate->AddVar('content', 'URL_BATAL', Dispatcher::Instance()->GetUrl(
							'history_transaksi_pencairan', 'HTRealisasiPencairan', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_POPUP_SKENARIO', Dispatcher::Instance()->GetUrl(
							'history_transaksi_pencairan', 'popupSkenario', 'view', 'html'));
	   $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl(
							'history_transaksi_pencairan', 'popupKegiatanDetilRealisasi', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK_PENERIMAAN', Dispatcher::Instance()->GetUrl(
	  						'history_transaksi_pencairan', 'PopupMakPenerimaan', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_KEGIATAN_UNIT_KERJA',
  		Dispatcher::Instance()->GetUrl('history_transaksi_pencairan', 'popupKegiatanDetilRealisasi', 'view', 'html'));
      ...
      $this->mrTemplate->AddVar('content', 'URL_POPUP_SISA_ANGGARAN',
  		Dispatcher::Instance()->GetUrl('history_transaksi_pencairan', 'popupSisaAnggaran', 'view', 'html'));
      $datadb = $data['datadb'];
		/**
...
//print_r($this->Data);
      ...
      if(isset($_GET['dataId']) AND $_GET['dataId']->Integer()->Raw() > 0) {
         ...
         $this->mrTemplate->AddVar('content', 'URL_ACTION',
		 					Dispatcher::Instance()->GetUrl('history_transaksi_pencairan',
							 				'updateHTRealisasiPencairan',
											 'do',
											 'html') .
											 '&dataId=' .
											 Dispatcher::Instance()->Encrypt($_GET['dataId']));

...
class AppTransaksi extends Database {
   ...
   public $indonesianMonth    = array(
	...
	function __construct($connectionNumber=0) {
      $this->mSqlFile   = 'module/history_transaksi_pencairan/business/transaksi_realisasi/apptransaksi.sql.php';
   ...
   function DoAddTransaksi($arrData) {
      ...
      if($result === true) return $insertId;
	  else return false;
   }
...
//MULAI EDIT DATA
   ...
   function DoUpdateTransaksiDetilAnggaran($transId, $mak) {
      ...
      $pengrealId = $arrMak[1];
      $this->StartTrans();
	  $result = $this->Execute($this->mSqlQueries['do_update_transaksi_detil_anggaran'],
            										array($kegdetId, $pengrealId,$transId));
	  if($result){
      $result =  $this->Execute($this->mSqlQueries['do_update_transaksi_detil_pencairan'],
            										array($kegdetId, $pengrealId,$transId));

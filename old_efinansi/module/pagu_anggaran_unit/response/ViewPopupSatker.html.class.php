<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/pagu_anggaran_unit/business/PopupSatker.class.php';

class ViewPopupSatker extends HtmlResponse {

	var $Pesan;

	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/pagu_anggaran_unit/template');
		$this->SetTemplateFile('view_popup_satker.html');
	}
	...
	function ProcessRequest() {
		$popupSatkerObj = new PopupSatker(); 
		$POST = $_POST->AsArray();
		if(!empty($POST)) {
			$satker = $POST['satker'];
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH_POPUP', Dispatcher::Instance()->GetUrl('pagu_anggaran_unit', 'popupSatker', 'view', 'html'));
		if($this->Pesan) {
		...
		if (empty($data['dataSatker'])) {
			...
			for ($i=0; $i<sizeof($dataSatker); $i++) {
				//$dataSatker[$i]['satker_nama']	= "Sat'ket nama";
				$dataSatker[$i]['satker_link']	= str_replace("'","\'",$dataSatker[$i]['satker_nama']);
				$no = $i+$data['start'];

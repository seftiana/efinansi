...
class ViewHTFormRealisasiPencairan extends HtmlResponse {
    ...
    function ProcessRequest() {
        ...
        if ($_POST || isset($_GET['cari'])) {
            if (isset($_POST['periode_mon'])) {
                ...
                $penerima = Dispatcher::Instance()->Decrypt($_GET['penerima']);
        ...
        if (isset($_GET['dataId'])) {
            ...
            $dataKomponen = $Obj->getKomponenAnggaranByTransId($datadb['id']);

            //get data filie
            $return['datadbFile'] = $Obj->GetTransaksiFile($datadb['id']);

            $return['datadbKomp'] = $Obj->getKompTransaksiFile($datadb['id']);
        ...
        Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'jenis_transaksi', array('jenis_transaksi', $arr_jenis_transaksi, 5,
            "false", ' style="width:200px;" id="jenis_transaksi"'), Messenger::CurrentRequest);
        /**
          Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'jenis_transaksi',
          array('jenis_transaksi', $arr_jenis_transaksi, 5, "false",
          ' style="width:200px;" id="jenis_transaksi" onchange="showHideMAK(this)"'),
          Messenger::CurrentRequest);
         */
        #onmousemove="showHideMAK(this)"   

        foreach ($arr_jenis_transaksi as $value)
            $jns_trans[$value['id']] = $value['name'];
        $return['jns_transaksi'] = json_encode($jns_trans);

        //COMBO TIPE TRANSAKSI
        $arr_tipe_transaksi = $Obj->GetComboTipeTransaksi();
        Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'tipe_transaksi', array('tipe_transaksi', $arr_tipe_transaksi, 4, "false",
            ' style="width:200px;" id="tipe_transaksi"'), Messenger::CurrentRequest);
        /**
          Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'tipe_transaksi',
          array('tipe_transaksi', $arr_tipe_transaksi, 4, "false",
          ' style="width:200px;" onChange="jsCheck(this.value)" id="tipe_transaksi"'),
          Messenger::CurrentRequest);
         */
    ...
    function ParseTemplate($data = NULL) {
        $url_view = Dispatcher::Instance()->GetUrl('history_transaksi_realisasi', 'HTFormRealisasiPencairan', 'view', 'html');
        ...
        $this->mrTemplate->AddVar('content', 'URL_POPUP_SKENARIO', Dispatcher::Instance()->GetUrl(
                        'history_transaksi_realisasi', 'popupSkenario', 'view', 'html'));
        $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl(
                        'history_transaksi_realisasi', 'popupRealisasi', 'view', 'html'));
        $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK_PENERIMAAN', Dispatcher::Instance()->GetUrl(
                        'history_transaksi_realisasi', 'PopupMakPenerimaan', 'view', 'html'));
        $this->mrTemplate->AddVar('content', 'URL_POPUP_KEGIATAN_UNIT_KERJA', Dispatcher::Instance()->GetUrl('history_transaksi_realisasi', 'popupKegiatanDetilRealisasi', 'view', 'html'));
        #$this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'popupKegiatanDetil', 'view', 'html'));
        $this->mrTemplate->AddVar('content', 'URL_POPUP_SISA_ANGGARAN', Dispatcher::Instance()->GetUrl('history_transaksi_realisasi', 'popupSisaAnggaran', 'view', 'html'));
        $datadb = $data['datadb'];
        /**
         * untuk keperluan update data
         * perubahan unitkerja id mempengaruhi no bukti transaksi
         */
        $this->mrTemplate->AddVar('is_tambah', 'NO_KKB', $datadb['no_kkb']);
        $this->mrTemplate->AddVar('is_tambah', 'TANGGAL_TRANS_LAMA', $datadb['tanggal']);
        $this->mrTemplate->AddVar('is_tambah', 'UNITKERJA_LAMA', $datadb['unitkerja']);
        /**
         * end
         */
        $this->mrTemplate->AddVar('content', 'NOMINAL', $datadb['nominal']);
        $this->mrTemplate->AddVar('content', "NOMINAL_DISETUJUI", $datadb['nominal_disetujui']);
        ...
        $this->mrTemplate->AddVar('content', 'PENERIMA', $datadb['penerima']);
...
//print_r($this->Data);
        ...
        if ($data['role_name'] == "Administrator") {
            ...
            $this->mrTemplate->AddVar('role', 'URL_POPUP_UNITKERJA', Dispatcher::Instance()->GetUrl(
                            'history_transaksi_realisasi', 'popupUnitKerja', 'view', 'html'));
        ...
        if (isset($_GET['dataId']) AND $_GET['dataId']->Integer()->Raw() > 0) {
            ...
            $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('history_transaksi_realisasi', 'updateHTRealisasiPencairan', 'do', 'html') .
                    '&dataId=' .
        ...
        $datadbKomp = $data['datadbKomp'];
        if (empty($datadbKomp)) {
            $this->mrTemplate->AddVar('tpl_detail_list', 'INVOICE_LIST_EMPTY', 'YES');
        } else {
            $this->mrTemplate->AddVar('tpl_detail_list', 'INVOICE_LIST_EMPTY', 'NO');
            $no_invoice_list = array();
            $no_invoice_list = $data['no_detail_list'];
            $number = 1;
            //print_r($datadbInvoice);
            for ($i = 0; $i < sizeof($datadbKomp); $i++) {
                $datadbI[$i]['number'] = ($i + 1);
                $datadbI[$i]['id_delete'] = $datadbKomp[$i]['id'];
                $this->mrTemplate->AddVars('tpl_invoice_item', $datadbKomp[$i], 'DATA_');
                $this->mrTemplate->parseTemplate('tpl_invoice_item', 'a');
            }
        }

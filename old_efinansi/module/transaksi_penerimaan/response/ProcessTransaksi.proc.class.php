...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
		'module/transaksi_penerimaan/business/AppTransaksi.class.php';
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
...
class ProcessTransaksi {
   ...
   var $cssAlert = "notebox-alert";
	...
	function __construct() {
		...
		$this->pageView = Dispatcher::Instance()->GetUrl('transaksi_penerimaan', 'Transaksi', 'view', 'html');
		$this->pageDetil = Dispatcher::Instance()->GetUrl(
									'history_transaksi', 
									'HTPenerimaan', 'view', 'html');
		$this->pageInputDetil = Dispatcher::Instance()->GetUrl('transaksi_penerimaan',
									 'transaksiDetil', 'view', 'html');
        ...
        // }else{   
            ...
            if($id_transaksi != "") {
               ...
               if($this->_POST['peng_real_id'] != '')
                  $update_status_transaksi = $this->Obj->DoUpdateStatusTransaksiDiPengajuanRealisasi($this->_POST['peng_real_id']);
   
               //if($transaksi_detil_gaji === true) {            
               Messenger::Instance()->Send('transaksi_penerimaan', 'transaksiDetil', 'view', 'html', array($this->_POST, 'Penambahan Data Transaksi Berhasil Dilakukan '.$this->noBuktiTransaksi, $this->cssDone),Messenger::NextRequest);
               //}
         ...
         //echo "666";
         Messenger::Instance()->Send('transaksi_penerimaan', 'Transaksi', 'view', 'html', array($this->_POST,'Transaksi Dengan Nomor <b>'.($this->_POST['no_kkb']).'</b> Sudah Dibuat', $this->cssFail),Messenger::NextRequest);
      } else {
         //gagal masukin data
         Messenger::Instance()->Send('transaksi_penerimaan', 'Transaksi', 'view', 'html', array($this->_POST,'Gagal Menambah Data', $this->cssFail),Messenger::NextRequest);
      }
         //exit();
	...
	function Update() {
      ...
      $cek = true;
         ...
         //}else{
            ...
            if($upd_transaksi == true) {
               ...
               */
               ...
               Messenger::Instance()->Send('transaksi_penerimaan', 'transaksiDetil', 'view', 'html', array($this->_POST, 'Perubahan Data Transaksi Berhasil Dilakukan', $this->cssDone),Messenger::NextRequest);
        ...
        // }
         ...
         //gagal masukin data
         //echo "gagal";
         Messenger::Instance()->Send('transaksi_penerimaan', 'Transaksi', 'view', 'html', array($this->_POST,'Gagal Merubah Data Transaksi', $this->cssFail),Messenger::NextRequest);
		   return $this->pageView . '&dataId=' . $this->encId;
      }
      //echo "adflj";
		return $this->pageView;
	...
	function Delete() {
      ...
      $deleteArrData = $this->Obj->DoDeleteDataById($arrId);  
		if($deleteArrData === true) {
			Messenger::Instance()->Send('history_transaksi', 'HTPenerimaan', 'view', 'html', array($this->_POST,'Penghapusan Data Berhasil Dilakukan', $this->cssDone),Messenger::NextRequest);
		} else {
			...
			Messenger::Instance()->Send('history_transaksi', 'HTPenerimaan', 'view', 'html', array($this->_POST, $gagal . ' Data Tidak Dapat Dihapus.', $this->cssFail),Messenger::NextRequest);
		}

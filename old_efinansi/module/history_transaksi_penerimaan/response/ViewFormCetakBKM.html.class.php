...
class ViewFormCetakBKM extends HtmlResponse {
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_ACTION', 
				Dispatcher::Instance()->GetUrl('history_transaksi_penerimaan', 'CetakBuktiTransaksi', 'view', 'html') . "&dataId=" . Dispatcher::Instance()->Encrypt($data['decDataId']) . '&tipe=bkm');
		$this->mrTemplate->AddVar('content', 'URL_CETAK', 
				Dispatcher::Instance()->GetUrl('history_transaksi_penerimaan', 'CetakBuktiTransaksi', 'view', 'html') . "&dataId=" . Dispatcher::Instance()->Encrypt($data['decDataId']) . '&tipe=bkm');
		
		
		$this->mrTemplate->AddVar('content', 'URL_BATAL', 
				Dispatcher::Instance()->GetUrl('history_transaksi_penerimaan', 'HTPenerimaan', 'view', 'html'));

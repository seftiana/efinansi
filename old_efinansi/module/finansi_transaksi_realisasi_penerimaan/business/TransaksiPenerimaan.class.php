...
/**
* ================= doc ====================
* FILENAME     : TransaksiPenerimaan.class.php
* @package     : TransaksiPenerimaan
* scope        : PUBLIC
* @Author      : Eko Susilo
* @Created     : 2015-03-04
* @Modified    : 2015-03-04
...
class TransaksiPenerimaan extends Database
{
   # internal variables
   private $mNumber;
   protected $mSqlFile;
   ...
   # Constructor
   function __construct ($connecttionNumber = 0)
   {
      $this->mSqlFile   = 'module/finansi_transaksi_realisasi_penerimaan/business/transaksi_penerimaan.sql.php';
      $this->_POST      = is_object($_POST) ? $_POST->AsArray() : $_POST;
      $this->_GET       = is_object($_GET) ? $_GET->AsArray() : $_GET;
      $this->method     = strtolower($_SERVER['REQUEST_METHOD']);
      $this->mNumber    = new GenerateNumber($connecttionNumber);
      parent::__construct($connecttionNumber);
   }
   ...
   public function GetComboTipeTransaksi($type = array()) {
      $kode       = implode("','", (array)$type);
   ...
   {
      $return     = $this->Open($this->mSqlQueries['get_data_transaksi_detail'], array(
         $id
      ));

      return self::ChangeKeyName($return[0], 'lower');
   }

   public function getSkenarioJurnal($nominal, $skenario = array())
   {
      $return     = $this->Open($this->mSqlQueries['get_skenario_jurnal'], array(
         $nominal,
         implode("','", $skenario)
      ));
   ...
   public function getFilesTransaksi($id)
   {
      ...
      if(!empty($return)){
         ...
         foreach ($return as $rt) {
            ...
            $files[$index]['name']        = preg_replace('/^p(\d+)_+/', '', $rt['fileName']);
            $files[$index]['size']        = 0;
   ...
   public function SaveTransaksi($data = array())
   {
      ...
      // insert transaksi detail realisasi penerimaan
      $result        &= $this->Execute($this->mSqlQueries['do_insert_transaksi_detail_anggaran_penerimaan'], array(
         $transaksiId,
         $data['map_id']
      ));
      ...
      // generate data realisasi penerimaan
      $realisasi     = array();
      foreach ($this->indonesianMonth as $mon) {
         if($mon['id'] == 0){
            continue;
         }
         if((int)$mon['id'] === (int)$blnRealisasai){
            $realisasi[$mon['id']]['nominal']   = $data['nominal'];
         }else{
            $realisasi[$mon['id']]['nominal']   = 0;
         }
      }

      // insert realisasi
      $result     &= $this->Execute($this->mSqlQueries['do_save_realisasi_penerimaan'], array(
         $data['nominal'],
         $data['catatan'],
         $data['map_id'],
         $realisasi[1]['nominal'],
         ...
         $realisasi[8]['nominal'],
         $realisasi[9]['nominal'],
         $realisasi[10]['nominal'],
         $realisasi[11]['nominal'],
         $realisasi[12]['nominal'],
         $transaksiId
      ));
      ...
      if(strtoupper($data['skenario']) == 'AUTO'){
         if($data['skenario_jurnal'] && !empty($data['skenario_jurnal'])){
            ...
            /*$result        &= $this->Execute($this->mSqlQueries['do_insert_pembukuan_detail_debet'], array(
               $pembukuanId,
               $data['nominal'],
               $subacc_1,
               $subacc_2,
            ...
            $result        &= $this->Execute($this->mSqlQueries['do_insert_pembukuan_detail_kredit'], array(
               $pembukuanId,
               $data['nominal'],
               $subacc_1,
               $subacc_2,

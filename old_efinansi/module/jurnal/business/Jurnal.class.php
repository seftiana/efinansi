...
class Jurnal extends Database
{
   ...
   public function GetDataCetak($noReferensi,$tahun, $bulan){
       //$this->SetDebugOn();
      $tglAwal = $tahun.'-'.$bulan.'-01';
	   $tglAkhir = $tahun.'-'.$bulan.'-31';
      $result = $this->Open($this->mSqlQueries['get_data_cetak'], array('%'.$noReferensi.'%',
			$tglAwal,
			$tglAkhir));
      return $result;
   }

	function GetData($no_referensi, $bulan, $tahun, $start, $count)
	{
	   $tglAwal = $tahun.'-'.$bulan.'-01';
	   $tglAkhir = $tahun.'-'.$bulan.'-31';
	...
	function GetCount($no_referensi,$bulan,$tahun)
	{
	   $tglAwal = $tahun.'-'.$bulan.'-01';
	   $tglAkhir = $tahun.'-'.$bulan.'-31';

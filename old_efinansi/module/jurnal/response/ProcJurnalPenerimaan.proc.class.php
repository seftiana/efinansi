<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/jurnal_penerimaan/business/JurnalPenerimaan.class.php';

class ProcJurnalPenerimaan {
   
   protected $msg;
   public $data;  
   
   public $moduleName = 'jurnal_penerimaan';
   public $moduleHome = 'jurnalPenerimaan';
   public $moduleInput = 'inputJurnalPenerimaan';
   public $moduleAdd = 'addJurnalPenerimaan';
   public $moduleUpdate = 'updateJurnalPenerimaan';
   public $moduleDelete = 'deleteJurnalPenerimaan';
   
   
   public $db;  //yang berhubungan ke database
   
    function ProcJurnalPenerimaan(){ //constructor	  	  
	  $this->db = new JurnalPenerimaan;	  	  
	...
	function validation($action) {
       ...
       if (empty($this->data['debet']['coa_id'])) $this->msg .= "Rekening debet belum dipilih.<br/>\r\n";
        ...
        if(trim($this->data['referensi_id'])=='')
		   ...
		   if(!$errorkredit) 
		      if($totalnilai != $this->data['debet']['nilai'] )
			     $this->msg .= 'Debet tidak sama dengan kredit <br />';
		//}      

...
class ViewExcelUnitKerja extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      if(empty($dataList)){
         ...
         $sheet->setCellValue('O'.$row, GTFWConfiguration::GetValue('language', 'sisa_saldo_rp'));
         $sheet->getStyle('A'.($row).':O'.($row+1))->applyFromArray($styledTableHeaderArray);
         ...
         foreach ($dataGrid as $list) {
            switch (strtoupper($list['tipe'])) {
               case 'PROGRAM':
                  ...
                  $sheet->getStyle('B'.$row.':O'.$row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('00CCCCCC');
                  $sheet->getStyle('A'.$row.':O'.$row)->getFont()->setBold(true);
                  $sheet->mergeCells('D'.$row.':H'.$row);
                  $sheet->getStyle('C'.$row.':O'.$row)->applyFromArray($borderTableStyledArray);
               ...
               case 'KEGIATAN':
                  ...
                  $list['nominal_realisasi'] = $dataRekap[$list['kode_sistem']]['nominal_realisasi'];
                  $list['sisa_dana']         = $dataRekap[$list['kode_sistem']]['sisa_dana'];
                  $sheet->getStyle('B'.$row.':O'.$row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('00DCDCDC');
                  $sheet->mergeCells('D'.$row.':H'.$row);                  
                  $sheet->getStyle('C'.$row.':O'.$row)->applyFromArray($borderTableStyledArray);
                  $unit_kerja_nama           = '';
                  break;
               case 'SUB_KEGIATAN':
                  ...
                  if(($list['idp'] =='') || empty($list['idp'])){
                      $sheet->getStyle('E'.$row.':O'.$row)->applyFromArray($borderTableStyledArray);     
                  }
            ...
            $sheet->setCellValue('O'.$row, $list['sisa_dana']);//, PHPExcel_Cell_DataType::TYPE_NUMERIC);
            $row++;
         }
         $sheet->getStyle('A8:O'.($row-1))->getAlignment()->setWrapText(true)->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
         ...
         $sheet->getStyle('M8:O'.($row-1))->applyFromArray($borderTableOutlineStyledArray);
         
         $sheet->getStyle('I8:O'.($row-1))->getNumberFormat()->setFormatCode('#,##0_);[RED](#,##0)');
         ...
         $sheet->setCellValue('O'.$rows, GTFWConfiguration::GetValue('language', 'sisa_saldo_rp'));
         $sheet->getStyle('A'.$rows.':O'.$rows)->applyFromArray($styledTableHeaderArray);
         

         $tableRow   = $rows+1;
         foreach ($dataResume as $resume) {
            $sheet->mergeCells('A'.$tableRow.':H'.$tableRow);
            $sheet->setCellValue('A'.$tableRow, $resume['unit_nama']);
            ...
            $sheet->setCellValueExplicit('O'.$tableRow, $resume['sisa_dana'], PHPExcel_Cell_DataType::TYPE_NUMERIC);
            $tableRow++;
         }
         $sheet->getStyle('A'.($rows+1).':O'.($tableRow-1))->applyFromArray($borderTableStyledArray);
         $sheet->getStyle('I'.($rows+1).':O'.($tableRow-1))->getNumberFormat()->setFormatCode('#,##0_);[RED](#,##0)');
         ...
         $sheet->setCellValue('H'.($rowSign+2), $requestData['unit_nama']);

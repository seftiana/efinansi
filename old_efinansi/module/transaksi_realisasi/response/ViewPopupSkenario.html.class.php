<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_realisasi/business/AppPopupSkenario.class.php';
...
class ViewPopupSkenario extends HtmlResponse {
	...
	function ProcessRequest() {
		...
		if(isset($pos['nama'])) {
			$jenis = $pos['jenis'];
			$nama = $pos['nama'];
		} elseif(isset($get['nama'])) {
			$jenis = Dispatcher::Instance()->Decrypt($get['jenis']);
			$nama = Dispatcher::Instance()->Decrypt($get['nama']);
		} else {
			$jenis = '';
			$nama = '';
		}
		if(empty($jenis)) $jenis2 = 'auto';
		else $jenis2 = $jenis;
	//view
		...
		$url = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, Dispatcher::Instance()->mSubModule, Dispatcher::Instance()->mAction, Dispatcher::Instance()->mType . '&jenis=' . Dispatcher::Instance()->Encrypt($jenis2) . '&nama=' . Dispatcher::Instance()->Encrypt($nama) . '&cari=' . Dispatcher::Instance()->Encrypt(1));
...
//combo jenis
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'popupSkenario', 'view', 'html'));
		if($this->Pesan) {

...
class ViewTransaksi extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
				'module/transaksi_realisasi/template');
	...
	function ProcessRequest() {
      ...
      Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'jenis_transaksi', 	
	  										array('jenis_transaksi', $arr_jenis_transaksi, 5, 
											  		"false", ' style="width:200px;" id="jenis_transaksi"'),
	...
	function ParseTemplate($data = NULL) {
      $url_view = Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'transaksi', 'view', 'html');
      $this->mrTemplate->AddVar('content', 'URL_CHECK', $url_view);
		$this->mrTemplate->AddVar('content', 'URL_DETIL', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'detilTransaksi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_BATAL', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'detilTransaksi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_POPUP_SKENARIO', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'popupSkenario', 'view', 'html'));
	   $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'popupKegiatanDetil', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK_PENERIMAAN', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'PopupMakPenerimaan', 'view', 'html')); 
      $this->mrTemplate->AddVar('content', 'URL_POPUP_KEGIATAN_UNIT_KERJA', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'popupKegiatanDetil', 'view', 'html')); 
	   #$this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'popupKegiatanDetil', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_SISA_ANGGARAN', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'popupSisaAnggaran', 'view', 'html'));
...
//print_r($this->Data);
      //jika edit, lihat jenis transaksi, jika jenis=umum, set mak == disabled
		...
		if($data['role_name'] == "Administrator") {
			...
			$this->mrTemplate->AddVar('role', 'URL_POPUP_UNITKERJA', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'popupUnitkerja', 'view', 'html'));
      ...
      if(isset($_GET['dataId']) AND $_GET['dataId']->Integer()->Raw() > 0) {
         ...
         $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'updateTransaksi', 'do', 'html') . '&dataId=' . Dispatcher::Instance()->Encrypt($_GET['dataId']));
         if($this->Data['jenis_transaksi'] != "1") {
         ...
         $this->mrTemplate->AddVar('content', "MODE_20080109", 'Tambah');
		   $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'addTransaksi', 'do', 'html'));
      }

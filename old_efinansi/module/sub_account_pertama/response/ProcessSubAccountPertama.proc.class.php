...
/**
 * Class ProcessSubAccountPertama
 * @package sub_account_pertama
 * @copyright 2011 gamatechno
 */

require_once GTFWConfiguration::GetValue( 'application', 'docroot') .
'module/sub_account_pertama/business/SubAccountPertama.class.php';
...
class ProcessSubAccountPertama {
   ...
   {
      $this->mSubAccoutPertama = new SubAccountPertama();
      $queryString             = $this->mSubAccoutPertama->_getQueryString();
   ...
   {
      $requestData      = array();
      if($this->mSubAccoutPertama->method == 'post'){
         $kode       = preg_replace('/\s[\s]+/', '', $this->mSubAccoutPertama->_POST['kode']);
         $kode       = preg_replace('/[\s\W]+/', '', $kode);
         $kode       = preg_replace('/^[\_]+/', '', $kode);
         $kode       = preg_replace('/[\_]+$/', '', $kode);
         $requestData['id']      = $this->mSubAccoutPertama->_POST['dataId'];
         $requestData['kode']    = $kode;
         $requestData['nama']    = $this->mSubAccoutPertama->_POST['nama'];
      }
   ...
   private function checkData()
   {
      ...
      $unique           = $this->mSubAccoutPertama->doCheckUniqueData(
         $requestData['kode'],
         $requestData['id']
      );
      if($unique === false){
   ...
   public function Update()
   {
      ...
      if($checkData['result'] === false){
         ...
         $process             = $this->mSubAccoutPertama->DoUpdate(
            $this->mData['kode'],
            $this->mData['nama'],
            $this->mData['id']
         );
   ...
   public function Delete()
   {
      ...
      if($this->mSubAccoutPertama->method != 'post'){
         ...
         $arrId         = $this->mSubAccoutPertama->_POST['idDelete'];
         $success       = 0;
         $failed        = 0;
         foreach ($arrId as $id) {
            $delete     = $this->mSubAccoutPertama->DoDelete($id);
            if($delete === true){
               $success+=1;
            }else{
               $failed+=1;

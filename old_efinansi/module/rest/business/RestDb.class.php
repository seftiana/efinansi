...
class RestDb extends Database
{
   # internal variables
   private static $mrInstance;
   private static $db_number;
   protected $mSqlFile;
   ...
   public function SendNull($method = 'post')
   {
      ...
      if(empty($query)){
         ...
         if(empty($queries) OR $queries === NULL OR !is_array($queries)){
            ...
            foreach ($queries as $key => $value) {
               $_queries[$key]   = htmlentities(htmlspecialchars($value));
            }

            if(method_exists(Dispatcher::Instance(), 'getQueryString')){
               $queryString      = Dispatcher::Instance()->getQueryString($_queries);
            }else{
   ...
   static function Instance($connectionNumber = 0) {

      if (!isset(self::$mrInstance) && (!isset(self::$db_number) OR self::$db_number <> $connectionNumber)){
         self::$db_number  = $connectionNumber;
         self::$mrInstance = new RestDb($connectionNumber);
      }

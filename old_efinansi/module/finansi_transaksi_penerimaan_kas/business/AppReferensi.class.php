...
class AppReferensi extends Database
{
   ...
   {
      $this->mSqlFile   = 'module/finansi_transaksi_penerimaan_kas/business/app_referensi.sql.php';
    ...
    * @return array
    */
   public function getReferensiKomponen($offset, $limit, $param = array())
   {                                               
      $return     = $this->Open($this->mSqlQueries['get_data_referensi_komponen'], array(
         '%'.$param['kode'].'%',
         $offset,
         $limit
      ));

      $this->mDataReferensiSppu = $return;
   ...
   /**
    * getDetailSppuItem
    * get data sppu item detail
    * @param number $offset
    * @param number $limit
    * @param array $param
    * @return array
    */
   public function getDetailSppuItem($offset, $limit, $param = array())
   {
      $sppuId = array();
      /*
      $rst        = $this->Open($this->mSqlQueries['get_sppu_id'], array(
        '%'.$param['kode'].'%',
         $offset,
         $limit
      ));
      ...
      $return     = $this->Open($this->mSqlQueries['get_detail_sppu_item'], array($sppuId));
      
      if(empty($return)){
         return null;
      } else {
         $data_sppu_item = self::ChangeKeyName($return);
         $id            = '';
         $index          = 0;
         $data_grid      = array();

         for ($i=0; $i < count($data_sppu_item);) {
            if((int) $id === (int) $data_sppu_item[$i]['id']){
               $data_grid[$id][$index]    =  $data_sppu_item[$i];
               $i++;
               $index++;
            }else{
               $id      = (int) $data_sppu_item[$i]['id'];
               unset($index);
               $index   = 0;
            }
         }
         return compact('data_grid');
      }
   }

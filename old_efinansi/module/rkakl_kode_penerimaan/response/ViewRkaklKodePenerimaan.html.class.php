...
class ViewRkaklKodePenerimaan extends HtmlResponse {
	...
	//view
		$totalData = $Obj->GetCountRkaklKodePenerimaan($kode, $nama);
		$itemViewed = 20;
		$currPage = 1;
		$startRec = 0 ;
		if(isset($_GET['page'])) {
		...
		$data = $Obj->GetRkaklKodePenerimaan($kode, $nama, $startRec, $itemViewed);
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('rkakl_kode_penerimaan', 'RkaklKodePenerimaan', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_ADD', Dispatcher::Instance()->GetUrl('rkakl_kode_penerimaan', 'inputRkaklKodePenerimaan', 'view', 'html'));
		...
		if (empty($data['data'])) {
			...
			$label = "RKAKL KodePenerimaan";
			$urlDelete = Dispatcher::Instance()->GetUrl('rkakl_kode_penerimaan', 'deleteRkaklKodePenerimaan', 'do', 'html');
			$urlReturn = Dispatcher::Instance()->GetUrl('rkakl_kode_penerimaan', 'RkaklKodePenerimaan', 'view', 'html');
			...
			for ($i=0; $i<sizeof($dataK); $i++) {
				...
				$dataK[$i]['url_edit'] = Dispatcher::Instance()->GetUrl('rkakl_kode_penerimaan', 'inputRkaklKodePenerimaan', 'view', 'html') . '&dataId=' . $idEnc/* . '&page=' . $encPage*/;

...
/**
 * 
 * ViewExcelLapBiayaLayananPerUnit
...
class ViewExcelLapBiayaLayananPerUnit extends XlsResponse 
{
    ...
    function GetFileName() 
    {
     ...
     $label = 'Laporan_Biaya_Layanan_Per_Unit';
    ...
    function ProcessRequest() 
    {
		...
		if (empty($listDataItem)) {
		   ...
		   $this->mWorksheets['Data']->write(0, 0, 'Laporan Pendapatan Belanja Agregat', $fTitle);
			...
			for($i = 0;$i < sizeof($listDataItem);){
				if( ($listDataItem[$i]['unit_kerja_id'] == $unitId) && 
					...
					($listDataItem[$i]['mak_id'] == $makId) 
						...
						$send[$i]['jumlah']= $listDataItem[$i]['komp_jumlah'] ;
						
						$this->mWorksheets['Data']->write($num, 0, $send[$i]['nomor'],$format);
					...
					$i++;	
						...
						$this->mWorksheets['Data']->write($num, 0, $send[$i]['nomor'],$formatProgram);
				...
				//}elseif($listDataItem[$i]['biaya_langsung'] != $bl){
						...
						$this->mWorksheets['Data']->write($num, 0, $send[$i]['nomor'],$formatKegiatan);
						...
						$send[$i]['volume']='';
						$send[$i]['jumlah']=$this->mDBObj->GetTotalPerMak(
														$listDataItem[$i]['tahun_anggaran_id'],
														$listDataItem[$i]['mak_id'] ,
														$listDataItem[$i]['unit_kerja_id'] 
														);
						$this->mWorksheets['Data']->write($num, 0, $send[$i]['nomor'],$formatSubKegiatan);

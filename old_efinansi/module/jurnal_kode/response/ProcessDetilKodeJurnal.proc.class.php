...
class ProcessDetilKodeJurnal {
	...
	function __construct() {
		$this->Obj = new DetilKodeJurnal();
		$this->_POST = $_POST->AsArray();
		$this->decId = Dispatcher::Instance()->Decrypt($_REQUEST['dataId']);
		$this->encId = Dispatcher::Instance()->Encrypt($this->decId);
		$this->pageView = Dispatcher::Instance()->GetUrl('jurnal_kode', 'detilKodeJurnal', 'view', 'html');
	...
	function Add() {
		$cek = $this->Check();
		if($cek === true) {
			$tmbhKodeJurnal = $this->Obj->DoAddData(strtoupper($this->_POST['kode']), $this->_POST['nama'], $this->getPOST());
	...
	function Update() {
		$cek = $this->Check();
		if($cek === true) {
			$updateKodeJurnal = $this->Obj->DoUpdateData(strtoupper($this->_POST['kode']), $this->_POST['nama'], $this->decId);
	...
	function Delete() {
		...
		if($deleteArrData === true) {
			Messenger::Instance()->Send('jurnal_kode', 'detilKodeJurnal', 'view', 'html', array($this->_POST,'Penghapusan Data Berhasil Dilakukan', $this->cssDone),Messenger::NextRequest);
		} else {
			...
			Messenger::Instance()->Send('jurnal_kode', 'detilKodeJurnal', 'view', 'html', array($this->_POST, $gagal . ' Data Tidak Dapat Dihapus.', $this->cssFail),Messenger::NextRequest);
		}

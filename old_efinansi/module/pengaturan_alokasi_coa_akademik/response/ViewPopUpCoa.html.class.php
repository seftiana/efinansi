...
'module/pengaturan_alokasi_coa_akademik/business/AppPopupCoa.class.php';
...
class ViewPopupCoa extends HtmlResponse
{
	...
	function ProcessRequest() 
	{
		...
		else
		{
			...
			if (is_object($_GET['tipe'])) $return['tipe'] = $_GET['tipe']->mrVariable;
			else $return['tipe'] = $_GET['tipe'];
		}
		...
		elseif (isset($_GET['data'])) 
		{
			...
			if (is_object($_GET['data'])) $this->data = $_GET['data']->AsArray();
			else $this->data = $_GET['data'];
		}
		$itemViewed = 20;
		$currPage = 1;
		$startRec = 0;
	...
	function ParseTemplate($data = NULL) 
	{
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('pengaturan_alokasi_coa_akademik', 'PopUpCoa', 'view', 'html') . '&data[name]=' . $this->data['nama'] . '&data[kode]=' . $this->data['kode'] . '&tipe=' . $data['tipe']);
		...
		else
		{
			...
			for ($i = 0;$i < sizeof($dataGrid);$i++) 
			{
				...
				if (!$dataGrid[$i]['isParent']) 
				{
					...
					if ($data['tipe'] == 'all') $dataGrid[$i]['set_parent'] = '<a class="link_'.$dataGrid[$i]['id'].'" href="javascript:void(0)" onclick="setParent(this, \'' . $dataGrid[$i]['id'] . '\',\'' . $dataGrid[$i]['kode'] . '\',\'' . $dataGrid[$i]['nama'] . '\')" onmouseover="status=\'Set preferences...\';return true" >&nbsp;Pilih&nbsp;</a>';

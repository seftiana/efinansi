 ...
 * modified 2015-03-30
 */
class AppApprovalPencairan extends Database {
   ...
   function __construct($connectionNumber=0) {
      $this->mSqlFile   = 'module/approval_pencairan/business/appapprovalpencairan.sql.php';
   ...
   function GetData ($offset, $limit, $data)
   {
      $return     = $this->Open($this->mSqlQueries['get_data'], array(
         ...
         (int)($data['bulan'] == '' OR strtolower($data['bulan']) == 'all'),         
            $data['bulan_anggaran'],
         (int)($data['bulan_anggaran'] == '' OR strtolower($data['bulan_anggaran']) == 'all'),         
         $offset,
         $limit
      ));
   ...
   public function doApproval($param = array())
   {
      ...
      if(strtoupper($param['status']) == 'YA'){
         if(empty($param['komponen'])){
            $result  &= false;
         }else{
            foreach ($param['komponen'] as $komponen) {
               $result  &= $this->Execute($this->mSqlQueries['do_approval_realisasi_detail'], array(
                  $komponen['nominal_approve'],
                  $userId,
                  $komponen['id'],
                  $komponen['pr_id']
               ));
   ...
   function GetDataApproval($kodenama,$offset, $limit, $tahun_anggaran, $unitkerja='', $program='', $jenis=''/*, $is_approve = ''*/) {
      //print_r($kodenama);exit;
      //if($unitkerja != "") {
      ...
      /*if($is_approve != "")
         $str_is_approve = " AND pengrealIsApprove=".$is_approve;
      else
         $str_is_approve = "";*/
      $sql = sprintf($this->mSqlQueries['get_data_approval'],
                        $kodenama,
                        $kodenama,
                        $kodenama,
                        '%'.$kodenama.'%',
...
//===DO==
   function DoUpdate($status, $nominal, $userId, $id) {
      $result = $this->Execute($this->mSqlQueries['do_update'], array($status, $nominal, $userId, $id));
      return $result;
   }

   function GetDataCetakTambahan($idPengReal, $subProgNama) {
      $result = $this->Open($this->mSqlQueries['get_data_cetak_tambahan'], array($idPengReal, $subProgNama));
      return $result;
   }
   function GetDataCetak ($id,$approvalId) {
      $result = $this->Open($this->mSqlQueries['get_data_cetak'], array($id,$approvalId));
      //$this->mdebug();
     return $result;
   }
   function GetKegDetIdCetak($id) {
      $result = $this->Open($this->mSqlQueries['get_kegdetid_cetak'], array($id));
      return $result[0];
   }

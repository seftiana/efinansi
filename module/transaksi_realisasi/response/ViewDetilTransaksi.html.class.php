<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_realisasi/business/AppDetilTransaksi.class.php';
...
class ViewDetilTransaksi extends HtmlResponse {
	...
	function ParseTemplate($data = NULL) {
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'detilTransaksi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_KEMBALI', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'Transaksi', 'view', 'html'));
		if($this->Pesan) {
		...
		if (empty($data['dataDetilTransaksi'])) {
         ...
         $label = 'Transaksi';
         $urlDelete = 'transaksi_realisasi|deleteTransaksi|do|html';
         $urlReturn = 'transaksi_realisasi|detilTransaksi|view|html';
...
//mulai bikin tombol delete
         ...
         $j=0; #print_r($dataDetilTransaksi);
            ...
            $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'FormCetakBKK', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkk';
            ...
            '&label='.$label.'&dataName='.$dataDetilTransaksi[$i]['kkb'];
               $url_edit = Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'Transaksi', 'view', 'html') . '&dataId=' . $idEnc;

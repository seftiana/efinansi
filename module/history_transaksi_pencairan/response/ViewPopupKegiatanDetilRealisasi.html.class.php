...
'module/history_transaksi_pencairan/business/transaksi_realisasi/AppPopupKegiatanDetil.class.php';

class ViewPopupKegiatanDetilRealisasi extends HtmlResponse {
	...
	function TemplateModule() {
		...
		$this->SetTemplateFile('transaksi_realisasi/view_popup_kegiatan_detil.html');
	}
	...
	function ProcessRequest() {
		...
		if(!empty($POST)) {
			...
			$kegiatan_detil = Dispatcher::Instance()->Decrypt($GET['kegiatan_detil']);
		} else {
			$kegiatan_detil="";
		}
		$this->decUnitkerjaId = Dispatcher::Instance()->Decrypt($GET['unitkerja']);
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('history_transaksi_pencairan', 'popupKegiatanDetilRealisasi', 'view', 'html') . '&unitkerja=' . $this->encUnitkerjaId);
		if($this->Pesan) {
		...
		if (empty($data['data_kegiatan_detil'])) {
			...
			for ($i=0; $i<sizeof($data_kegiatan_detil); $i++) {
				$no = $i+$data['start'];
				$data_kegiatan_detil[$i]['nama']	= str_replace("'","\'",
													  $data_kegiatan_detil[$i]['nama']);
				...
				$data_kegiatan_detil[$i]['nominal_aprove_hidden'] = ($data_kegiatan_detil[$i]
				['nominal_aprove'] - $data_kegiatan_detil[$i]['nominal_yg_sudah_dicairkan']);

				$data_kegiatan_detil[$i]['nominal_aprove'] = number_format(
				$data_kegiatan_detil[$i]['nominal_aprove'], 2, ',', '.');
				$data_kegiatan_detil[$i]['nominal_yg_sudah_dicairkan'] = number_format
				($data_kegiatan_detil[$i]['nominal_yg_sudah_dicairkan'], 2, ',', '.');

				$data_kegiatan_detil[$i]['nominal_yg_bisa_dicairkan'] = number_format
				($data_kegiatan_detil[$i]['nominal_aprove_hidden'], 2, ',', '.');

				$this->mrTemplate->AddVars('data_kegiatan_detil_item',
				$data_kegiatan_detil[$i], 'KEGIATAN_DETIL_');

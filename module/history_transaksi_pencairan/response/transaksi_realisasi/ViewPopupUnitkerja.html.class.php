<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_realisasi/business/AppPopupUnitkerja.class.php';
...
class ViewPopupUnitkerja extends HtmlResponse {
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_realisasi', 'popupUnitkerja', 'view', 'html') . "&satker=" . Dispatcher::Instance()->Encrypt($search['satker']));
		if($this->Pesan) {
         ...
         */
         //print_r($dataUnitkerja);
			for ($i=0; $i<sizeof($dataUnitkerja); $i++) {
               ...
               $dataUnitkerja[$i]['label_unit'] = 
			   str_replace("'","\'",$dataUnitkerja[$i]['satker']) . " / " . 
			   str_replace("'","\'",$dataUnitkerja[$i]['unit']);
            }

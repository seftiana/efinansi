...
class ViewCetakLaporanKonsolidasiDetail extends HtmlResponse {
    protected $mObj;

    function TemplateModule() {
        $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application', 'docroot') . 'module/laporan_konsolidasi/template');
        $this->SetTemplateFile('view_cetak_laporan_konsolidasi_detail.html');
    }
        ...
        foreach ($gridList as $key => $itemLaporan) {
            ...
            if ($itemLaporan['is_summary'] == 'Y') {
                ...
                if ($itemLaporan['is_child'] == '0') {
                    ...
                    $dataDetail = $this->mObj->LaporanBuilder()->getLaporanDetail($requestData['tanggal_awal'], $requestData['tanggal_akhir'], $itemLaporan['id'],'00',$status);
                    $dataDetailYayasan = $this->mObj->LaporanBuilder()->getLaporanDetail($requestData['tanggal_awal'], $requestData['tanggal_akhir'], $itemLaporan['id'],'01',$status);
                    ...
                    if(!empty($dataDetail)){
                        ...
                        foreach($dataDetail as $key => $valueDet){
                            $valueDet['padding'] = ($itemLaporan['level'] + 1) * 15;

                            if (!empty($data['status']) && $data['status'] == 'TL') {
                                $nominal = $valueDet['kellap_coa_saldo_lalu'];
                            } else {
                                $nominal = $valueDet['kellap_coa_saldo'];
                            }

                            if (!empty($data['status']) && $data['status'] == 'TL') {
                                $nominalYayasan = $dataDetailYayasan[$key]['kellap_coa_saldo_lalu'];
                            } else {
                                $nominalYayasan = $dataDetailYayasan[$key]['kellap_coa_saldo'];
                            }

                            $nominalGabung = $nominal + $nominalYayasan;
                            ...
                            if ($nominalYayasan >= 0) {
                                $valueDet['kellap_nominal_saldo_yayasan'] = number_format($nominalYayasan, 2, ',', '.');
                            } else {
                                $valueDet['kellap_nominal_saldo_yayasan'] = '(' . number_format($nominalYayasan * (-1), 2, ',', '.') . ' )';
                            }

                            if ($nominalGabung >= 0) {
                                $valueDet['kellap_nominal_saldo_gabung'] = number_format($nominalGabung, 2, ',', '.');
                            } else {
                                $valueDet['kellap_nominal_saldo_gabung'] = '(' . number_format($nominalGabung * (-1), 2, ',', '.') . ' )';
                            }

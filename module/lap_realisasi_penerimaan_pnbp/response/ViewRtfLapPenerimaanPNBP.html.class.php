<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/lap_realisasi_penerimaan_pnbp/business/AppLapPenerimaanPNBP.class.php';

require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/user_unit_kerja/business/UserUnitKerja.class.php';

class ViewRtfLapPenerimaanPNBP extends HtmlResponse {	
	...
	function ProcessRequest() {	
		...
		$data = $Obj->GetDataRealisasiPNBP($tahun_anggaran, $userId); 
		$data_jumlah = $Obj->GetDataForTotal($tahun_anggaran, $userId);
      ...
      $contents = str_replace('[PERIODE]', $data['periode']['name'], $contents);
		...
		for ($i=0; $i<sizeof($data_list);) {
			if(($data_list[$i]['kode_satker'] == $kode_satker) && ($data_list[$i]['kode_unit'] == $kode_unit)) {
				if($data_list[$i]['idrencana'] == "") {
					...
					$send = $data_list[$i];
					$send['total_penerimaan'] = number_format($data_list[$i]['total'], 2, ',', '.');
					$send['volume'] = $data_list[$i]['volume'];
					$send['pagu'] = $data_list[$i]['pagu'];
					$send['tarif'] = number_format($data_list[$i]['tarif'], 2, ',', '.');
					$send['totalterima'] = number_format($data_list[$i]['total_kali'], 2, ',', '.');
					...
					$send['total_penerimaan'] = number_format($data_list[$i]['jumlah_total'], 2, ',', '.');
					$send['volume'] = "";
					$send['pagu'] = "";
					$send['tarif'] = "";
					$send['totalterima'] = number_format($data_list[$i]['totalterima'], 2, ',', '.');
		...
		$jumlah = ' '.number_format($data['jumlah'], 2, ',', '.').'\par';
		$terima = ' '.number_format($data['penerimaan'], 2, ',', '.')."\par";

...
class ViewExcelLapPosisiKeuanganDetil extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $this->SetFileName('posisi_keuangan_detil_' . date("d-m-Y") . '.xls');
      ...
      if (empty($posisiKeuangan)) {
         ...
         $sheet->setCellValueByColumnAndRow(0, 2, 'Laporan Posisi Keuangan Detail');
         $sheet->setCellValueByColumnAndRow(0, 3, 'Periode ' . IndonesianDate($tgl_awal, 'yyyy-mm-dd').' s/d '.IndonesianDate($tgl_akhir, 'yyyy-mm-dd'));
         ...
         foreach ($posisiKeuangan as $key => $itemLaporan) {
            ...
            if ($itemLaporan['is_summary'] == 'Y') {
               ...
               $sheet->getStyle('B'.$row.':C'.$row)->getNumberFormat()->setFormatCode('_("Rp"* #,##0_);_("Rp"* \(#,##0\);_("Rp"* "-"_);_(@_)');
            } else {
               ...
               if ($itemLaporan['is_child'] == '0') {
                  ...
                  $dataDetail = $this->mObj->LaporanBuilder()->getLaporanDetail($tgl_awal, $tgl_akhir, $itemLaporan['id'],$subAccount,$status);
               }
            }
            ...
            if(!empty($dataDetail) && $isShowDet){
               foreach($dataDetail as $valueDet){
                  ...
                  $sheet->setCellValueByColumnAndRow(0, $row, str_repeat($space,$itemLaporan['level']+4).$valueDet['kellap_coa_kode'].' ['.$valueDet['kellap_sub_acc'].'] - '.$valueDet['kellap_coa_nama']);

...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
		'module/transaksi_spj/business/AppTransaksi.class.php';
...
class ViewTransaksiDetil extends HtmlResponse {
	...
	function ProcessRequest() {
      ...
      if(isset($_GET['dataId'])) {
         ...
         //$return['datadbMAK'] = $Obj->GetTransaksiMAK($datadb['id']);
         //print_r($datadb);
         $return['dataJurnal'] = $Obj->GetDataJurnal($datadb['id']);
      } 
      ...
      //COMBO JENIS TRANSAKSI
		...
		Messenger::Instance()->SendToComponent(
												...
												array(
														'jenis_transaksi', 
														$arr_jenis_transaksi, 5, 
														"false", 
														' style="width:200px;" 
														id="jenis_transaksi" onchange="showHideMAK(this)"'),
											 	Messenger::CurrentRequest);
	...
	function ParseTemplate($data = NULL) 
	{
		...
		$this->mrTemplate->AddVar('content', 'PENANGGUNG_JAWAB', $datadb['penanggung_jawab']);
      	...
      	$this->mrTemplate->AddVar('content', "MAK_LABEL", $datadb['mak_label']);
      ...
      if(empty($datadbFile)) {
			...
			$no_file_list = $data['no_file_list'];
         	...
         	for($i=0;$i<sizeof($datadbFile);$i++) {
            	...
            	//$datadbFile[$i]['url_delete'] =   $url_view . 
				//									"&no_file_delete=" . 
      	...
      	$dataJurnal = $data['dataJurnal'];
		//print_r($dataJurnal);
	 	if(empty($dataJurnal)) {
			$this->mrTemplate->AddVar('tpl_jurnal_list', 'JURNAL_LIST_EMPTY', 'YES');
      	} else {
			$this->mrTemplate->AddVar('tpl_jurnal_list', 'JURNAL_LIST_EMPTY', 'NO');
			$total_debet = 0;
			$total_kredit = 0;
         	for($i=0;$i<sizeof($dataJurnal);$i++) {
         		$total_debet += $dataJurnal[$i]['debet'];
         		$total_kredit +=$dataJurnal[$i]['kredit'];
         		$dataJurnal[$i]['debet'] = number_format($dataJurnal[$i]['debet'],2,',','.');
         		$dataJurnal[$i]['kredit'] = number_format($dataJurnal[$i]['kredit'],2,',','.');
				$this->mrTemplate->AddVars('tpl_jurnal_item', $dataJurnal[$i], 'DATA_');
				$this->mrTemplate->parseTemplate('tpl_jurnal_item', 'a');
        	 }
        	 $this->mrTemplate->AddVar('tpl_jurnal_list', 'TOTAL_DEBET', 
			 											number_format($total_debet,2,',','.'));
        	 $this->mrTemplate->AddVar('tpl_jurnal_list', 'TOTAL_KREDIT', 

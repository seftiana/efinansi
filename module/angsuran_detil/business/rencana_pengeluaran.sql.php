...
FROM
   ...
   LEFT JOIN rencana_pengeluaran2
...
FROM bulan_angsuran_detail
   ...
   LEFT JOIN (
      ...
      FROM
         rencana_pengeluaran2 rpeng_sts
...
LIMIT %s, %s) AS rkat
   ...
   FROM bulan_angsuran_detail_status
   GROUP BY kegdetstatusRncnpengeluaranId) AS kds_a
   INNER JOIN bulan_angsuran_detail_status AS kds_b 
   ...
   AND kds_a.kodeaksi = kds_b.kegdetstatusLogAktifitas) AS kdstatus
      ...
      AND kdstatus.rpengId = rencana_pengeluaran2.rncnpengeluaranId
...
SELECT
   komponen1.kompId AS id,
   IF(rncnpengeluaranId IS NOT NULL, komponen1.kompId, NULL) AS dataId,
   IFNULL(komponen1.kompKode, rncnpengeluaranKomponenKode) AS kode,
   LEFT(komponen1.kompKode,2) AS basIdkomponen,
   IFNULL(komponen1.kompNama, rncnpengeluaranKomponenNama) AS nama,
   IFNULL(rncnpengeluaranSatuan, 0) AS jumlah,
   IF(IFNULL(komponen1.kompFormula, rncnpengeluaranFormula) IS NULL OR IFNULL(komponen1.kompFormula, rncnpengeluaranFormula) = '', 1, IFNULL(komponen1.kompFormula, rncnpengeluaranFormula)) AS formula,
   IF(komponen1.kompFormulaHasil = '0' ,1, komponen1.kompFormulaHasil) AS hasilFormula,
   IFNULL(komponen1.kompNamaSatuan, rncnpengeluaranNamaSatuan) AS satuan,
   kompkegBiaya AS biayaMax,
   komponen1.kompIsSBU AS isSbu,
   IFNULL(rncnpengeluaranKomponenNominal,kompkegBiaya) AS biaya,
   IF(IFNULL(komponen1.kompDeskripsi,rncnpengeluaranKomponenDeskripsi) = '', '-', IFNULL(komponen1.kompDeskripsi,rncnpengeluaranKomponenDeskripsi)) AS deskripsi,
   ...
   komponen1.kompSumberDanaId AS sumberDanaId,
...
FROM bulan_angsuran_detail
   ...
   LEFT JOIN rencana_pengeluaran2
      ...
      AND IF(komponen1.kompId IS NULL, 1, komponen1.kompKode = rncnpengeluaranKomponenKode)
   LEFT JOIN finansi_ref_pagu_bas AS mak
      ON (mak.paguBasId = rncnpengeluaranMakId OR mak.paguBasId = kompMakId)
   LEFT JOIN finansi_ref_pagu_bas AS bas
      ON bas.paguBasId = mak.paguBasParentId
   LEFT JOIN finansi_pa_komponen_unit_kerja
      ON finansi_pa_komponen_unit_kerja.kompUnitKompId = komponen1.kompId
...
INSERT INTO rencana_pengeluaran2 SET rncnpengeluaranKegdetId = '%s',
...
rncnpengeluarankompId  = '%s'
";

$sql['do_delete_rencana_pengeluaran_kegiatan']  = "
DELETE FROM rencana_pengeluaran2 WHERE rncnpengeluaranKegdetId = '%s'
";
...
FROM
  rencana_pengeluaran2 rp
  JOIN bulan_angsuran_detail kd ON (kd.kegdetId=rp.rncnpengeluaranKegdetId)
...
$sql['get_data_komponen_add'] ="
    ...
    FROM
       bulan_angsuran_detail kd INNER JOIN
          (
           kegiatan_ref kr INNER JOIN
             (komponen_kegiatan kk INNER JOIN komponen1 komp ON komp.kompId = kk.kompkegKompId)
...
$sql['get_data_komponen_edit'] ="
    ...
    FROM
       bulan_angsuran_detail kd
       JOIN rencana_pengeluaran2 rp  ON (kd.kegdetId=rp.rncnpengeluaranKegdetId)
...
FROM
   ...
   LEFT JOIN bulan_angsuran ON bulan_angsuran.kegId = kd.kegdetKegId
   LEFT JOIN komponen_kegiatan kk ON kr.kegrefId = kk.kompkegKegrefId
   LEFT JOIN komponen1 komp ON komp.kompId = kk.kompkegKompId
   LEFT JOIN rencana_pengeluaran2 rp ON kd.kegdetId = rp.rncnpengeluaranKegdetId AND IF(komp.kompId IS NULL, 1, komp.kompKode = rp.rncnpengeluaranKomponenKode)
   ...
   RIGHT JOIN finansi_pa_komponen_unit_kerja AS ku ON
         (ku.kompId = komp.kompId AND ku.unitkerjaId = bulan_angsuran.kegUnitkerjaId)
WHERE
...
(
    ...
    FROM
      ...
      LEFT JOIN finansi_ref_pagu_bas m
       ...
       LEFT JOIN komponen1 k ON k.kompKode =  rp.rncnpengeluarankomponenKode
WHERE
...
/**
 * get jumlah komponen1 yang ada di rencana pengeluran
 * berdasarkan bulan_angsuran detail
 */
$sql['get_count_detai_belanja']="
SELECT
COUNT(rpeng.`rncnpengeluaranId`) AS total
FROM rencana_pengeluaran2 rpeng
...
$sql['get_jenis_kegiatan']="
   ...
   FROM
      bulan_angsuran_detail kd
      JOIN bulan_angsuran k ON k.kegId=kd.kegdetKegId
...
WHEN LENGTH(kr.kegrefNomor) = 2 THEN kr.kegrefNomor END*/kr.kegrefNomor),'')
 ...
 IF (rp.rncnpengeluaranIsAprove = 'Ya',(rp.rncnpengeluaranSatuanAprove * rp.rncnpengeluaranKomponenNominalAprove * IF(komponen1.kompFormulaHasil = '0',1, komponen1.kompFormulaHasil)), (rp.rncnpengeluaranSatuan * rp.rncnpengeluaranKomponenNominal * IF(komponen1.kompFormulaHasil = '0',1,komponen1.kompFormulaHasil))) AS jumlah
FROM
rencana_pengeluaran2 rp
JOIN bulan_angsuran_detail kd  ON (kd.kegdetId=rp.rncnpengeluaranKegdetId)
JOIN bulan_angsuran k ON k.kegId=kd.kegdetKegId
...
JOIN kegiatan_ref kr ON kr.kegrefId=kd.kegdetKegrefId
LEFT JOIN komponen1 ON kompKode = rncnpengeluaranKomponenKode
...
$sql['do_add']       = "
INSERT INTO rencana_pengeluaran2
   (
...
$sql['do_delete']    = "
DELETE FROM `rencana_pengeluaran2`
WHERE `rncnpengeluaranId`=%s
";

$sql['do_update_rutin']    = "
UPDATE `rencana_pengeluaran2`
SET
...
FROM rencana_pengeluaran2
   JOIN bulan_angsuran_detail ON rncnpengeluaranKegdetId = kegdetId
   JOIN bulan_angsuran ON kegdetKegId = kegId
   JOIN komponen1 ON rncnpengeluarankomponenKode = kompKode
...
FROM
   ...
   LEFT JOIN kegiatan_ref kr ON kd.kegdetKegrefId = kr.kegrefId
   LEFT JOIN bulan_angsuran ON bulan_angsuran.kegId = kd.kegdetKegId
   LEFT JOIN rencana_pengeluaran2 rp ON kd.kegdetId = rp.rncnpengeluaranKegdetId
  WHERE
      kd.kegdetId = %s
";
 ...
 * @since 8 Maret 2012
 */
$sql['get_max_rencana_pengeluaran2']="
SELECT
   SUM(rncnpengeluaranKomponenNominal * rncnpengeluaranSatuan) AS max_nominal
FROM
   rencana_pengeluaran2
   LEFT JOIN bulan_angsuran_detail ON  kegdetId = rncnpengeluaranKegdetId
   LEFT JOIN bulan_angsuran ON kegId = kegdetKegId
   LEFT JOIN `unit_kerja_ref` ON `unit_kerja_ref`.`unitkerjaId` = `bulan_angsuran`.`kegUnitkerjaId`
WHERE
    rncnpengeluaranIsAprove NOT IN('Tidak' )
    AND
   `unit_kerja_ref`.`unitkerjaId` = '%s'
   AND
   `bulan_angsuran`.`kegThanggarId` = '%s'
";

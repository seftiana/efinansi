...
/**
* ================= doc ====================
* FILENAME     : ViewExcelLapPenerimaanPNBPUnit.xlsx.class.php
* @package     : ViewExcelLapPenerimaanPNBPUnit
* scope        : PUBLIC
* @Author      : Eko Susilo
* @Created     : 2014-07-10
* @Modified    : 2014-07-10
...
'module/lap_realisasi_penerimaan_pnbp_unit/business/AppLapPenerimaanPNBPUnit.class.php';
require_once GTFWConfiguration::GetValue( 'application', 'docroot') .
'module/user_unit_kerja/business/UserUnitKerja.class.php';

class ViewExcelLapPenerimaanPNBPUnit extends XlsxResponse
{
   # Internal Variables
   public $Excel;
   ...
   function ProcessRequest()
   {
      ...
      $this->SetFileName('laporan_penerimaan_pnbp_unit_'.date('Ymd', time()).'.xls');
      ...
      $Obj              = new AppLapPenerimaanPNBPUnit();
      ...
      if (empty($data)) {
          ...
          * set label
          */
         $lap_penerimaan_label   = GTFWConfiguration::GetValue('language','lap_penerimaan_pnbp_alokasi_unit');
         ...
         $sheet->getStyle('A3:Q5')->getFont()->setBold(true);
         $sheet->getStyle('A6:Q7')->applyFromArray($styledTableHeaderArray);
         ...
         for ($i=0; $i < count($data);) {
            if((int)$unit === (int)$data[$i]['idunit']){
               ...
               $dataList[$index]['nama']     = $data[$i]['nama'];
         ...
         $sheet->getStyle('A8:Q'.$row)->applyFromArray($borderTableStyledArray);
         $sheet->getStyle('D8:Q'.$row)->getNumberFormat()->setFormatCode('#,##0_);[RED](#,##0)');
      }

<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/lap_rekap_unitkerja/business/AppPopupProgram.class.php';
...
class PopupProgram extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/lap_rekap_unitkerja/template');
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('lap_rekap_unitkerja', 'program', 'popup', 'html') . '&tahun_anggaran=' . Dispatcher::Instance()->Encrypt($search['tahun_anggaran']) . '&tahun_anggaran_label=' . Dispatcher::Instance()->Encrypt($search['tahun_anggaran_label']));
		if($this->Pesan) {
		...
		if (empty($data['dataProgram'])) {
			...
			for ($i=0; $i<sizeof($dataProgram); $i++) {
				...
				//if($dataProgram[$i]['kode'] < 10) $dataProgram[$i]['kode'] = '0' . $dataProgram[$i]['kode'] . '.00.00';
				//else $dataProgram[$i]['kode'] = $dataProgram[$i]['kode'] . '.00.00';

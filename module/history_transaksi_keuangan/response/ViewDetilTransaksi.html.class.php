...
'module/history_transaksi_keuangan/business/AppDetilTransaksi.class.php';
...
class ViewDetilTransaksi extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/transaksi/template');
	...
	function ProcessRequest() {
      ...
      if(isset($_POST['mulai_day'])) {
         ...
         $decSelesaiTahun = Dispatcher::Instance()->Decrypt($_GET['selesai_year']);
			...
			$uraian = Dispatcher::Instance()->Decrypt($_GET['uraian']);
	...
	//view
		...
		$dataDetilTransaksi = $Obj->getData($startRec, $itemViewed, $mulai_selected, $selesai_selected, $nomor, $uraian, $posting);
      ...
      Messenger::Instance()->SendToComponent('tanggal', 'Tanggal', 'view', 'html', 'selesai', array($selesai_selected, $tahun['start'], $tahun['end'], '', '', 'selesai'), Messenger::CurrentRequest);
		...
		$return['nomor'] = $nomor;
		$return['uraian'] = $uraian;
		$return['start'] = $startRec+1;
      return $return;
	}

	function ParseTemplate($data = NULL) {
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('history_transaksi_keuangan', 'detilTransaksi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_KEMBALI', Dispatcher::Instance()->GetUrl('history_transaksi_keuangan', 'Transaksi', 'view', 'html'));
		...
		if (empty($data['dataDetilTransaksi'])) {
         ...
         $label = 'Transaksi';
         $urlDelete = 'history_transaksi_keuangan|deleteTransaksi|do|html';
         $urlReturn = 'history_transaksi_keuangan|detilTransaksi|view|html';
         $URLDELETE = Dispatcher::Instance()->GetUrl('history_transaksi_keuangan', 'deleteTransaksi', 'do', 'html');
...
//mulai bikin tombol delete
         ...
         $j=0; #print_r($dataDetilTransaksi);
            ...
            if($dataDetilTransaksi[$i]['tipe_id'] == 1) {
               $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('history_transaksi_keuangan', 'FormCetakBKM', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkm';
               $dataDetilTransaksi[$i]['label_cetak_bukti'] = 'Cetak BKM';
               #$url_cetak_bukti = Dispatcher::Instance()->GetUrl('transaksi', 'CetakBuktiTransaksi', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkm';
               ...
               $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('history_transaksi_keuangan', 'FormCetakBKK', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkk';
               $dataDetilTransaksi[$i]['label_cetak_bukti'] = 'Cetak BKK';
               #$url_cetak_bukti = Dispatcher::Instance()->GetUrl('transaksi', 'CetakBuktiTransaksi', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bkk';
               ...
               $dataDetilTransaksi[$i]['url_cetak_bukti'] = Dispatcher::Instance()->GetUrl('history_transaksi_keuangan', 'FormCetakBM', 'view', 'html') . '&dataId=' . $idEnc . '&tipe=bm';
            ...
            $dataDetilTransaksi[$i]['nominal_label'] = number_format($dataDetilTransaksi[$i]['nominal'], 2, ',', '.');
				...
				$dataDetilTransaksi[$i]['url_cetak'] = Dispatcher::Instance()->GetUrl('history_transaksi_keuangan', 'FormCetakTransaksi', 'view', 'html') . '&dataId=' . $idEnc;
            ...
            '&id='.Dispatcher::Instance()->Encrypt($dataDetilTransaksi[$i]['id']).
            '&label='.$label.'&dataName='.$dataDetilTransaksi[$i]['kkb']; 
               $url_edit = Dispatcher::Instance()->GetUrl('history_transaksi_keuangan', 'Transaksi', 'view', 'html') . '&dataId=' . $idEnc;

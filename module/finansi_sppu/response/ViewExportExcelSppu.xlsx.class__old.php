...
class ViewExportExcelSppu extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $sheet->getStyle('I2:J3')->applyFromArray(array(
         'alignment' => array(
      ...
      $getHeightRow = ceil(strlen($dataSppu['terbilang'])/40) * 15;     
      $sheet->getRowDimension(($row+2))->setRowHeight($getHeightRow);
      ...
      $kabiro_keu    = strtolower(GTFWConfiguration::GetValue('organization', 'kabiro_keu'));

      $sheet->mergeCells('B'.($row+6).':D'.($row+6));
      $sheet->mergeCells('B'.($row+7).':D'.($row+7));
      $sheet->setCellValue('B'.($row+6), ucwords($kabiro_keu));

      $sheet->mergeCells('I'.($row+6).':K'.($row+6));
      $sheet->mergeCells('I'.($row+7).':K'.($row+7));
      $sheet->setCellValue('I'.($row+6), GTFWConfiguration::GetValue('organization', 'warek_akademik_dan_keuangan'));
      ...
      $sheet->mergeCells('B'.($row+10).':D'.($row+10));
      $sheet->mergeCells('I'.($row+10).':K'.($row+10));
      $sheet->setCellValue('B'.($row+10), $namaPejabatKabiroKeuanganUmum);
      $sheet->setCellValue('I'.($row+10), $namaPejabatWarekSdm);
      $sheet->getStyle('A'.($row+10).':K'.($row+10))->applyFromArray(array(
         'alignment' => array(
            'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
            'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER
         )

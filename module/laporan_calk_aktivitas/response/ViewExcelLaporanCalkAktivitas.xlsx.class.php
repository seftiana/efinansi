...
class ViewExcelLaporanCalkAktivitas extends XlsxResponse{
   # Internal Variables
   public $Excel;
   protected $mObj;
   ...
   public function ProcessRequest(){
      ...
      if (empty($aktivitas)) {
         ...
         $sheet->setCellValueByColumnAndRow(0, 2, 'Laporan CALK Aktivitas');
         ...
         $sheet->setCellValueByColumnAndRow(3, 6, 'Akum '.$this->mObj->indonesianMonth[$bulanIni]);
         $sheet->setCellValueByColumnAndRow(4, 6, '%');
         ...
         foreach ($aktivitas as $itemLaporan) {
            ...
            if ($itemLaporan['is_summary'] == 'Y') {
               ...
               $sheet->getStyle('B'.$row.':D'.$row)->getNumberFormat()->setFormatCode(
                  '_("Rp"* #,##0_);_("Rp"* \(#,##0\);_("Rp"* "-"_);_(@_)'
               );
               $sheet->getStyle('E'.$row)->getNumberFormat()->setFormatCode('0.00%');
               
               $sheet->setCellValueByColumnAndRow(1, $row, $jumlahSaldoKlp);
               $sheet->setCellValueByColumnAndRow(2, $row, $jumlahSaldoTransLalu);
               $sheet->setCellValueByColumnAndRow(3, $row, $jumlahSaldoKlpAkm);
               $sheet->setCellValue('E'.$row, "=IF(C$row <> 0,(B$row-C$row)/ABS(C$row),(B$row)/ABS(IF(B$row=0,1,B$row)))");
            } else {
            ...
            if(!empty($dataDetail) && $isShowDet){
               ...
               foreach($dataDetail as $valueDet){
                  ...
                  $sheet->setCellValueByColumnAndRow(2, $row, $transLalu);
                  $sheet->setCellValueByColumnAndRow(3, $row, $akumulasi);
                  $sheet->getStyle('A'.$row.':E'.$row)->applyFromArray(array(
                     'font' => array(
                        'italic' => true
                     )
                  ...
                  $sheet->setCellValue('E'.$row, "=IF($colLalu<>0,($colSekarang-$colLalu)/ABS($colLalu),($colSekarang)/ABS(IF($colSekarang=0,1,$colSekarang)))");
                  $sheet->getStyle('E'.$row)->getNumberFormat()->setFormatCode('0.00%');
                  $sheet->getStyle('B'.$row.':D'.$row)->getNumberFormat()->setFormatCode(
                     '_("Rp"* #,##0_);_("Rp"* \(#,##0\);_("Rp"* "-"_);_(@_)'
                  );

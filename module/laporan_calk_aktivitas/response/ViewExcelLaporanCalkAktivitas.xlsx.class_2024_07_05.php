      ...
      if (empty($aktivitas)) {
         ...
         foreach ($aktivitas as $itemLaporan) {
            ...
            if ($itemLaporan['is_summary'] == 'Y') {
               ...
               $sheet->setCellValue('E'.$row, "=IF(D$row <> 0,(B$row-D$row)/D$row,0)");
            } else {
               ...
               if ($itemLaporan['is_child'] == '0') {
                  ...
                  $dataDetail = $this->mObj->LaporanBuilder()->getLaporanDetail(
                     $tgl_awal, $tgl_akhir, $itemLaporan['id'],$subAccount,$status
                  );
               }
            }
            ...
            if(!empty($dataDetail) && $isShowDet){
               ...
               foreach($dataDetail as $valueDet){
                  ...
                  $sheet->setCellValue('E'.$row, "=IF($colAkm<>0,($colSekarang-$colAkm)/$colAkm,0)");
                  $sheet->getStyle('E'.$row)->getNumberFormat()->setFormatCode('0%');
                  $sheet->getStyle('B'.$row.':D'.$row)->getNumberFormat()->setFormatCode(
                     '_("Rp"* #,##0_);_("Rp"* \(#,##0\);_("Rp"* "-"_);_(@_)'
                  );

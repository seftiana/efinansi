<?php
          ...
          (SELECT
           ...
           (IFNULL(
            ...
            FROM 
                ...
                FROM kegiatan_adjust) AS data_approve_tambah_adjust 
            WHERE pengrealKegdetId=kd.kegdetId),0)) AS realisasi_pencairan,
           (IFNULL((
            SELECT SUM(pengrealNominal) 
            FROM pengajuan_realisasi 
            WHERE pengrealKegdetId=kd.kegdetId AND 
            IFNULL(pengrealIsApprove,'Tidak') = 'Tidak'),0)) AS realisasi_nominal,
          programId,
          programNomor,
          programNama,
          kegUnitkerjaId,
    ...
    WHERE
        (uk.unitkerjaKode LIKE '%s' OR uk.unitkerjaNama LIKE '%s') AND
...
SELECT 
   ...
   (rpeng.rncnpengeluaranKomponenTotalAprove  - IFNULL(pencairan.nominal,0))AS nilai, 
   rpeng.rncnpengeluaranKegdetId 
FROM
  rencana_pengeluaran rpeng 
  LEFT JOIN (
  SELECT
   ...
   SUM(peng_real_det.`pengrealdetNominalApprove`) AS nominal
...
WHERE peng_real.`pengrealIsApprove` = 'Ya'

...
class AppLapBukubesar extends Database {
    ...
    public function getData($param = array()) {
        ...
        $return = $this->Open($this->mSqlQueries['get_data_buku_besar_sementara'] . $this->mSqlQueries['get_limit'], array(
            ...
            '%'.$param['sub_account'].'%',
            $param['coa_id'],
            (int) (($param['coa_id'] == '' || $param['coa_id'] === null) || strtolower($param['coa_id']) == 'all'),
            $param['page'],
            $param['limit']
        ));
    ...
    public function getTotalSaldo($param = array()) {
        ...
        for ($k = 0; $k < $max;) {
            ...
            if ($kodeAkun == $dataList[$k]['akun_kode']) {
                ...
                $totalSaldoAkun[$dataList[$k]['akun_kode']] = $saldoAkhir;
                $k++;
        ...
        //return self::ChangeKeyName($return);
         return array( 'data_saldo' => $dataSaldo,'total_saldo' => $totalSaldoAkun);
    }
    ...
    public function GetBukuBesarHis($rekening, $tgl_awal, $tgl_akhir, $sub_akun) {
        $result = $this->open($this->mSqlQueries['get_data_buku_besar_sementara'], array(
            ...
            '%'.$sub_akun.'%',
            $rekening,
            (int) (($rekening == '' || $rekening === null) || strtolower($rekening) == 'all')
        ));

  ...
  FROM(
    ...
    from
      ...
      join (
        ...
        group by pdCoaId,coaSubAkun
      ) tmp_sub on coaSubId = c.coaId AND ( coaSubAkun LIKE '%s')
      where
    ...
    group by c.coaId
      ...
      from
        ...
        bbCoaId AS coaSubId,
          ...
          from buku_besar_his
          group by bbCoaId,coaSubAkun
        ) tmp_sub on coaSubId = c.coaId AND ( coaSubAkun LIKE '%s')
        where ( (transTanggalEntri BETWEEN '%s' 
      AND '%s') OR (bbPembukuanRefId IS NULL AND bbPdId IS NULL) ) 
      group by c.coaId
      )) coa
    group by coaId
) coa
left join	(
	...
	left join (
      ...
      CONCAT_WS(
		...
		where
            transTanggalEntri BETWEEN '%s' AND '%s' and prIsPosting = 'T'

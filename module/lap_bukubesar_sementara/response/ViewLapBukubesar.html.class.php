...
class ViewLapBukubesar extends HtmlResponse {
    ...
    function TemplateModule() {
        $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application', 'docroot') . 'module/lap_bukubesar_sementara/template');
        $this->SetTemplateFile('view_lap_bukubesar.html');
    }

    function ProcessRequest() {
        $mObj = new AppLapBukubesar();
        $arrSubAkun = $mObj->getSubAccountCombo();
        $requestData = array();
        $queryString = '';
        $getdate = getdate();
        $currmon = $getdate['mon'];
        ...
        if (isset($mObj->_POST['btncari'])) {
            ...
            $requestData['sub_account'] = $mObj->_POST['sub_account'];
            ...
            $requestData['sub_account'] = Dispatcher::Instance()->Decrypt($mObj->_GET['sub_account']);
            ...
            $requestData['sub_account'] = '01-00-00-00-00-00-00';
        ...
        $totalSaldo = $mObj->getTotalSaldo((array) $requestData);
    ...
    function ParseTemplate($data = NULL) {
        $dataSaldo = $data['saldo']['data_saldo'];
        $totalSaldo = $data['saldo']['total_saldo'];
        $requestData = $data['request_data'];
        $queryString = $data['query_string'];
        $dataList = $data['data_list'];
        $start = $data['start'];
        $urlSearch = Dispatcher::Instance()->GetUrl(
                'lap_bukubesar_sementara', 'LapBukubesar', 'view', 'html'
        );
        $this->mrTemplate->AddVar('content', 'URL_SEARCH', $urlSearch);
        $this->mrTemplate->AddVars('content', $requestData);

        $this->mrTemplate->AddVar('content', 'URL_CETAK', Dispatcher::Instance()->GetUrl(
                        'lap_bukubesar_sementara', 'CetakLapBukubesar', 'view', 'html'
                ) .
        ...
        $this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl(
                        'lap_bukubesar_sementara', 'ExcelLapBukubesar', 'view', 'xlsx'
                ) .
                '&rekening=' . Dispatcher::Instance()->Encrypt($requestData['coa_id']) .
                '&tgl_awal=' . Dispatcher::Instance()->Encrypt($requestData['start_date']) .
                '&tgl_akhir=' . Dispatcher::Instance()->Encrypt($requestData['end_date']) .
        ...
        $this->mrTemplate->AddVar('content', 'URL_POPUP_COA', Dispatcher::Instance()->GetUrl('lap_bukubesar_sementara', 'popupCoa', 'view', 'html'));
        ...
        if (empty($dataList)) {
            ...
            $totalSaldoJumlah = 0;
            if(!empty($totalSaldo)) {
                foreach($totalSaldo as   $saldoAkun) {
                    $totalSaldoJumlah += $saldoAkun; 
                }
            }
            if($totalSaldoJumlah > 1){
                $totalSaldoF =  number_format($totalSaldoJumlah, 2, ',', '.');
            } else {
                $totalSaldoF = '('. number_format($totalSaldoJumlah * (-1), 2, ',', '.').')';
            }
            
            $this->mrTemplate->AddVar('data_grid', 'TOTAL_SALDO', $totalSaldoF);
        }
        /**
          if (empty($data['bbhis'])) {

...
class ViewExcelLapBukubesar extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $dataList      = $Obj->GetBukuBesarHis($rekening, $tgl_awal, $tgl_akhir, $sub_account);
      ...
      $this->SetFileName('laporan_buku_besar_sementara.xls');
      ...
      if (empty($dataList)) {
         ...
         $sheet->setCellValueByColumnAndRow(0, 1, 'Laporan Buku Besar Sementara');
         $sheet->mergeCells('A1:I1');
         $sheet->setCellValueByColumnAndRow(0, 3, 'Tanggal Transaksi : '. IndonesianDate($tgl_awal, 'yyyy-mm-dd') .' s/d '. IndonesianDate($tgl_akhir, 'yyyy-mm-dd'));
         ...
         for ($i=0;$i<sizeof($data);$i++) {
            $sheet->setCellValueByColumnAndRow(0, $num, IndonesianDate($data[$i]['tanggal_posting'],'yyyy-mm-dd'), $fColNomor);
         ...
         for($k = 0; $k < $max;){
            ...
            if($kodeAkun == $dataList[$k]['akun_kode']) {     
               ...
               switch ($dataList[$k]['kelompok_id']) {
                  ...
                  case 3://modal
                  case 4://pendapatan
                     $sheet->setCellValueByColumnAndRow(5, $num,'=(F'.($num - 1 ).'+E'.$num.'-D'.$num.')');
                     break;
                  default ://default
                       $sheet->setCellValueByColumnAndRow(5, $num,'=(F'.($num - 1 ).'+D'.$num.'-E'.$num.')');
                     break;
                ...
                if($kodeAkun != $cek){
                    ...
                    $sheet->getStyle('D'.$num.':F'.$num)->getNumberFormat()->setFormatCode('_("Rp "* #,##0_);_("Rp "* \(#,##0\);_("Rp "* "-"_);_(@_)');
                    $sheet->getStyle('A'.$num.':F'.$num)->getFont()->setBold(true);
                    $num++;
                }
                $k++;

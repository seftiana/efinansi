...
/**
* ================= doc ====================
* FILENAME     : LaporanHarianBank.class.php
* @package     : LaporanHarianBank
* scope        : PUBLIC
* @Author      : Eko Susilo
* @Created     : 2015-05-26
* @Modified    : 2015-05-26
...
class LaporanHarianBank extends Database
{
   # internal variables
   protected $mSqlFile;
   public $_POST;
   ...
   {
      $this->mSqlFile   = 'module/laporan_harian_bank/business/laporan_harian_bank.sql.php';
   ...
   {
      if($param['nama_bank'] != ''){
        $filter_bank  = "AND (transaksiBankPenerima  LIKE '".'%'.$param['nama_bank'].'%'."'OR transaksiBankTujuan LIKE '".'%'.$param['nama_bank'].'%'."')";
      }else{
        $filter_bank  = '';
      }
      ...
      return self::ChangeKeyName($data);
   }

   public function getDataExcel($param = array())
   {
      if($param['nama_bank'] != ''){
        $return     = $this->Open($this->mSqlQueries['get_data_laporan_excel_filter_bank'], array(
                      date('Y-m-d', strtotime($param['start_date'])),
                      date('Y-m-d', strtotime($param['end_date'])),
                      '%'.$param['nama_bank'].'%',
                      '%'.$param['nama_bank'].'%',
        ...
        $return     = $this->Open($this->mSqlQueries['get_data_laporan_excel'], array(
                      date('Y-m-d', strtotime($param['start_date'])),
                      date('Y-m-d', strtotime($param['end_date'])),
                      '%'.$param['nomor_bukti'].'%'
        ));

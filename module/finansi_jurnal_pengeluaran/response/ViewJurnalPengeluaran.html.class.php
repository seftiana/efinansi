...
/**
* ================= doc ====================
* FILENAME     : ViewJurnalPengeluaran.html.class.php
* @package     : ViewJurnalPengeluaran
* scope        : PUBLIC
* @Author      : Eko Susilo
* @Created     : 2015-04-13
* @Modified    : 2015-04-13
...
class ViewJurnalPengeluaran extends HtmlResponse
{
   ...
   function ProcessRequest(){
      ...
      $data             = $mObj->getDataJurnalPengeluaran($offset, $limit, (array)$requestData);
         ...
         foreach ($dataJurnal as $grid) {
            foreach ($grid as $jurnal) {
               if($jurnal['type'] AND strtoupper($jurnal['type']) == 'PARENT'){
                  ...
                  // url delete
                  $urlAccept              = 'finansi_jurnal_pengeluaran|DeleteJurnalPengeluaran|do|json-search|'.$keyUrl.'-1|'.$valueUrl;
                  $urlReturn              = 'finansi_jurnal_pengeluaran|JurnalPengeluaran|view|html-search|'.$keyUrl.'-1|'.$valueUrl;
                  $label                  = GTFWConfiguration::GetValue('language', 'jurnal_pengeluaran');

...
/**
* ================= doc ====================
* FILENAME     : FinansiJurnalPengeluaran.php
* @package     : FinansiJurnalPengeluaran
* scope        : PUBLIC
* @Author      : Eko Susilo
* @Created     : 2015-04-13
* @Modified    : 2015-04-13
...
class FinansiJurnalPengeluaran
{
   # internal variables
   ...
   {
      $this->mObj       = new JurnalPengeluaran($connectionNumber);
   ...
   private function setData()
   {
      ...
      if($this->mObj->method == 'post'){
         ...
         $getDate          = getdate();
         $tanggalDay       = (int)$getDate['mday'];
         $tanggalMon       = (int)$getDate['mon'];
         $tanggalYear      = (int)$getDate['year'];
         ...
         if($this->mObj->_POST['debet'] && !empty($this->mObj->_POST['debet'])){
            ...
            foreach ($this->mObj->_POST['debet'] as $debet) {
               ...
               $requestData['akun_debet'][$index]['referensi']    = $this->mObj->_POST['referensi_nama'];
         ...
         if($this->mObj->_POST['kredit'] && !empty($this->mObj->_POST['kredit'])){
            ...
            foreach ($this->mObj->_POST['kredit'] as $kredit) {
               ...
               $requestData['akun_kredit'][$index]['referensi']   = $this->mObj->_POST['referensi_nama'];
   ...
   public function Update()
   {
      ...
      if($checkData['result'] === false){
         ...
         $process             = $this->mObj->doUpdateJurnal($this->mData);
         // $process       = false;
         if($process === true){
            $return['url']       = $this->urlReturn;
   ...
   public function Delete()
   {
      ...
      if($this->mObj->method != 'post'){
         ...
         $process             = $this->mObj->doDeleteJurnal($pembukuanId, $transaksiId);
         if($process === true){

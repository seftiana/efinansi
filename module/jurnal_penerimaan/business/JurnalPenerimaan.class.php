...
/**
* ================= doc ====================
* FILENAME     : ProcJurnalPenerimaan.php
* @package     : ProcJurnalPenerimaan
* scope        : PUBLIC
* @Author      : Eko Susilo
* @modified    : noor hadi
* @Created     : 2015-04-13
...
class JurnalPenerimaan extends Database
{
   ...
   {
      $this->mSqlFile   = 'module/jurnal_penerimaan/business/jurnalpenerimaan.sql.php';
   ...
   public function getDataJurnalPenerimaan($offset, $limit, $param = array())
   {  //$this->SetDebugOn();
      $result     = $this->Open($this->mSqlQueries['get_data_jurnal_penerimaan'], array(
         '%'.$param['referensi'].'%',
         $param['posting'],
      ...
      $rCount     = $this->Open($this->mSqlQueries['count_jurnal_penerimaan'], array(
         '%'.$param['referensi'].'%',
         $param['posting'],
         (int)($param['posting'] == '' OR strtolower($param['posting']) == 'all'),
         date('Y-m-d', strtotime($param['start_date'])),
   ...
   public function doJurnalBalik($transaksiId, $pembukuanId)
   {
      ...
      $result        &= $this->Execute($this->mSqlQueries['do_add_log'], array(
         $userId,
         $ipAddress,
         'Jurnal Balik - Jurnal Penerimaan'
      ));
      $loggerId      = $this->LastInsertId();
      if(is_array($queryLog)){
         foreach ($queryLog as $query) {

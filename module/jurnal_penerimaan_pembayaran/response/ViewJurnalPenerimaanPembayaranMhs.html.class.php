...
'module/jurnal_penerimaan_pembayaran/business/JurnalPenerimaan.class.php';
...
class ViewJurnalPenerimaan extends HtmlResponse
{
   ...
   function ProcessRequest(){
      ...
      if(isset($mObj->_POST['btnSearch'])){
         ...
         $requestData['sub_account'] = strtoupper($mObj->_POST['sub_account']);
         ...
         $requestData['sub_account'] = strtoupper(Dispatcher::Instance()->Decrypt($mObj->_GET['sub_account']));
      }else{
      ...
      if(empty($dataList)){
         ...
         for ($i=0; $i < count($dataList);) {
            if((int)$transaksiId === (int)$dataList[$i]['id']
               ...
               $dataJurnal[$kodeSistem][$idx]['jenis_id']         = $dataList[$i]['jenis_id'];
         ...
         foreach ($dataJurnal as $grid) {
            foreach ($grid as $jurnal) {
               if($jurnal['type'] AND strtoupper($jurnal['type']) == 'PARENT'){
                  ...
                  // url delete
                  $urlAccept              = 'jurnal_penerimaan_pembayaran|DeleteJurnalPenerimaan|do|json-search|'.$keyUrl.'-1|'.$valueUrl;
                  $urlReturn              = 'jurnal_penerimaan_pembayaran|JurnalPenerimaan|view|html-search|'.$keyUrl.'-1|'.$valueUrl;
                  $label                  = GTFWConfiguration::GetValue('language', 'jurnal_penerimaan_pembayaran');
                  ...
                  if((int)$jurnal['jurnal'] > 1){ 
                     ...
                     $this->mrTemplate->SetAttribute('history_jurnal', 'visibility', 'hidden');
                  }
                  ...
                  // Crud Dilakukan Di menu Transaksi Penerimaan BANK
                  if($jurnal['jenis_id'] == '8'){
                     $this->mrTemplate->AddVar('content_links', 'DISPLAY_EDIT', 'display: none;');
                     $this->mrTemplate->AddVar('content_links', 'DISPLAY_DELETE', 'display: none;');
                     $this->mrTemplate->AddVar('content_links', 'DISPLAY_JURNAL_BALIK', 'display: none;');
                  }

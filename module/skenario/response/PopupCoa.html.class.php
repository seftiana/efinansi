...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/skenario/business/AppPopupCoa.class.php';
...
class PopupCoa extends HtmlResponse {
   ...
   function ProcessRequest() {	  
      if(isset($_GET['tipe']))	  
	    ...
	    echo 'kegagalan memanggil popup coa silahkan ulangi!';
		exit;
	  }
      ...
      $url = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, 
               ...
               Dispatcher::Instance()->mType).'&tipe='.$return['tipe'].'&data[nama]='.$this->data['nama'];
   ...
   function ParseTemplate($data = NULL) {      
      ...
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('skenario', 'coa', 'popup', 'html').'&tipe='.$data['tipe'] );      
      ...
      if (empty($data['dataGrid'])) {	     
         ...
         for ($i=0; $i<sizeof($dataGrid); $i++) {              			   
			   ...
			   if ($dataGrid[$i]['no'] % 2 != 0) $dataGrid[$i]['class_name'] = 'table-common-even';
			   else $dataGrid[$i]['class_name'] = '';
               
               //if(!$dataGrid[$i]['isParent']) {
                  $dataGrid[$i]['set_parent'] ='<a href="javascript:setParent(\''.$dataGrid[$i]['id'].'\',\''.$dataGrid[$i]['kode'].'\',\''.$dataGrid[$i]['nama'].'\')" onmouseover="status=\'Set preferences...\';return true" ><img src="images/button-check.gif" alt="Pilih"/>PILIH</a>';			   

<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_realisasi_penerimaan/business/AppPopupUnitkerja.class.php';
...
class ViewPopupUnitkerja extends HtmlResponse {
	...
	//view
		$totalData = $this->unitkerjaObj->GetCountDataUnitkerja($kode, $unitkerja, $tipeunit, $role, $unitkerjaUserId);
		$itemViewed = 20;
		$currPage = 1;
		$startRec = 0 ;
		if(isset($_GET['page'])) {
		...
		$dataUnitkerja = $this->unitkerjaObj->getDataUnitkerja($startRec, $itemViewed, $kode, $unitkerja, $tipeunit, $role, $unitkerjaUserId);
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_realisasi_penerimaan', 'popupUnitkerja', 'view', 'html') . "&satker=" . Dispatcher::Instance()->Encrypt($search['satker']));
		if($this->Pesan) {

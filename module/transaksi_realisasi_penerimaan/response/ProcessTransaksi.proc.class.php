...
require_once GTFWConfiguration::GetValue('application', 'docroot') .
        'module/transaksi_realisasi_penerimaan/business/AppTransaksi.class.php';
require_once GTFWConfiguration::GetValue('application', 'docroot') .
...
class ProcessTransaksi {
    ...
    function __construct() {
        ...
        $this->pageView = Dispatcher::Instance()->GetUrl(
                'transaksi_realisasi_penerimaan', 'Transaksi', 'view', 'html');

        $this->pageViewDetil = Dispatcher::Instance()->GetUrl(
                'transaksi_realisasi_penerimaan', 'transaksiDetil', 'view', 'html');

        $this->pageDetil = Dispatcher::Instance()->GetUrl(
                'history_transaksi', 'HTRealisasiPenerimaan', 'view', 'html');
    }
    ...
    function Check() {
        ...
        if (isset($_POST['btnsimpan'])) {
            ...
            if($this->_POST['nominal_disetujui_hidden'] < $this->_POST['sisa']) {
                return "lebih";
            }
            */

            $tanggal =  $this->_POST['tanggal_transaksi_year'] . "-" .
            $this->_POST['tanggal_transaksi_mon'] . "-" .
            $this->_POST['tanggal_transaksi_day'];

            $tahunPembukuan = $this->Obj->getRangeTanggalPembukuan();
            if($tanggal < $tahunPembukuan['awal'] || $tanggal > $tahunPembukuan['akhir']){
                return  array(
                    "err"=>true,
                    "msg"=>"Tanggal transaksi tidak boleh di luar rentang tanggal tahun pembukuan yang aktif. Rentang tanggal pembukuan aktif <b>".IndonesianDate($tahunPembukuan['awal'], 'YYYY-MM-DD').' - '.IndonesianDate($tahunPembukuan['akhir'], 'YYYY-MM-DD').'</b>'
                );
            }
    ...
    function Add() {
        ...
        $cek = $this->Check();
        if ($cek === true) {
            //if(empty($this->_POST['no_invoice_list'])) {
            ...
            //file_put_contents('C:/asdf.txt', print_r($arrData['renterimaId'],1).'<br>');
            ...
            if ($id_transaksi != "") {
                ...
                /* if($this->_POST['mak'] != "") {
                  //jika pencairan anggaran insert ke transaksi_detail_pencairan
                  //if($this->_POST['tipe_transaksi'] == 5 AND $this->_POST['unitkerja'] != 1)
                  //$pencairan = $this->Obj->DoAddTransaksiDetilPengembalianAnggaran($id_transaksi, $this->_POST['mak']);
                  //elseif($this->_POST['tipe_transaksi'] == 4 AND $this->_POST['unitkerja'] == 1) {
                  $pencairan = $this->Obj->DoAddTransaksiDetilPencairan($id_transaksi, $this->_POST['mak']);
                  //}else{
                  //$satu = $this->Obj->DoAddTransaksiDetilAnggaran($id_transaksi, $this->_POST['mak']);
                  //if(!$satu)
                  //$this->Obj->DoAddTransaksiDetilAnggaranPenerimaan($id_transaksi, $this->_POST['mak']);
                  }
                  } */
                ...
                if ($this->_POST['skenario'] == "auto") {
                    ...
                    $detil_pembukuan = $this->Obj->DoAddPembukuanDetil($id_pembukuan, $this->_POST['sisa'], $this->_POST['skenario_list']);
                }
                ...
                $rel_penerimaan = $this->Obj->DoAddRealisasiPerencanaan(
                        $this->_POST['sisa'], $arrData['transCatatan'], $arrData['renterimaId'], $bln['01'], $bln['02'], $bln['03'], $bln['04'], $bln['05'], $bln['06'], $bln['07'], $bln['08'], $bln['09'], $bln['10'], $bln['11'], $bln['12'], $id_transaksi);

                Messenger::Instance()->Send(
                        'transaksi_realisasi_penerimaan', 'transaksiDetil', 'view', 'html', array(
                    $this->_POST,
                    'Penambahan Data Transaksi Berhasil Dilakukan',
                    $this->cssDone), Messenger::NextRequest);
                return $this->pageViewDetil . '&dataId=' . $this->Obj->GetLastInsertTransId();
            }
        } elseif ($cek === "exists") {
            //echo "666";
            Messenger::Instance()->Send(
                    'transaksi_realisasi_penerimaan', 'Transaksi', 'view', 'html', array(
                $this->_POST,
        ...
        /* elseif ($cek === "lebih") {
            ...
            Messenger::Instance()->Send(
                    'transaksi_realisasi_penerimaan', 'Transaksi', 'view', 'html', array(
                $this->_POST,
                'Nominal Melebihi Sisa Realisasi',
                $this->cssFail), Messenger::NextRequest);
        }*/ else {
            //gagal masukin data
            Messenger::Instance()->Send(
        ...
        if ($cek === true) {
            ...
            if ($upd_transaksi == true) {
                ...
                $updaterel_penerimaan = $this->Obj->DoUpdateRealisasiPerencanaan(
                        $this->_POST['sisa'], $arrData['transCatatan'], $arrData['renterimaId'], $bln['01'], $bln['02'], $bln['03'], $bln['04'], $bln['05'], $bln['06'], $bln['07'], $bln['08'], $bln['09'], $bln['10'], $bln['11'], $bln['12'], $arrData['transId']);
                ...
                if ($this->_POST['skenario'] == "auto") {
                    ...
                    $detil_pembukuan = $this->Obj->DoAddPembukuanDetil(
                            $id_pembukuan, $this->_POST['sisa'], $this->_POST['skenario_list']);
                }
            ...
            Messenger::Instance()->Send(
                ...
                $this->_POST,
                'Transaksi Dengan Nomor <b>' . ($this->_POST['no_kkb']) .
                '</b> Sudah Dibuat', $this->cssFail), Messenger::NextRequest);
            return $this->pageView . '&dataId=' . $this->encId;
        } else {
            //gagal masukin data
            //echo "gagal";
            Messenger::Instance()->Send('transaksi_realisasi_penerimaan', 'Transaksi', 'view', 'html', array($this->_POST, 'Gagal Merubah Data Transaksi', $this->cssFail), Messenger::NextRequest);
            return $this->pageView . '&dataId=' . $this->encId;
        }
        return $this->pageView;
    }
    ...
    function Delete() {
        ...
        if ($deleteArrData === true) {
            Messenger::Instance()->Send(
                    'history_transaksi', 'HTRealisasiPenerimaan', 'view', 'html', array(
                $this->_POST,

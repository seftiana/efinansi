...
class ViewHTFormRealisasiPencairanDetil extends HtmlResponse {
    ...
    function ProcessRequest() {
        ...
        if (isset($_GET['dataId'])) {
            ...
            $return['dataKomponenAnggaran'] = $Obj->getKomponenAnggaranByTransId($datadb['id']);
    ...
    function ParseTemplate($data = NULL) {
        ...
        $this->mrTemplate->AddVar('content', 'NOMINAL_DISETUJUI', number_format($datadb['nominal_disetujui'], 2, ',', '.')
        );
        ...
        $dataKomponenAnggaran = $data['dataKomponenAnggaran'];
        if (empty($dataKomponenAnggaran)) {
            $this->mrTemplate->AddVar('data_komponen_anggaran', 'DATA_EMPTY', 'YES');
        } else {
            $this->mrTemplate->AddVar('data_komponen_anggaran', 'DATA_EMPTY', 'NO');

            foreach ($dataKomponenAnggaran as $v) {
                $v['nominal_f'] = number_format($v['nominal'], 0, ',', '.');
                $this->mrTemplate->AddVars('data_komponen_anggaran_list', $v, 'DATA_');
                $this->mrTemplate->parseTemplate('data_komponen_anggaran_list', 'a');
            }
        }

...
class DoCetakBkk extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $sheet->setCellValue('G1', 'KODE DOKUMEN : F-KUA-001.05'); #
	  $sheet->setCellValue('G2', 'BPKB'); #
      $sheet->getStyle('H2')->getNumberFormat()->setFormatCode(': _(@_)'); #
      $sheet->setCellValueExplicit('H2', $dataList['nomor_referensi']); #
      $sheet->setCellValue('G3', 'TGL'); #
      $sheet->getStyle('H3')->getNumberFormat()->setFormatCode(': _(@_)'); #
      $sheet->setCellValue('H3', date('d-m-Y', strtotime($dataList['tanggal_entri']))); #
		#
	  ...
	  $sheet->getStyle('C7'.$row.':I7'.$row)->getAlignment()->setWrapText(TRUE);

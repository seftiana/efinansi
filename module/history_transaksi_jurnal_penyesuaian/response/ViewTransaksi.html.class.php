<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_jurnal_penyesuaian/business/AppTransaksi.class.php';
...
class ViewTransaksi extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot') . 
			'module/transaksi_jurnal_penyesuaian/template');
	...
	function ProcessRequest() {
      ...
      $arr_jenis_transaksi = $Obj->GetComboJenisTransaksi();
		Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'jenis_transaksi', array('jenis_transaksi', $arr_jenis_transaksi, 1, "false", ' style="width:200px;" id="jenis_transaksi"'), Messenger::CurrentRequest);
		/**
      ...
      $arr_tipe_transaksi = $Obj->GetComboTipeTransaksi();
		Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'tipe_transaksi', array('tipe_transaksi', $arr_tipe_transaksi, $this->Data['tipe_transaksi'], "false", ' style="width:200px;" onChange="jsCheck(this.value)" id="tipe_transaksi"'), Messenger::CurrentRequest);
		*/
		$arr_tipe_transaksi = $Obj->GetComboTipeTransaksi();
		Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'tipe_transaksi', array('tipe_transaksi', $arr_tipe_transaksi, $this->Data['tipe_transaksi'], "false", ' style="width:200px;"  id="tipe_transaksi"'), Messenger::CurrentRequest);
	...
	function ParseTemplate($data = NULL) {
      $url_view = Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'transaksi', 'view', 'html');
      $this->mrTemplate->AddVar('content', 'URL_CHECK', $url_view);
		$this->mrTemplate->AddVar('content', 'URL_DETIL', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'detilTransaksi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_POPUP_SKENARIO', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'popupSkenario', 'view', 'html'));
	   $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'popupKegiatanDetil', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK_PENERIMAAN', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'PopupMakPenerimaan', 'view', 'html')); 
      $this->mrTemplate->AddVar('content', 'URL_POPUP_KEGIATAN_UNIT_KERJA', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'popupKegiatanUnitKerja', 'view', 'html')); 
	   $this->mrTemplate->AddVar('content', 'URL_POPUP_MAK', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'popupKegiatanDetil', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_SISA_ANGGARAN', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'popupSisaAnggaran', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_POPUP_KODE_JURNAL', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'popupKodeJurnal', 'view', 'html'));
      $datadb = $data['datadb'];
...
//print_r($this->Data);
      //jika edit, lihat jenis transaksi, jika jenis=umum, set mak == disabled
		...
		if($data['role_name'] == "Administrator") {
			...
			$this->mrTemplate->AddVar('role', 'URL_POPUP_UNITKERJA', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'popupUnitkerja', 'view', 'html'));
      ...
      if(isset($_GET['dataId']) AND $_GET['dataId']->Integer()->Raw() > 0) {
         ...
         $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'updateTransaksi', 'do', 'html') . '&dataId=' . Dispatcher::Instance()->Encrypt($_GET['dataId']));
         if($this->Data['jenis_transaksi'] != "1") {
         ...
         $this->mrTemplate->AddVar('content', "MODE_20080109", 'Tambah');
		   $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal', 'addTransaksi', 'do', 'html'));
      }

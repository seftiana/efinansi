<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/realisasi_pencairan_2/business/AppPopupProgram.class.php';
...
class PopupProgram extends HtmlResponse {
	...
	function ProcessRequest() {
		...
		//$unit_id = Dispatcher::Instance()->Decrypt($_GET['grp']);
		$unit_id = $_GET['grp'];
		...
		$totalData = $popupProgramObj->GetCountDataProgram($tahun_anggaran, $program, $kode,$unit_id);
		...
		$dataProgram = $popupProgramObj->getDataProgram($startRec, $itemViewed, $tahun_anggaran, $program, $kode,$unit_id);
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('realisasi_pencairan_2', 'program', 'popup', 'html') . '&tahun_anggaran=' . Dispatcher::Instance()->Encrypt($search['tahun_anggaran']) . '&tahun_anggaran_label=' . Dispatcher::Instance()->Encrypt($search['tahun_anggaran_label']) . '&grp='.$_GET['grp']);
		if($this->Pesan) {
		...
		if (empty($data['dataProgram'])) {
			...
			/*for($i=0;$i<sizeof($dataProgram);$i++) {
				...
				$dataProgram[$i]['kegiatanunit_id'] = Dispatcher::Instance()->Encrypt($dataProgram[$i]['kegiatanunit_id']);

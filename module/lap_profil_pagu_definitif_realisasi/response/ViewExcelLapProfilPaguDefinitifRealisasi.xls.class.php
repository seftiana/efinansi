...
/**
 * 
 * class ViewExceklLapProfilPaguDefinitifRealisasi
...
class ViewExcelLapProfilPaguDefinitifRealisasi extends XlsResponse 
{
    ...
    function GetFileName() 
    {
     ...
     $label = 'lap_profil_pagu_definitif_realisasi';
    ...
    function ProcessRequest() 
    {
		...
		if (empty($listDataItem)) {
		   ...
		   $this->mWorksheets['Data']->write(0, 0, $this->L('lap_profil_pagu_definitif_realisasi'), $fTitle);
           ...
           $this->mWorksheets['Data']->write($num, 1,  $this->L('uraian') . ' (unit kerja/kegiatan/output/komponen)', $formatHeader);
           $this->mWorksheets['Data']->set_column(2,2,20);
           $this->mWorksheets['Data']->write($num, 2, 'Total Pagu Definitif', $formatHeader);
           $this->mWorksheets['Data']->set_column(3,3,20);
		   $this->mWorksheets['Data']->write($num, 3, 'Total Realisasi' , $formatHeader);
		   ...
		   $num=7;
			...
			for($i = 0;$i < sizeof($listDataItem);)
			{
				if( ($listDataItem[$i]['unit_kerja_id'] == $unitId) && 
					...
					($listDataItem[$i]['output_id'] == $outputId) 
						...
						$send[$i]['total_pagu_def']= $listDataItem[$i]['total_pagu_def'];
						$send[$i]['total_realisasi']=  $listDataItem[$i]['total_realisasi'];
												
						$this->mWorksheets['Data']->write($num, 0, $send[$i]['kode'],$formatR);
						$this->mWorksheets['Data']->write_string($num, 1, $send[$i]['uraian'],$format);
						$this->mWorksheets['Data']->write($num, 2, $send[$i]['total_pagu_def'],$formatCurrency);
						$this->mWorksheets['Data']->write($num, 3, $send[$i]['total_realisasi'],$formatCurrency);
					...
					$i++;	
						...
						$send[$i]['total_pagu_def'] = $this->mDBObj->GetTotalPerUnit(
														$listDataItem[$i]['tahun_anggaran_id'],
														$listDataItem[$i]['unit_kerja_id'] 
														);
						$send[$i]['total_realisasi']= 0;
						...
						$this->mWorksheets['Data']->write($num, 2, $send[$i]['total_pagu_def'],$formatCurrencyProgram);
						$this->mWorksheets['Data']->write($num, 3, $send[$i]['total_realisasi'],$formatCurrencyProgram);
						...
						$send[$i]['total_pagu_def'] =  $this->mDBObj->GetTotalPerKegiatan(
														$listDataItem[$i]['tahun_anggaran_id'],
						...
						$this->mWorksheets['Data']->write($num, 2, $send[$i]['total_pagu_def'],$formatCurrencyKegiatan);
						$this->mWorksheets['Data']->write($num, 3, $send[$i]['total_realisasi'],$formatCurrencyKegiatan);
						...
						$send[$i]['total_pagu_def'] = $this->mDBObj->GetTotalPerOutput(
														$listDataItem[$i]['tahun_anggaran_id'],
						...
						$this->mWorksheets['Data']->write($num, 2, $send[$i]['total_pagu_def'],$formatCurrencySubKegiatan);
						$this->mWorksheets['Data']->write($num, 3, $send[$i]['total_realisasi'],$formatCurrencySubKegiatan);
				}			
			...
			$this->mWorksheets['Data']->write($num, 2, $totalAll,$formatCurrencyProgram);
			$this->mWorksheets['Data']->write($num, 3, 0,$formatCurrencyProgram);
			$this->mWorksheets['Data']->merge_cells($num,0,$num,1);

<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_kode_jurnal_penerimaan/business/AppPopupKegiatanDetil.class.php';

class ViewPopupSisaAnggaran extends HtmlResponse {
	...
	function TemplateModule() {
		...
		$this->SetTemplateFile('view_popup_sisa_anggaran.html');
	}
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'popupKegiatanDetil', 'view', 'html') . '&unitkerja=' . $this->encUnitkerjaId);
		if($this->Pesan) {
		...
		if (empty($data['data_kegiatan_detil'])) {
            ...
            $data_kegiatan_detil[$i]['nominal_aprove_hidden'] = ($data_kegiatan_detil[$i]['nominal_aprove'] - $data_kegiatan_detil[$i]['nominal_yg_sudah_dicairkan']);   
            ...
            $data_kegiatan_detil[$i]['nominal_yg_bisa_dicairkan'] = number_format($data_kegiatan_detil[$i]['nominal_aprove_hidden'], 2, ',', '.');

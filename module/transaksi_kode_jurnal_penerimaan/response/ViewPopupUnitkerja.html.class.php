<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/transaksi_kode_jurnal_penerimaan/business/AppPopupUnitkerja.class.php';
...
class ViewPopupUnitkerja extends HtmlResponse {
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('transaksi_kode_jurnal_penerimaan', 'popupUnitkerja', 'view', 'html') . "&satker=" . Dispatcher::Instance()->Encrypt($search['satker']));
		if($this->Pesan) {
         ...
         */
         //print_r($dataUnitkerja);
			for ($i=0; $i<sizeof($dataUnitkerja); $i++) {
               ...
               $dataUnitkerja[$i]['label_unit'] = $dataUnitkerja[$i]['unit'];
				} else {
               $dataUnitkerja[$i]['label_unit'] = $dataUnitkerja[$i]['satker'] . " / " . $dataUnitkerja[$i]['unit'];
            }

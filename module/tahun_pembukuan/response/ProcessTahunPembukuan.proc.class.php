...
class ProsessTahunPembukuan
{
	...
	function BukaBuku() {
		...
		$tppRolledback = $TahunPembukuanPeriode->GetTahunPembukuanIsRolledback();
		if(!empty($tppRolledback)){
			$idRolledback = $tppRolledback[0]['tppId'];
			$TahunPembukuanPeriode->UpdatTransaksiKeTppBaru($id_tahun_aktif, $idRolledback);
        }
	}
	...
	function tutupBukuLabaRugi($id_tahun_aktif,$tanggal_akhir){
		...
		foreach ($labaRugiTahunBerjalan as $key => $value){
			$subAcc = $key;

			$totalRL = $value['saldo'];//nominal LR
			$coaLRBerjalanId = $value['coa_id'];

			if(isset($labaRugiAwalTahunSa[$subAcc])){
				$totalSaldoAwal = $labaRugiAwalTahunSa[$subAcc]['saldo_akhir'];
				$coaLRAwalTahunId = $labaRugiAwalTahunSa[$subAcc]['coa_id'];
			}else{
				$totalSaldoAwal = $labaRugiAwalTahunSa['default']['saldo_akhir'];
				$coaLRAwalTahunId = $labaRugiAwalTahunSa['default']['coa_id'];
			}
			...
			// echo "<pre>"; 
			// print_r($params);die();
			$BukuBesar->DoSaveJurnalTutupBukuRL($params, $subAcc, $no);
			// var_dump($result); die();
			$no++;
		}
	}

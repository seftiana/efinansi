...
select  
   ...
   CONCAT_WS('-',
   IFNULL(bbh.`bbhisSubaccPertamaKode`,'00'),
   IFNULL(bbh.`bbhisSubaccKeduaKode`,'00'),
   IFNULL(bbh.`bbhisSubaccKetigaKode`,'00'),
   IFNULL(bbh.`bbhisSubaccKeempatKode`,'00'),
   IFNULL(bbh.`bbhisSubaccKelimaKode`,'00'),
   IFNULL(bbh.`bbhisSubaccKeenamKode`,'00'),
   IFNULL(bbh.`bbhisSubaccKetujuhKode`,'00')) AS subAcc
from 
   buku_besar_his bbh 
...
SELECT
   c.coaId AS coa_id,  
   tpSaldoAkhir AS saldo_akhir,
   CONCAT_WS('-',
   IFNULL(tp.`tpSubaccPertamaKode`,'00'),
   IFNULL(tp.`tpSubaccKeduaKode`,'00'),
   IFNULL(tp.`tpSubaccKetigaKode`,'00'),
   IFNULL(tp.`tpSubaccKeempatKode`,'00'),
   IFNULL(tp.`tpSubaccKelimaKode`,'00'),
   IFNULL(tp.`tpSubaccKeenamKode`,'00'),
   IFNULL(tp.`tpSubaccKetujuhKode`,'00')) AS subAcc
FROM 
   tahun_pembukuan tp 
   JOIN coa c ON c.coaId = tp.tpCoaId 
   JOIN tahun_pembukuan_periode tpp ON tpp.tppId = tp.tpTppId 
WHERE 
   c.coaIsLabaRugiThAwal ='1'
   AND
   tppIsBukaBuku ='Y'
GROUP BY  coa_id, subAcc
";
...
SELECT
   ...
   SUM(IF(tppIsBukaBuku = 'Y' AND coaIsLabaRugiThJln ='1', 0, tpDebet)) AS debet,
   SUM(IF(tppIsBukaBuku = 'Y' AND coaIsLabaRugiThJln ='1', 0, tpKredit)) AS kredit,
   SUM(tpSaldoAkhir) AS tpSaldoAkhir
FROM coa
LEFT JOIN tahun_pembukuan ON tpCoaId = coaId
LEFT JOIN tahun_pembukuan_periode ON tppId = tpTppId
...
SELECT
   ...
   ctrNamaTipe,
   tpSaldoAwal,
   IF(tppIsBukaBuku = 'Y' AND coaIsLabaRugiThJln ='1', 0, tpDebet) AS debet,
   IF(tppIsBukaBuku = 'Y' AND coaIsLabaRugiThJln ='1', 0, tpKredit) AS kredit,
   tpSaldoAkhir
   [SUBACC_VIEW]
...
$sql['get_buku_besar_as_tahun_pembukuan'] = "
   SELECT
      ...
      (IFNULL(sa.saldo_awal,0) +  SUM(IF( bbh.bbPdId IS NULL AND bbh.bbPembukuanRefId IS NULL ,0,bbh.bbSaldo))) AS saldo_akhir
      [SUBACC_VIEW]
   FROM 
      buku_besar_his bbh 
      JOIN coa c ON(c.coaId= bbh.bbCoaId)
      ...
      LEFT JOIN (
         SELECT
            ...
            CONCAT_WS('-',
               ...
               IFNULL(`tphSubaccKetujuhKode`,'00')) AS sub_acc
         FROM `tahun_pembukuan_hist`
         JOIN coa c ON c.`coaId` = `tphCoaId`
         JOIN coa_kelompok ck ON ck.`coaKelompokId` = c.`coaCoaKelompokId`
         WHERE tphTppId = (SELECT tppId FROM tahun_pembukuan_periode WHERE tppId < '%s'  ORDER BY tppId DESC  LIMIT 0,1)
         GROUP BY tphCoaId, sub_acc
      ) sa ON c.coaId = sa.coa_id AND 
      CONCAT_WS('-',
         IFNULL(bbh.`bbhisSubaccPertamaKode`,'00'),
         IFNULL(bbh.`bbhisSubaccKeduaKode`,'00'),
         IFNULL(bbh.`bbhisSubaccKetigaKode`,'00'),
         ...
         IFNULL(bbh.`bbhisSubaccKelimaKode`,'00'),
         IFNULL(bbh.`bbhisSubaccKeenamKode`,'00'),
         IFNULL(bbh.`bbhisSubaccKetujuhKode`,'00')) = sa.sub_acc
   WHERE 
      c.coaId = '%s' 
      AND 
      bbh.bbTppId = '%s'

...
'module/rencana_kinerja_tahunan_kegiatan/business/AppPopupSatker.class.php';

class ViewPopupSatkerPimpinan extends HtmlResponse {
	...
	function TemplateModule() {
		...
		$this->SetTemplateFile('view_popup_satker_pimpinan.html');
	}
	...
	function ProcessRequest() {
		$this->popupSatkerObj = new AppPopupSatker(); 
		$POST = $_POST->AsArray();
		if(!empty($POST)) {
			$satker = $POST['satker'];
		...
		$unit = $this->popupSatkerObj->GetUnit();
		$totalData = $this->popupSatkerObj->GetCountDataSatker(
								$satker, $pimpinan,$unit[0]['kodeSistem']);

		//$totalData = $this->popupSatkerObj->GetCountDataSatker($satker, $pimpinan);
		...
		//$dataSatker = $this->popupSatkerObj->getDataSatker($startRec, $itemViewed, $satker, $pimpinan);
		$dataSatker = $this->popupSatkerObj->getDataSatker(
										$startRec, 
										$itemViewed, 
										$satker, 
										$pimpinan,
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH_POPUP', 

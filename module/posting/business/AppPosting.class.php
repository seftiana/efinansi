...
class AppPosting extends Database {
    ...
    * @return int
    */
   public function getStatePosting($subAkun){
        $statePosting = $this->Open($this->mSqlQueries['get_last_posting'], array($subAkun));
    ...
    */
   public function getParameterDate($subAkun)
   {
      $getDate       = getdate();
      ...
      $posting       = $this->Open($this->mSqlQueries['get_last_posting'], array($subAkun));
      $jurnal        = $this->Open($this->mSqlQueries['get_last_transaksi'], array($subAkun));

      if(empty($jurnal) OR $jurnal[0]['lastTransaksi'] === NULL){
         $return['last_transaksi']  = $return['first_transaksi'] = date('Y-m-d', mktime(0,0,0, $currmon, $currday, $currYear));
      } else {
   ...
   public function getJurnalDueDate($last_posting, $curr_date, $subAkun = "00-00-00-00-00-00-00")
   {
   ...
   public function getJurnalPembukuan($last_posting, $curr_date, $subAkun = "00-00-00-00-00-00-00")
   {
      // pastikan format tanggal sesuai
      $last_posting     = date('Y-m-d', strtotime($last_posting));
      $curr_date        = date('Y-m-d', strtotime($curr_date));
   ...
   public function getPembukuanLabaRugi()
   {
      ...
      if(empty($dataBukuBesar)){
         ...
         foreach($dataBukuBesar as $value){
            $return[$value['sub_akun']]['id']           = $value['bb_id'];
            $return[$value['sub_akun']]['saldo_awal']   = $value['saldo_awal'];
            $return[$value['sub_akun']]['saldo_akhir']  = $value['saldo_akhir'];
         }
   ...
   public function doPosting($curr_date, $subAkun = "00-00-00-00-00-00-00")
   {
      $date             = date('Y-m-d', strtotime($curr_date));
      $dateParameter    = $this->getParameterDate($subAkun);
      ...
      if(!empty($dataJurnal)){
         ...
         for ($i=0; $i < count($dataJurnal);) {
            if($coaId == (int)$dataJurnal[$i]['akun_id'].'-'.$dataJurnal[$i]['sub_account']){
               ...
               */
               ...
               $coaId         = (int)$dataJurnal[$i]['akun_id'].'-'.$dataJurnal[$i]['sub_account'];
      ...
      if(!empty($dataLabaRugi)){
         $saldoAwal = array();
         $saldoAwalLabaRugi = array();
         $saldoAkhir = array();
         $dataJurnalLabaRugi = array();

         if(!empty($pembukuanLabaRugi)){
            foreach($pembukuanLabaRugi as $key=>$value){
               $saldoAwal[$key] = $value['saldo_akhir'];
               $saldoAwalLabaRugi[$key] = $value['saldo_akhir'];
               $saldoAkhir[$key] = 0;
               $dataJurnalLabaRugi[$key]['id'] = $value['id'];
               $dataJurnalLabaRugi[$key]['saldo_awal'] = $value['saldo_akhir'];
               $dataJurnalLabaRugi[$key]['debet'] = 0;
               $dataJurnalLabaRugi[$key]['kredit'] = 0;
               $dataJurnalLabaRugi[$key]['saldo'] = 0;
               $dataJurnalLabaRugi[$key]['saldo_akhir'] = $value['saldo_akhir'];
            }
         }
         ...
         foreach ($dataLabaRugi as $laba_rugi) {
            ...
            */
            $subAkun = $laba_rugi['sub_account'];

            if(!isset($dataJurnalLabaRugi[$subAkun])){
               $saldoAwal[$subAkun] = 0;
               $saldoAwalLabaRugi[$subAkun] = 0;
               $saldoAkhir[$subAkun] = 0;
               $dataJurnalLabaRugi[$subAkun]['id'] = null;
               $dataJurnalLabaRugi[$subAkun]['saldo_awal'] = 0;
               $dataJurnalLabaRugi[$subAkun]['debet'] = 0;
               $dataJurnalLabaRugi[$subAkun]['kredit'] = 0;
               $dataJurnalLabaRugi[$subAkun]['saldo'] = 0;
               $dataJurnalLabaRugi[$subAkun]['saldo_akhir'] = 0;
            }
            ...
            */
            ...
            $dataJurnalLabaRugi[$subAkun]['sub_account']  = $laba_rugi['sub_account'];
            $dataJurnalLabaRugi[$subAkun]['saldo_awal']   = $saldoAwal[$subAkun];
            $dataJurnalLabaRugi[$subAkun]['debet']        += $debet;
            $dataJurnalLabaRugi[$subAkun]['kredit']       += $kredit;
            $dataJurnalLabaRugi[$subAkun]['saldo']        += $saldo;
            $dataJurnalLabaRugi[$subAkun]['saldo_akhir']  += $saldo;
            ...
            $dataPembukuanLabaRugi[$index]['saldo_awal']    = $saldoAwalLabaRugi[$subAkun];
            $dataPembukuanLabaRugi[$index]['debet']         = $debet;
            $dataPembukuanLabaRugi[$index]['kredit']        = $kredit;
            $dataPembukuanLabaRugi[$index]['saldo']         = $saldo;
            $dataPembukuanLabaRugi[$index]['saldo_akhir']   = $saldoAwalLabaRugi[$subAkun] + $saldo;
            $index++;
            $saldoAwalLabaRugi[$subAkun]   += $saldo;
         }
      }
      ...
      if(!empty($dataJurnalLabaRugi)){
         foreach($dataJurnalLabaRugi as $value){
            if($value['sub_account'] != "" && $value['sub_account'] != null){
               // extract sub account
               list($subacc_1, $subacc_2, $subacc_3, $subacc_4, $subacc_5, $subacc_6, $subacc_7)   = explode('-', $value['sub_account']);
               if($value['id'] === NULL OR $value['id'] == ''){
                  ...
                  $queryLog[]    = sprintf($this->mSqlQueries['do_insert_buku_besar_lr_sub_account'], $date, $subacc_1, $subacc_2, $subacc_3, $subacc_4, $subacc_5, $subacc_6, $subacc_7, $value['saldo_awal'], $value['saldo_awal'], $value['debet'], $value['debet'], $value['kredit'], $value['kredit'], $value['saldo'], $value['saldo'], $value['saldo_akhir'], $value['saldo_akhir'], $userId);
               }else{
                  ...
                  $queryLog[]    = sprintf($this->mSqlQueries['do_insert_buku_besar_lr_sub_account'], $date, $subacc_1, $subacc_2, $subacc_3, $subacc_4, $subacc_5, $subacc_6, $subacc_7, $value['saldo_awal'], $value['saldo_awal'], $value['debet'], $value['debet'], $value['kredit'], $value['kredit'], $value['saldo'], $value['saldo'], $value['saldo_akhir'], $value['saldo_akhir'], $userId, $value['id']);
               }
            }
         }

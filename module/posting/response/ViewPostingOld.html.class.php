...
class ViewPostingOld extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application', 'docroot') .
                'module/posting/template');
        $this->SetTemplateFile('view_posting_old.html');
    }
    ...
    function ProcessRequest() {
        ...
        $dateParameter = $Obj->getParameterDateOld($tahun); ##add ccp 29-5-2023
        $requestData['tahun'] = $tahun; ## add ccp 9-5-2023
        $requestData['last_posting'] = $dateParameter['last_posting'];
        $requestData['last_transaksi'] = $dateParameter['last_transaksi'];
        $dataJurnalDueDate = $Obj->getJurnalDueDateOld($requestData['last_posting'], $requestData['last_transaksi'],$tahun);
        $countJurnalDueDate = $Obj->countJurnalDueDateOld($requestData['last_posting'], $requestData['last_transaksi'],$tahun);
        $statePosting = $Obj->getStatePosting();
		
        $dataJurnal = $Obj->getJurnalPembukuanOld(
                $requestData['last_posting'], $requestData['last_transaksi'],$tahun
        );
...
// echo"<pre>";
// echo $statePosting."<br>";
		// print_r($dateParameter);
		// print_r($dataJurnal);
		// echo"</pre>";
    ...
    function ParseTemplate($data = NULL) {
        ...
        if ($statePosting == 1) {
            if ($gcJBP > 0) {
                ...
                $this->mrTemplate->AddVar('tgl_posting', 'TAHUN', $requestData['tahun']);
            }
        } else {
            $this->mrTemplate->AddVar('tgl_posting', 'AVAILABLE', 'DISABLED');
        }

        $this->mrTemplate->AddVar('content', 'URL_SUBMIT', $urlAction.'&tahun=2022');

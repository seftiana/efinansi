...
require_once GTFWConfiguration::GetValue('application', 'docroot').
	'module/kode_penerimaan/business/AppPopupJenisPembayaran.class.php';

class ViewPopupJenisPembayaran extends HtmlResponse 
{
	...
	public function TemplateModule()
	{
		...
		$this->SetTemplateFile('view_popup_jenis_pembayaran.html');
	}
    ...
    {
		$kodeObj = new AppPopupJenisPembayaran();
		...
		$totalData = $kodeObj->GetCountData($namaSumberDana);		
		$itemViewed = 20;
		$currPage = 1;
		$startRec = 0 ;
		...
		$dataList = $kodeObj->GetData($startRec, $itemViewed, $namaSumberDana);
	...
	public function ParseTemplate($data = NULL) 
	{
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', 
				Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, 'popupJenisPembayaran', 'view', 'html'));
		...
		if (empty($data['dataList'])) {
			...
			for ($i=0; $i<sizeof($dataList); $i++) {
				...
				$dataList[$i]['kode'] = $dataList[$i]['kode'];

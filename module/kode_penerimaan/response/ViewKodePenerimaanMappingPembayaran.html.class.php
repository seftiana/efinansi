...
class ViewKodePenerimaanMappingPembayaran extends HtmlResponse {

   protected $proc;

   function ViewKodePenerimaanMappingPembayaran(){
      $this->proc = new ProcessKodePenerimaanMappingPembayaran();
   }
   
   function TemplateModule() {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/kode_penerimaan/template');
      $this->SetTemplateFile('view_kode_penerimaan_mapping_pembayaran.html');
   } 
   ...
   function ProcessRequest() { 
	...
	function ParseTemplate($data = NULL) 
	{
		...
		if (empty($data['data'])) {
		 ...
		 $program_nomor='';
		 $no=$data['start'];
		// echo'uhuy<pre>';print_r($dataGrid);echo'</pre>';die;

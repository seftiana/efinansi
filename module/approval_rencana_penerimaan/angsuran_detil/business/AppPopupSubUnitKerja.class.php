...
class AppPopupSubUnitkerja extends Database {

   protected $mSqlFile= 'module/rencana_pengeluaran/business/app_popup_subunit_kerja.sql.php';
  ...
  function GetQueryKeren($sql,$params) {
	...
	{
		if($satker != ""):
			$str_satker 	= " AND ukr.unitkerjaParentId = $satker ";
		else:
			$str_satker 	= "";
		endif;
		if($tipeunit != ""):
			$str_tipeunit 	= " AND tukr.tipeunitId = $tipeunit ";
		else:
			$str_tipeunit 	= "";
		endif;

		//$sql = sprintf($this->mSqlQueries['get_data_unitkerja'], '%s', '%s', '%s', '%s', '%d', '%d');
		$result 			= $this->GetQueryKeren($this->mSqlQueries['get_data_unitkerja'],
									 array('%'.$kode.'%', '%'.$unitkerja.'%', 
									 $str_satker, $str_tipeunit, $offset, $limit));
		//echo $result;
	...
	function GetCountDataUnitkerja ($satker='', $kode='', $unitkerja='', $tipeunit='') {
		...
		$sql 	= $this->GetQueryKeren($this->mSqlQueries['get_count_data_unitkerja'], 
						 array('%'.$kode.'%', '%'.$unitkerja.'%', $str_satker, $str_tipeunit));

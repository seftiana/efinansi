...
class ViewBuktiTransaksi extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $this->SetFileName('bukti_transaksi_penerimaan_bank.xls');
      ...
      $sheet->setCellValue('H2', ': '.$mObj->_dateToIndo($data_transaksi['tanggal']));
      ...
      $sheet->setCellValue('A3', 'BUKTI PENERIMAAN BANK');
      $sheet->getStyle('A3')->getFont()->setUnderline(PHPExcel_Style_Font::UNDERLINE_SINGLE);
      ...
      $sheet->setCellValue('C5', ucwords($data_transaksi['nama_penyetor']));
      $sheet->setCellValue('A6', 'Kepada');
      $sheet->setCellValue('B6', ':');
      $sheet->setCellValueExplicit('C6', ucwords($data_transaksi['bank_penerima']), PHPExcel_Cell_DataType::TYPE_STRING);
      $sheet->setCellValue('A7', 'Banyaknya');
      $sheet->setCellValue('B7', ':');
      $sheet->setCellValue('C7', ' # '.strtoupper($data_transaksi['terbilang']).' # ');
      $getHeightRow = ceil(strlen($data_transaksi['terbilang'])/61.5) * 14;     
      $sheet->getRowDimension(7)->setRowHeight($getHeightRow);
      ...
      $sheet->setCellValue('A10', strtoupper($data_transaksi['keterangan']).' :');
  
      $getHeightRow = ceil(strlen($data_transaksi['keterangan'])/43) * 17;
      
      $sheet->getRowDimension(10)->setRowHeight($getHeightRow);

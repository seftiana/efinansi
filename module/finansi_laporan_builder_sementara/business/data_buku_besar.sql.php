...
LEFT JOIN (
  SELECT
    bhis.`bbCoaId` AS coa_id,
    MAX(bhis.`bbhisId`) AS last_id
  FROM `buku_besar_his` bhis
   
  WHERE bhis.`bbTppId` = '%s'
  GROUP BY bhis.`bbCoaId`
) AS last_trans
  ON c.`coaId` = last_trans.`coa_id`
  AND bhis.`bbhisId` = last_trans.`last_id`
WHERE 
  c.`coaId` = last_trans.`coa_id`
  AND bhis.`bbhisId` = last_trans.`last_id`
";

$sql['get_data_buku_besar_saldo_mutasi_sudah_posting'] ="
SELECT
  bhis.`bbTppId` AS tpp_id,
  bhis.`bbCoaId` AS coa_id,
  c.`coaNamaAkun` AS coa_nama,
...
WHERE
    ...
    AND (IF(bhis.`bbTppId` > 10, transCatatan NOT LIKE 'Tutup Buku - Kenaikan (Penurunan) Aktiva Bersih Periode Berjalan%%', 1=1))

...
class ViewJurnalPenerimaan extends HtmlResponse
{
	...
	function ProcessRequest() 
	{
		...
		else
		{
			...
			$this->data = $this->proc->db->GetData($tgl_awal, $tgl_akhir, $startRec, $itemViewed);
			$totalData = $this->proc->db->GetCount($tgl_awal, $tgl_akhir);
		}
	...
	function ParseTemplate($data = NULL) 
	{
		...
		else
		{
			...
			for ($i = 0;$i < sizeof($dataGrid);$i++) 
			{
				...
				#$dataGrid[$i]['url_detail']= Dispatcher::Instance()->GetUrl($this->proc->moduleName, 'jurnal_penerimaanDetail', 'view', 'html').'&grp=' .$idEnc;
				$dataGrid[$i]['url_edit'] = Dispatcher::Instance()->GetUrl($this->proc->moduleName, 'inputJurnalPenerimaan', 'view', 'html') . '&grp=' . $idEnc;
				$dataGrid[$i]['url_jurnalbalik'] = Dispatcher::Instance()->GetUrl('jurnal_penerimaan', 'JurnalPenerimaan', 'view', 'html') . '&balik=' . Dispatcher::Instance()->Encrypt('yes') . '&grp=' . $idEnc;
				...
				if (strtoupper($dataGrid[$i]['tipeakun']) == 'D') $dataGrid[$i]['debet'] = number_format($dataGrid[$i]['nilai'], 2, ',', '.');
				elseif (strtoupper($dataGrid[$i]['tipeakun']) == 'K') $dataGrid[$i]['kredit'] = number_format($dataGrid[$i]['nilai'], 2, ',', '.');
				$dataGrid[$i]['tanggal'] = $this->proc->db->date2string($dataGrid[$i]['tanggal']);
				
				if ($dataGrid[$i]['is_posting'] == 'Y') $dataGrid[$i]['class_name'] = 'table-common-even2';
				else $dataGrid[$i]['class_name'] = '';

...
class JurnalPenerimaan extends Database {

   protected $mSqlFile= 'module/jurnal_penerimaan/business/jurnalpenerimaan.sql.php';
...
//==GET==      
   ...
   function GetData($tgl_awal, $tgl_akhir, $start , $count){
      #
     $result = $this->open($this->mSqlQueries['get_data'],array($tgl_awal, $tgl_akhir, $start , $count));	  	  
   #echo $this->GetLastError();
   return $result;
   }
   
   function GetDataAll($start , $count){
     return $this->open($this->mSqlQueries['get_data_all'],array($start , $count));        
   }
   
   function GetCount($tgl_awal, $tgl_akhir){
    $tot = $this->open($this->mSqlQueries['get_count'],array($tgl_awal, $tgl_akhir));     
     if(!empty($tot)) {
       return sizeof($tot);
     } else {
       return false;  
   ...
   function GetDataById($id){
      $ret = $this->open($this->mSqlQueries['get_data_by_id'],array($id));	  
	  return $ret;
   } 
...
//===DO==
   function DoAddDetail($pembukuan_id,$coa_id, $nilai , $keterangan, $tipe , &$sql = ''){
      $ret =  $this->Execute($this->mSqlQueries['do_add_pembukuan_detail'], array($pembukuan_id, $coa_id, $nilai, $keterangan, $tipe));
      if($ret) {
   ...
   function DoAdd($data,&$msgerr){
      ...
      $ok = $this->Execute($this->mSqlQueries['do_add_pembukuan_referensi'], array($data['referensi_id'], $user_id, date('Y-m-d'), $data['referensi_nama']));
	  ...
	  if($ok){	     		 
		 ...
		 if(is_array($data['kredit']['tambah']) && $ok) {		    
		    foreach($data['kredit']['tambah'] as $val){			   
			   $detok = $this->DoAddDetail($pembukuan_id,
			                               $val['id'],
										   $val['nilai'],
										   $val['keterangan'],
										   'K',$sqlret
	  ...
	  if($ok)
	     $this->DoAddLog('Tambah Jurnal Penerimaan', $sql);
   ...
   function DoUpdate($data,&$msgerr){ 
	  ...
	  if(isset($data['deleted']['id']) && $ok){
	     ...
	     foreach($data['deleted']['id'] as $val){
		    $delok=$this->Execute($this->mSqlQueries['do_delete_pembukuan_detail_single'], array($val));		
	  ...
	  //update debetnya
	  if($ok) {
	     $ok = $this->DoUpdateDetail($data['debet']['coa_id'],
		                             $data['debet']['nilai'],
									 $data['referensi_keterangan'],
									 $data['debet']['detail_id'] , $sqlret
			                         );
	     $sql[] = $sqlret;
	  } else
	    $msgerr = 'gagal update jurnal';
	  ...
	  $this->EndTrans($ok);
	  if($ok)
	     $this->DoAddLog('Update Jurnal Penerimaan',$sql);
	  return $ok;	  
   }
   ...
   function DoDelete($id){
	...
	$this->EndTrans($ok);
	if($ok)
	   $this->DoAddLog('Delete Jurnal Penerimaan',$sql);

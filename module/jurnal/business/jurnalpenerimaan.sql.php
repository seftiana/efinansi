...
ORDER BY pr.prTanggal DESC , pr.prId DESC, t.transId DESC, pd.pdStatus ASC
...
WHERE 
   t.transTtId IN (1,5)
   AND pr.prIsJurnalBalik <> 1
ORDER BY pr.prTanggal DESC , t.transId DESC, pd.pdStatus ASC
LIMIT %s, %s
";
...
WHERE 
   pr.prId IN (
         ...
         WHERE t.transTtId IN (1,5) AND prTanggal BETWEEN %s AND %s
      )
   AND pr.prIsJurnalBalik <> 1
GROUP BY prId
";

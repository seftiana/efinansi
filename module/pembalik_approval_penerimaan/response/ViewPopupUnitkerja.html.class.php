...
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
	'module/pembalik_approval_penerimaan/business/AppPopupUnitkerja.class.php';
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 
	...
	function ParseTemplate($data = NULL) {
		...
		if (empty($data['dataUnitkerja'])) {
			...
			for ($i=0; $i<sizeof($dataUnitkerja); $i++) {
				...
				$dataUnitkerja[$i]['link'] = $dataUnitkerja[$i]['unit'];
				//if ($no % 2 != 0) $dataUnitkerja[$i]['class_name'] = 'table-common-even';
				//else $dataUnitkerja[$i]['class_name'] = '';
				if($this->unitkerjaObj->GetTotalSubUnitKerja($dataUnitkerja[$i]['id'])) {

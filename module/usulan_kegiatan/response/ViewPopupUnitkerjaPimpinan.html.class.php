<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/usulan_kegiatan/business/AppPopupUnitkerja.class.php';

class ViewPopupUnitkerjaPimpinan extends HtmlResponse {

	var $Pesan;

	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/usulan_kegiatan/template');
		$this->SetTemplateFile('view_popup_unitkerja_pimpinan.html');
	}
   ...
   function TemplateBase() {
	...
	//view
		...
		$dataUnitkerja = $unitkerjaObj->getDataUnitkerja($startRec, $itemViewed, $satker, $kode, $unitkerja, $tipeunit,$satker);
	...
	function ParseTemplate($data = NULL) {
		...
		Dispatcher::Instance()->GetUrl('usulan_kegiatan', 'popupUnitkerjaPimpinan', 'view', 'html');
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('usulan_kegiatan', 'popupUnitkerjaPimpinan', 'view', 'html') . "&satker=" . Dispatcher::Instance()->Encrypt($search['satker']) . "&satker_label=" . Dispatcher::Instance()->Encrypt($search['satker_label']));
		if($this->Pesan) {

...
class ViewExcelLapBukubesar extends XlsxResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $requestData['sub_account'] = Dispatcher::Instance()->Decrypt($_GET['sub_account']);

      if($requestData['sub_account'] == '01-00-00-00-00-00-00'){
         $header = strtoupper(GTFWConfiguration::GetValue('organization', 'header_lap_yayasan'));
      }elseif($requestData['sub_account'] == '00-00-00-00-00-00-00'){
      ...
      $dataSaldo     = $Obj->getTotalSaldo((array) $requestData); 
      ...
      if (empty($dataList)) {
         ...
         $sheet->setCellValueByColumnAndRow(0, 2, 'Laporan Buku Besar');
         $sheet->mergeCells('A2:I2');

         $sheet->setCellValueByColumnAndRow(0, 4, 'Tanggal Transaksi : '. IndonesianDate($requestData['start_date'], 'yyyy-mm-dd') .' s/d '. IndonesianDate($requestData['end_date'], 'yyyy-mm-dd'));
         $sheet->mergeCells('A4:I4');
         $sheet->mergeCells('A5:I5');
         $sheet->mergeCells('A6:I6');
         $sheet->setCellValueByColumnAndRow(0, 5, 'Nama Rekening : '. $info_coa['rekening']);
         $sheet->setCellValueByColumnAndRow(0, 6, 'Nomor Rekening : '. $info_coa['no_rekening']);
         $sheet->getStyle('A1:I6')->applyFromArray(array(
            'font' => array(
               'bold' => true
            ), 'alignment' => array(
        ...
        */
         ...
         for($k = 0; $k < $max;){
            ...
            if($kodeAkun == $dataList[$k]['akun_kode']) {  
               ...
               //  $sheet->setCellValueByColumnAndRow(5, $num,'=(F'.($num - 1 ).'+D'.$num.'-E'.$num.')');  
                $sheet->setCellValueByColumnAndRow(5, $num,$saldoAkhir);  
         ...
         $sheet->getStyle('A8:F'.($num - 1))->applyFromArray(array(
            'borders' => array(
               'allborders' => array(
         ...
         $sheet->getStyle('B8:B'.($num - 1))->applyFromArray(array(
            'alignment' => array(
               'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER
            )
         ));
         $sheet->getStyle('D9:F'.($num - 1))->applyFromArray(array(
            'alignment' => array(

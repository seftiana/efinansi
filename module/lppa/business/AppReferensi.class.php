...
class AppReferensi extends Database
{
   ...
   {
      $this->mSqlFile   = 'module/lppa/business/app_referensi.sql.php';
   ...
   public function getTipeUnit()
   {
    ...
    public function GetDetailBelanjaFpa()
    {       
        $result = $this->Open($this->mSqlQueries['get_detail_belanja_fpa'], array());
        foreach($result as $k => $data) {
            $datax[$data['pid']][] = $result[$k];
        }        
        return $datax;
    }

    public function GetDetailBelanjaFpaByTaUnit($taId,$unitId)
    {//$this->SetDebugOn();
        $query = $this->mSqlQueries['get_detail_belanja_fpa'] .  $this->mSqlQueries['get_detail_belanja_fpa_where_2'];       
        $result = $this->Open($query, array($taId,$unitId));        
        foreach($result as $k => $data) {
          $datax[$data['pid']][] = $result[$k];
        }        
        return $datax;
    ...
    public function GetDetailBelanjaFpaById($idFpa,$idLppa)
    {//$this->SetDebugOn();
        $query = $this->mSqlQueries['get_detail_belanja_fpa'] .  $this->mSqlQueries['get_detail_belanja_fpa_where'];
        $flag =  (!empty($idLppa) ? 0 : 1) ;  
        $result = $this->Open($query, array($idFpa,$idLppa,$flag));
        return $result ;
    }  

...
class ViewList extends HtmlResponse
{
   ...
   function ParseTemplate ($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_KEMBALI', Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, 'input', 'view', 'html'));
      if($this->Pesan) {
      ...
      if (empty($data['dataDetilTransaksi'])) {
         ...
         $label = 'Transaksi Aset';
         $urlDelete = 'transaksi_aset|DeleteTransaksiAset|do|html';
         $urlReturn = 'transaksi_aset|DetilTransaksiPengadaan|view|html';
         $URLDELETE = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, 'DeleteTransaksiAset', 'do', 'html');
...
//mulai bikin tombol delete
         ...
         for ($i=0; $i<sizeof($dataDetilTransaksi); $i++) {
            ...
            '&id='.Dispatcher::Instance()->Encrypt($dataDetilTransaksi[$i]['id']).
            '&label='.$label.'&dataName='.$dataDetilTransaksi[$i]['kkb']; 
               $url_edit = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, 'TransaksiAset', 'view', 'html') . '&dataId=' . $idEnc;

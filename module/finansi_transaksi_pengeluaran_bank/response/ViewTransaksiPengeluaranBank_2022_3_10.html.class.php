...
class ViewTransaksiPengeluaranBank extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application', 'docroot') . 
                    'module/finansi_transaksi_pengeluaran_bank/template');
        $this->SetTemplateFile('view_transaksi_pengeluaran_bank.html');
    }
    ...
    function ProcessRequest() {
        ...
        $data = $mObj->getDataJurnal($offset, $limit, (array) $requestData);
    ...
    function ParseTemplate($data = NULL) {
        ...
        $urlSearch = Dispatcher::Instance()->GetUrl(
                'finansi_transaksi_pengeluaran_bank', 'TransaksiPengeluaranBank', 'view', 'html'
        );

        $urlAdd = Dispatcher::Instance()->GetUrl(
                        'finansi_transaksi_pengeluaran_bank', 'Transaksi', 'view', 'html'
                ) . '&' . $queryString;

        $urlEdit = Dispatcher::Instance()->GetUrl(
                        'finansi_transaksi_pengeluaran_bank', 'editTransaksi', 'view', 'html'
                ) . '&' . $queryString;
        ...
        if (empty($dataList)) {
            ...
            for ($i = 0; $i < count($dataList);) {
                if ((int) $transaksiId === (int) $dataList[$i]['id'] && (int) $pembukuanId === (int) $dataList[$i]['pembukuan_id']) {
                    ...
                    $dataJurnal[$kodeSistem][$idx]['sppu_id'] = $dataList[$i]['sppu_id'];
            ...
            foreach ($dataJurnal as $grid) {
                foreach ($grid as $jurnal) {
                    if ($jurnal['type'] AND strtoupper($jurnal['type']) == 'PARENT') {
                        ...
                        // url delete
                        $urlAccept = 'finansi_transaksi_pengeluaran_bank|DeleteTransaksi|do|json-search|' . $keyUrl . '-1|' . $valueUrl;
                        $urlReturn = 'finansi_transaksi_pengeluaran_bank|TransaksiPengeluaranBank|view|html-search|' . $keyUrl . '-1|' . $valueUrl;
                        ...
                        $jurnal['url_delete'] = Dispatcher::Instance()->GetUrl(
                                ...
                                . '&urlReturn=' . $urlReturn
                                . '&id=' . $jurnal['id'] . '.' . $jurnal['pembukuan_id'] . '.' . $jurnal['sppu_id']
                                . '&label=' . $label
                        ...
                        if ($jurnal['sppu_id'] === '0') {
                            $this->mrTemplate->AddVar('content_links_excel', 'IS_SPPU', 'T');
                        } else {
                            $this->mrTemplate->AddVar('content_links_excel', 'IS_SPPU', 'Y');
                        }
                        ...
                        $this->mrTemplate->AddVar('content_links_excel', 'URL_EXPORT', $urlExport . '&data_id=' . Dispatcher::Instance()->Encrypt($jurnal['id']) . '&pr_id=' . Dispatcher::Instance()->Encrypt($jurnal['pembukuan_id']));

...
class AppReferensi extends Database
{
   ...
   {
      $this->mSqlFile   = 'module/finansi_transaksi_pengeluaran_bank/business/app_referensi.sql.php';
   ...
   //dipakai
   public function getDataCoaSppu()
   {
        $result = array();      
        $getCoa     = $this->Open($this->mSqlQueries['get_data_coa_sppu'], array());
        if(!empty($getCoa)) {
            foreach($getCoa as $data) {
                $result[$data['sppuId']][] = array(
                    'coa_id' => $data['coaId'],
                    'coa_kode' => $data['coaKode'],
                    'coa_nama' => $data['coaNama'],
   ...
   //dipakai
   public function getReferensiSppu($offset, $limit, $param = array())
   {
      $return     = $this->Open($this->mSqlQueries['get_data_referensi_sppu'], array(
         '%'.$param['kode'].'%',
         $offset,
         $limit
      ));
